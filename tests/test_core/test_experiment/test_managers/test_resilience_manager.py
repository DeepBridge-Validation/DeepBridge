"""
Comprehensive tests for ResilienceManager.

This test suite validates:
1. run_tests - running resilience tests on primary model
2. compare_models - comparing resilience across models
...
def manager(mock_dataset):
    """Create ResilienceManager instance"""
    return ResilienceManager(dataset=mock_dataset)


@pytest.fixture
def verbose_manager(mock_dataset):
    """Create ResilienceManager with verbose=True"""
    return ResilienceManager(dataset=mock_dataset, verbose=True)
...
class TestRunTests:
    ...
    def test_run_tests_with_custom_metric(self, mock_run_tests, manager, mock_dataset):
        ...
        result = manager.run_tests(metric='accuracy')
    ...
    def test_run_tests_with_both_config_and_metric(
        ...
        manager.run_tests(config_name='full', metric='f1')
...
class TestCompareModels:
    ...
    def test_compare_models_with_no_alternatives(
        ...
        assert result['primary_model'] == {'resilience_score': 0.85}
    ...
    def test_compare_models_with_alternatives(
        ...
        mock_run_tests.side_effect = [
            {'resilience_score': 0.85},  # primary
            {'resilience_score': 0.80},  # model1
            {'resilience_score': 0.82},  # model2
        ]
        ...
        assert result['alternative_models']['model1'] == {'resilience_score': 0.80}
        assert result['alternative_models']['model2'] == {'resilience_score': 0.82}
    ...
    def test_compare_models_with_custom_metric(
        ...
        manager_with_alternatives.compare_models(metric='accuracy')

        # Check all calls used 'accuracy' metric
        for call in mock_run_tests.call_args_list:
            assert call[1]['metric'] == 'accuracy'
...
class TestIntegration:
    ...
    def test_full_workflow_run_then_compare(
        ...
        mock_run_tests.side_effect = [
            {'resilience_score': 0.85},  # run_tests
            {'resilience_score': 0.85},  # compare primary
            {'resilience_score': 0.80},  # compare model1
            {'resilience_score': 0.82},  # compare model2
        ]
