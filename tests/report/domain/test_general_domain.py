...
class TestReportSection:
    ...
    def test_primary_charts_property(self):
        ...
        section.add_chart(ChartSpec(id="c1", type=ChartType.LINE, title="C1", data={}, is_primary=True))
        section.add_chart(ChartSpec(id="c2", type=ChartType.BAR, title="C2", data={}, is_primary=False))
...
class TestReport:
    ...
    def test_add_section(self):
        ...
        metadata = ReportMetadata(model_name="Test", test_type=ReportType.ROBUSTNESS)
        report = Report(metadata=metadata)
    ...
    def test_add_summary_metric(self):
        ...
        metadata = ReportMetadata(model_name="Test", test_type=ReportType.UNCERTAINTY)
        report = Report(metadata=metadata)
    ...
    def test_get_section(self):
        ...
        metadata = ReportMetadata(model_name="Test", test_type=ReportType.RESILIENCE)
        report = Report(metadata=metadata)
    ...
    def test_get_all_charts(self):
        ...
        section = ReportSection(id="sec1", title="Section 1")
        section.add_chart(ChartSpec(id="c1", type=ChartType.LINE, title="C1", data={}))
        section.add_chart(ChartSpec(id="c2", type=ChartType.BAR, title="C2", data={}))

        subsection = ReportSection(id="sub1", title="Subsection")
        subsection.add_chart(ChartSpec(id="c3", type=ChartType.COVERAGE, title="C3", data={}))
        section.add_subsection(subsection)
    ...
    def test_display_title_auto(self):
        ...
        metadata = ReportMetadata(model_name="MyModel", test_type=ReportType.ROBUSTNESS)
        report = Report(metadata=metadata)
...
class TestDomainModelIntegration:
    ...
    def test_build_complete_report(self):
        ...
        report.add_summary_metric(
            Metric(name="overall_uncertainty", value=0.85, is_primary=True, type=MetricType.PERCENTAGE)
        )
        ...
        results_section.add_metric(
            Metric(name="coverage_90", value=0.92, description="Coverage at 90%")
        )
        results_section.add_metric(
            Metric(name="mean_width", value=0.15, description="Mean interval width")
        )
        ...
        assert len(report.summary_metrics) == 1
        assert len(report.get_all_metrics()) == 4  # 1 summary + 2 in main + 1 in sub
