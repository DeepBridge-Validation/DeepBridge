...
class TestMarkdownAdapter:
    ...
    def test_adapter_with_custom_settings(self):
        ...
        adapter = MarkdownAdapter(
            include_toc=False, heading_level_start=2, chart_placeholder='link'
        )
    ...
    def test_create_anchor(self):
        ...
        assert (
            adapter._create_anchor('Results & Analysis') == 'results--analysis'
        )
    ...
    def test_validation_error_no_metadata(self):
        ...
        with pytest.raises(ValidationError):
            report = Report(metadata=None, title='Test')  # Invalid
...
class TestPDFAdapter:
    ...
    def test_adapter_with_custom_settings(self):
        """Test adapter with custom settings."""
        adapter = PDFAdapter(theme='custom', page_size='Letter')
    ...
    def test_render_includes_content(self, sample_report):
        ...
        assert (
            'TestModel' in content or len(result) > 1000
        )  # Basic sanity check
    ...
    def test_get_static_chart_type(self):
        ...
        assert (
            adapter._get_static_chart_type('width_vs_coverage')
            == 'width_vs_coverage_static'
        )
        assert (
            adapter._get_static_chart_type('perturbation_impact')
            == 'perturbation_impact_static'
        )
        assert (
            adapter._get_static_chart_type('unknown_chart') == 'unknown_chart'
        )
    ...
    def test_validation_error_no_model_name(self):
        ...
        with pytest.raises(ValidationError):
            metadata = ReportMetadata(
                model_name=None, test_type=ReportType.UNCERTAINTY  # Invalid
            )
    ...
    def test_pdf_adapter_with_nested_sections(self):
        ...
        report = Report(metadata=metadata, title='Nested Report')

        # Create parent section
        parent = ReportSection(id='parent', title='Parent Section')

        # Create child section
        child = ReportSection(id='child', title='Child Section')
...
class TestMultiFormatGeneration:
    ...
    def test_generate_all_formats(self, sample_report):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            md_path = md_adapter.save_to_file(
                md_content, str(tmpdir / 'report.md')
            )

            # PDF
            pdf_adapter = PDFAdapter()
            pdf_bytes = pdf_adapter.render(sample_report)
            pdf_path = pdf_adapter.save_to_file(
                pdf_bytes, str(tmpdir / 'report.pdf')
            )
