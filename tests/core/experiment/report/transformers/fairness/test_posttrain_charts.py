...
class TestDisparateImpactGaugeChart:
    ...
    def test_create_with_no_disparate_impact(self, sample_protected_attrs):
        ...
        assert result == '{}', "Should return empty JSON when disparate_impact is missing"
    ...
    def test_gauge_colors_based_on_compliance(self, sample_protected_attrs):
        ...
        result_compliant = chart.create({
            'posttrain_metrics': {
                'gender': {
                    'disparate_impact': {'ratio': 0.85, 'passes_80_rule': True}
                }
            },
            'protected_attrs': sample_protected_attrs
        })
        assert result_compliant != '{}', "Should create chart for compliant ratio"

        # Test with warning ratio (0.7-0.8)
        result_warning = chart.create({
            'posttrain_metrics': {
                'gender': {
                    'disparate_impact': {'ratio': 0.75, 'passes_80_rule': False}
                }
            },
            'protected_attrs': sample_protected_attrs
        })
        ...
        result_critical = chart.create({
            'posttrain_metrics': {
                'gender': {
                    'disparate_impact': {'ratio': 0.65, 'passes_80_rule': False}
                }
            },
            'protected_attrs': sample_protected_attrs
        })
        assert result_critical != '{}', "Should create chart for critical ratio"
...
class TestDisparityComparisonChart:
    ...
    def test_create_with_no_statistical_parity(self, sample_protected_attrs):
        ...
        assert result == '{}', "Should return empty JSON when statistical_parity is missing"

    def test_diverging_bar_with_positive_negative_disparities(self, sample_protected_attrs, plotly_validator):
        ...
        assert plotly_validator(result), "Should handle both positive and negative disparities"
...
class TestComplianceStatusMatrixChart:
    ...
    def test_matrix_includes_all_main_metrics(self, sample_protected_attrs, plotly_validator):
        ...
        metrics = {
            'gender': {
                ...
                'false_negative_rate_difference': {'interpretation': '✗ CRITICAL'}
            }
        }
        ...
        assert plotly_validator(result), "Should create valid chart with all main metrics"
    ...
    def test_status_symbols_mapping(self, sample_protected_attrs):
        ...
        metrics = {
            'gender': {
                ...
                'disparate_impact': {'interpretation': 'GOOD'},
                'false_negative_rate_difference': {'interpretation': 'Unknown'}
            }
        }
