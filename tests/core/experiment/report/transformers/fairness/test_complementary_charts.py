...
class TestPrecisionAccuracyComparisonChart:
    ...
    def test_create_with_valid_data(
        self, sample_confusion_matrix, sample_protected_attrs, plotly_validator
    ):
    ...
    def test_create_with_empty_confusion_matrix(self, sample_protected_attrs):
        ...
        chart = PrecisionAccuracyComparisonChart()
        result = chart.create(
            {'confusion_matrix': {}, 'protected_attrs': sample_protected_attrs}
        )

        assert (
            result == '{}'
        ), 'Should return empty JSON for empty confusion matrix'

    def test_calculates_precision_and_accuracy(
        ...
        # Perfect predictions
        cm_perfect = {
            'gender': {'Male': {'TP': 50, 'TN': 50, 'FP': 0, 'FN': 0}}
        }
...
class TestComplementaryMetricsRadarChart:
    ...
    def test_includes_complementary_metrics(
        self, sample_protected_attrs, plotly_validator
    ):
