...
def extract_base64_from_log(log_file: str, chart_name: str) -> Optional[str]:
    ...
    try:
        ...
        chart_pattern = re.compile(
            rf'Chart {re.escape(chart_name)} has data of length (\d+)'
        )
        matches = chart_pattern.findall(log_content)

        if matches:
            logger.info(
                f'Found chart {chart_name} with data length {matches[0]}'
            )
        ...
        logger.warning(
            f'Could not find base64 data for chart {chart_name} in the log file'
        )
        return None
...
def fix_html_report(
    html_file: str,
    chart_name_mapping: Dict[str, str],
    ...
    try:
        ...
        charts_dir = os.path.join(
            os.path.dirname(html_file), 'uncertainty_charts'
        )
        ...
        logger.info(
            f'Found {len(chart_refs)} chart references in the HTML: {chart_refs}'
        )
        ...
        for chart_name, template_name in chart_name_mapping.items():
            ...
            if pattern in html_content:
                ...
                if f'src="data:image/png;base64,' in html_content:
                    logger.info(
                        f'Chart {template_name} already has image data'
                    )
                    continue
                ...
                if base64_data:
                    ...
                    img_tag = (
                        f'<img src="{base64_data}" alt="{template_name}" />'
                    )
                    html_content = html_content.replace(pattern, img_tag)
                    fixed = True
                    logger.info(
                        f'Added image for {template_name} from log data'
                    )
    ...
    try:
        ...
        for chart_name in missing_charts:
            ...
            if pattern in html_content:
                ...
                if 'comparison' in chart_name:
                    ...
                    'distribution' in chart_name
                    or 'bandwidth' in chart_name
                    or 'widths' in chart_name
                ):
        ...
        if fixed:
            ...
            with open(html_file, 'w', encoding='utf-8') as f:
                f.write(html_content)
            logger.info(
                f'Successfully added placeholder charts to HTML report: {html_file}'
            )
            return True
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Post-process uncertainty reports to fix missing charts'
    )
    parser.add_argument('html_file', help='Path to the HTML report to fix')
    parser.add_argument(
        '--log', help='Optional path to a log file to extract chart data from'
    )
    parser.add_argument(
        '--placeholder',
        action='store_true',
        help='Create placeholder charts for missing charts',
    )
    ...
    if args.placeholder and not fixed:
        ...
        if missing_charts:
            logger.info(
                f'Found {len(missing_charts)} missing charts: {missing_charts}'
            )
