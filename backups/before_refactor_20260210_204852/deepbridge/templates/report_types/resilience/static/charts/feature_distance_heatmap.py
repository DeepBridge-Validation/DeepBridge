...
class FeatureDistanceHeatmapChart(BaseChartGenerator):
    ...
    def generate(
        ...
        if not self._validate_data(feature_distances):
            logger.warning(
                'Invalid or empty feature distances data for heatmap'
            )
            return ''
        ...
        sorted_features = sorted(
            combined_features.items(), key=lambda x: x[1], reverse=True
        )
        top_features = [
            feature
            for feature, _ in sorted_features[
                : min(top_n, len(sorted_features))
            ]
        ]

        if not top_features:
            logger.warning(
                'No features available for heatmap after processing'
            )
            return ''
        ...
        if self.chart_generator and hasattr(
            ...
            try:
                ...
                logger.error(
                    f'Error using chart generator for feature distance heatmap: {str(e)}'
                )
        ...
        try:
            ...
            pivot_df = df.pivot(
                index='Feature', columns='Metric', values='Distance'
            )
