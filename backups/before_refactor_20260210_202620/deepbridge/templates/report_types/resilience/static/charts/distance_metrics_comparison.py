...
class DistanceMetricsComparisonChart(BaseChartGenerator):
    ...
    def generate(
        ...
        if not self._validate_data(alpha_levels) or not self._validate_data(
            ...
            logger.warning(
                'Invalid alpha levels or metrics data for distance metrics comparison'
            )
            return ''
        ...
        if not clean_alphas:
            logger.warning(
                'No valid numeric alpha levels for distance metrics comparison'
            )
            return ''
        ...
        if not clean_metrics:
            logger.warning(
                'No valid metrics data for distance metrics comparison'
            )
            return ''
            ...
            try:
                ...
                for metric_name, values in clean_metrics.items():
                    if len(values) < len(clean_alphas):
                        ...
                        padded_values = values + [float('nan')] * (
                            len(clean_alphas) - len(values)
                        )
                        chart_data['series'].append(
                            {'name': metric_name, 'values': padded_values}
                        )
                        ...
                        chart_data['series'].append(
                            {'name': metric_name, 'values': values}
                        )
                ...
                logger.error(
                    f'Error using chart generator for distance metrics comparison: {str(e)}'
                )
        ...
        try:
            ...
            ax.xaxis.set_major_formatter(
                self.plt.FuncFormatter(lambda x, _: f'{x:.2f}')
            )
