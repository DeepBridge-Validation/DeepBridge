...
class ReliabilityRegionsChart(BaseChartGenerator):
    ...
    def generate(
        ...
        try:
            ...
            fig, axes = self.plt.subplots(
                n_features, 1, figsize=(12, 4 * n_features)
            )
            if n_features == 1:
            ...
            for idx, feature_name in enumerate(features_to_plot):
                ...
                if 'bins' not in data or 'confidence_scores' not in data:
                    logger.warning(
                        f'Missing required data for feature {feature_name}'
                    )
                    continue
                ...
                ax.set_title(
                    f'Reliability Regions for {feature_name}',
                    fontsize=12,
                    fontweight='bold',
                )
                ax.set_ylim([0, 1])
            ...
            logger.error(
                f'Error generating reliability regions chart: {str(e)}'
            )
            return ''
    ...
    def generate_comparison(
        self,
        reliability_data: Dict[str, Dict[str, Any]],
        title: str = 'Feature Confidence Comparison',
    ) -> str:
        """
        ...
        try:
            ...
            if not features:
                logger.warning(
                    'No features with confidence data for comparison'
                )
                return ''
            ...
            logger.error(
                f'Error generating confidence comparison chart: {str(e)}'
            )
            return ''
