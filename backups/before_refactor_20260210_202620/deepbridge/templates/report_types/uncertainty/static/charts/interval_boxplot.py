        ...
        try:
            ...
            for name, widths in zip(model_names, all_widths):
                df_temp = pd.DataFrame(
                    {'Model': [name] * len(widths), 'Width': widths}
                )
            ...
            sns.boxplot(
                ...
                medianprops=dict(color='black', linewidth=2),
                flierprops=dict(
                    marker='o', markerfacecolor='gray', markersize=3, alpha=0.5
                ),
            )

            # Sample points for strip plot (limit to avoid overcrowding)
            n_samples_per_model = min(
                100, len(df_combined) // len(model_names)
            )
            if n_samples_per_model > 10:
                df_sample = df_combined.groupby('Model').sample(
                    n=n_samples_per_model, replace=False, random_state=42
                )
                sns.stripplot(
                    data=df_sample,
                    x='Model',
            ...
            ax2.set_title(
                'Detailed Distribution (Violin + Box + Points)',
                fontsize=14,
                fontweight='bold',
            )
