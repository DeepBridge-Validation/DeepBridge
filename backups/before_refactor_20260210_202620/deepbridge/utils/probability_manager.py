...
class DatabaseProbabilityManager:
    ...
    def _validate_dataset(self):
        ...
        if (
            not hasattr(self.dataset, 'original_prob')
            or self.dataset.original_prob is None
        ):
            if self.verbose:
        ...
        if self.verbose:
            print(
                f'Dataset probability info - type: {type(self.dataset.original_prob)}'
            )
            if isinstance(self.dataset.original_prob, pd.DataFrame):
                print(
                    f'Probability columns: {self.dataset.original_prob.columns.tolist()}'
                )
                print(
                    f'First 3 rows of probabilities:\n{self.dataset.original_prob.head(3)}'
                )
    ...
    def get_train_test_probabilities(self, test_indices=None):
        ...
        if hasattr(self.dataset, 'train_data') and hasattr(
        ...
        # Otherwise split using provided test_indices
            ...
            if self.verbose:
                print(
                    f'Splitting probabilities using provided test indices (n={len(test_indices)})'
                )
            train_indices = [
                i for i in range(len(probs)) if i not in test_indices
            ]
            ...
            if self.verbose:
                print(
                    'No splitting information available, returning all probabilities as train'
                )
            train_probs = probs
            test_probs = None
        ...
        if self.verbose:
            print(
                f"Train probabilities shape: {train_probs.shape if train_probs is not None else 'None'}"
            )
            print(
                f"Test probabilities shape: {test_probs.shape if test_probs is not None else 'None'}"
            )
