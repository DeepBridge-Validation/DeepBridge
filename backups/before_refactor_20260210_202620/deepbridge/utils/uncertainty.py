...
def run_uncertainty_tests(
    ...
    try:
        from deepbridge.validation.wrappers.enhanced_uncertainty_suite import (
            run_enhanced_uncertainty_tests,
        )

        # Run the enhanced version with more detailed metrics and visualizations
        if verbose:
            print(
                'Using enhanced uncertainty analysis with additional metrics and visualizations'
            )
        results = run_enhanced_uncertainty_tests(
    ...
    if verbose:
        ...
        print(
            f"Average coverage error: {results.get('avg_coverage_error', 0):.3f}"
        )
        print(
            f"Average normalized width: {results.get('avg_normalized_width', 0):.3f}"
        )
        ...
        if 'threshold_value' in results:
            print(
                f"Reliability threshold: {results.get('threshold_value', 0):.3f}"
            )
        if 'reliable_count' in results.get('reliability_analysis', {}):
            ...
            print(
                f'Reliable predictions: {reliable}/{total} ({100*reliable/total:.1f}%)'
            )
