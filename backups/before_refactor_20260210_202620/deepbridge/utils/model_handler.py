...
class ModelHandler:
    ...
    def generate_predictions(
        self, data: t.Dict[str, pd.DataFrame], features: t.List[str]
    ) -> None:
        """
        ...
        for dataset_name, dataset in data.items():
            ...
            try:
                ...
                preds_df = pd.DataFrame(
                    proba, columns=cols, index=dataset.index
                )
                ...
                raise ValueError(
                    f'Failed to generate predictions for {dataset_name} data: {str(e)}'
                )
    ...
    def set_predictions(
        ...
        if prob_cols is None and self._prob_cols is None:
            raise ValueError(
                'prob_cols must be provided when setting predictions for the first time'
            )
        ...
        if (
            not self._initialize_predictions
            and train_data is not None
            and test_data is not None
        ):
            # First time initializing with prob_cols
            if all(col in train_data.columns for col in prob_cols) and all(
                col in test_data.columns for col in prob_cols
            ):
    ...
    def _validate_predictions(
        ...
        if not all(col in pred_df.columns for col in prob_cols):
            raise ValueError(
                f'Probability columns {prob_cols} not found in {name}_predictions'
            )
        if len(pred_df) != len(data_df):
            raise ValueError(
                f'Length of {name}_predictions ({len(pred_df)}) must match length of {name} data ({len(data_df)})'
            )
