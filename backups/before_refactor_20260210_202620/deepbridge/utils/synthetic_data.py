...
class SyntheticDataGenerator:
    ...
    def _identify_column_types(self, data: pd.DataFrame) -> None:
        ...
        for column in self.columns:
            if data[column].dtype in ['object', 'category']:
                ...
                self.inverse_category_mappings[column] = {
                    idx: val for idx, val in enumerate(unique_values)
                }
            else:
    ...
    def _transform_categorical(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self.categorical_columns:
            ...
            transformed[col] = data[col].map(
                lambda x: self.category_mappings[col].get(x, -1)
            )
    ...
    def _inverse_transform_categorical(
        ...
        for col in self.categorical_columns:
            ...
            indices = (
                data[col]
                .round()
                .clip(0, len(self.inverse_category_mappings[col]) - 1)
                .astype(int)
            )
    ...
    def generate(
        self, num_samples: int, random_state: Optional[int] = None
    ) -> pd.DataFrame:
        """
        ...
        if not self._is_fitted:
            raise ValueError(
                'The generator must be fitted before generating data'
            )
        ...
        if self.preserve_correlations and len(columns_for_generation) > 1:
            ...
            feature_corr = self.correlations.loc[
                columns_for_generation, columns_for_generation
            ]
            ...
            try:
                ...
                np.fill_diagonal(
                    feature_corr_adjusted.values, np.diag(feature_corr) + 1e-5
                )
            ...
            synthetic_data = pd.DataFrame(
                np.random.normal(
                    size=(num_samples, len(columns_for_generation))
                ),
                columns=columns_for_generation,
            )
        ...
        for col in self.categorical_columns:
            if (
                col in columns_for_generation
            ):  # Skip target if it's categorical
                n_categories = len(self.category_mappings[col])
                synthetic_data[col] = np.random.randint(
                    0, n_categories, size=num_samples
                )
                ...
                try:
                    if hasattr(self.model, 'predict_proba'):
                        ...
                        if self.target_column in self.numerical_columns:
                            noise_std = (
                                self.stats[self.target_column]['std'] * 0.05
                            )  # 5% noise
                            synthetic_data[
                                self.target_column
                            ] += np.random.normal(
                            ...
                            synthetic_data[
                                self.target_column
                            ] = synthetic_data[self.target_column].clip(
                    ...
                    if self.target_column in self.numerical_columns:
                        ...
                        n_categories = len(
                            self.category_mappings[self.target_column]
                        )
                        synthetic_data[self.target_column] = np.random.randint(
                            0, n_categories, size=num_samples
                        )
            else:
    ...
    def evaluate_quality(
        ...
        for col in self.numerical_columns:
            if col in synthetic_data.columns and col in real_data.columns:
                ...
                metrics[col] = {
                    ...
                    'mean_diff_pct': abs(real_mean - synth_mean)
                    / (abs(real_mean) + 1e-10)
                    * 100,
                    'std_real': real_std,
        ...
        for col in self.categorical_columns:
            if col in synthetic_data.columns and col in real_data.columns:
                real_dist = (
                    real_data[col].value_counts(normalize=True).sort_index()
                )
                synth_dist = (
                    synthetic_data[col]
                ...
                metrics[col] = {
                    'distribution_difference': np.mean(
                        abs(combined['real'] - combined['synthetic'])
                    ),
