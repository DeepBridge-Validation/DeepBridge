...
def run_robustness_tests(
            ...
            if perturb_type in results and perturb_type in subset_results:
                for level_key in results[perturb_type]['by_level']:
                    if level_key in subset_results[perturb_type]['by_level']:
                        ...
                        if (
                            'feature_subset'
                            not in results[perturb_type]['by_level'][
                                level_key
                            ]['runs']
                        ):
                            results[perturb_type]['by_level'][level_key][
                                'runs'
                            ]['feature_subset'] = []

                        # Copy the runs from subset_results to the main results
                        results[perturb_type]['by_level'][level_key]['runs'][
                            'feature_subset'
                        ] = subset_results[perturb_type]['by_level'][
                            level_key
                        ][
                            'runs'
                        ][
                        ...
                        results[perturb_type]['by_level'][level_key][
                            'overall_result'
                        ]['feature_subset'] = subset_results[perturb_type][
                            'by_level'
                        ][
                            level_key
                        ][
    ...
    metrics = {'base_score': results.get('base_score', 0)}
    ...
    if verbose:
        ...
        print(
            f"{model_label}Overall robustness score: {1.0 - results.get('avg_overall_impact', 0):.3f}"
        )
        print(
            f"{model_label}Average impact from Gaussian noise: {results.get('avg_raw_impact', 0):.3f}"
        )
        print(
            f"{model_label}Average impact from Quantile perturbation: {results.get('avg_quantile_impact', 0):.3f}"
        )
        if feature_subset:
            print(
                f"{model_label}Features perturbed: {', '.join(feature_subset)}"
            )
