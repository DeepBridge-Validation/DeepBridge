...
class TestRunner:
    ...
    def run_initial_tests(self, **kwargs) -> dict:
        ...
        try:
            if hasattr(self.dataset.model, 'feature_importances_') and hasattr(
                ...
                if len(importances) == len(features):
                    ...
                    if total > 0:
                        results['models']['primary_model'][
                            'feature_importance'
                        ] = {
                            features[i]: float(importances[i]) / total
                            for i in range(len(features))
                        }
            elif hasattr(self.dataset.model, 'coef_') and hasattr(
                self.dataset, 'features'
            ):
                features = self.dataset.features
                if (
                    hasattr(self.dataset.model.coef_, 'shape')
                    and len(self.dataset.model.coef_.shape) > 1
                ):
            ...
            self.logger.debug(
                f'Could not extract feature importances: {str(e)}'
            )
        ...
        if self.alternative_models:
            for model_name, model in self.alternative_models.items():
                self.logger.debug(
                    f'Calculating metrics for alternative model: {model_name}'
                )
        ...
        if 'hyperparameters' in self.tests:
            test_configs['hyperparameters'] = self._get_test_config(
                'hyperparameters'
            )
        ...
        if 'models' in results and 'primary_model' in results['models']:
            model_type = results['models']['primary_model'].get(
                'type', 'Unknown'
            )
    ...
    def _get_experiment_config(self) -> dict:
        ...
        config = {
            ...
            'dataset_info': {
                ...
                if len(self.X_train) + len(self.X_test) > 0
                else 0,
            },
        }
    ...
    def _calculate_model_metrics(self, model, model_name: str) -> dict:
        ...
        try:
            ...
            metrics = self._calculate_metrics_for_model(
    ...
    def _get_test_config(self, test_type: str) -> dict:
        ...
        if not is_valid_test_type(test_type):
            self.logger.warning(
                f'Invalid test type: {test_type}. Using fallback configurations.'
            )
    ...
    def _run_model_test(
        ...
        """
        # Initialize results dictionary
        test_results = {'primary_model': {}, 'alternative_models': {}}

        # Test primary model
        self.logger.info(f'Testing {test_type} of primary model...')

        # Get appropriate arguments for this test type for primary model
        test_args = self._get_test_arguments(
            test_type, config_name, 'primary_model', metric_param
        )
        ...
        if (
            'models' in self.test_results
            and 'primary_model' in self.test_results['models']
        ):
            primary_metrics = self.test_results['models']['primary_model'].get(
                'metrics', {}
            )
        ...
        if self.alternative_models:
            for model_name, model in self.alternative_models.items():
                self.logger.info(
                    f'Testing {test_type} of alternative model: {model_name}'
                )
                ...
                test_args = self._get_test_arguments(
                    test_type, config_name, model_name, metric_param
                )
                ...
                if (
                    'models' in self.test_results
                    and model_name in self.test_results['models']
                ):
                    model_metrics = self.test_results['models'][
                        model_name
                    ].get('metrics', {})
                ...
                test_results['alternative_models'][model_name][
                    'model_type'
                ] = type(model).__name__
    ...
    def run_tests(self, config_name: str = 'quick', **kwargs) -> dict:
        ...
        **kwargs : dict
        ...
        if (
            not hasattr(self, 'test_results')
            or not self.test_results
            or not 'models' in self.test_results
        ):
        ...
        if 'fairness' in self.tests:
            from deepbridge.validation.wrappers.fairness_suite import (
                FairnessSuite,
            )
            ...
            protected_attributes = kwargs.get(
                'sensitive_features', kwargs.get('protected_attributes', [])
            )

            if not protected_attributes:
                self.logger.warning(
                    'No protected attributes provided for fairness test. Skipping.'
                )
            else:
                try:
    ...
    def run_test(self, test_type: str, config_name: str = 'quick', **kwargs):
        ...
        **kwargs : dict
        ...
        if test_type not in valid_tests:
            raise ValueError(
                f"Invalid test type '{test_type}'. Valid types: {valid_tests}"
            )
    ...
    def _create_alternative_dataset(self, model):
        ...
        return DBDatasetFactory.create_for_alternative_model(
            original_dataset=self.dataset, model=model
        )
