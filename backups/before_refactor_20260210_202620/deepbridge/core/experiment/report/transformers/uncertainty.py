    ...
    def transform(
        ...
        if 'uncertainty_score' not in report_data:
            ...
            if 'crqr' in report_data and 'by_alpha' in report_data['crqr']:
                ...
                coverage_ratios = []
                for alpha_key, alpha_data in report_data['crqr'][
                    'by_alpha'
                ].items():
                    if 'overall_result' in alpha_data:
                        actual = alpha_data['overall_result'].get(
                            'coverage', 0
                        )
                        expected = alpha_data['overall_result'].get(
                            'expected_coverage', 0
                        )
                        if expected > 0:
                            # Penalize over-coverage less than under-coverage
                            ratio = (
                                min(actual / expected, 1.1)
                                if actual > expected
                ...
                if coverage_ratios:
                    report_data['uncertainty_score'] = sum(
                        coverage_ratios
                    ) / len(coverage_ratios)
                else:
        ...
        if (
            'avg_coverage' not in report_data
            and 'crqr' in report_data
            and 'by_alpha' in report_data['crqr']
        ):
            coverages = []
            widths = []
            ...
            for alpha_key, alpha_data in report_data['crqr'][
                ...
                if 'overall_result' in alpha_data:
                    coverages.append(
                        alpha_data['overall_result'].get('coverage', 0)
                    )
                    widths.append(
                        alpha_data['overall_result'].get('mean_width', 0)
                    )
        ...
        if (
            'alpha_levels' not in report_data
            and 'crqr' in report_data
            and 'by_alpha' in report_data['crqr']
        ):
            report_data['alpha_levels'] = list(
                map(float, report_data['crqr']['by_alpha'].keys())
            )
        ...
        if (
            'alternative_models' not in report_data
            and 'results' in report_data
        ):
            if 'uncertainty' in report_data['results']:
                uncertainty_results = report_data['results']['uncertainty']
                if (
                    'results' in uncertainty_results
                    and 'alternative_models' in uncertainty_results['results']
                ):
                    logger.info('Found alternative_models in nested structure')
                    report_data['alternative_models'] = uncertainty_results[
                        'results'
                    ]['alternative_models']
