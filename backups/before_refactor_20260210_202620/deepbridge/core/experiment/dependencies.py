...
def check_version_compatibility(
    ...
    is_compatible = version.parse(installed_version) >= version.parse(
        required_version
    )
...
def check_component_dependencies(component: str) -> Dict[str, Any]:
    ...
    for package_name, import_name in DEPENDENCIES[component][
        'required'
    ].items():
        if not is_package_installed(import_name):
            ...
            (
                ...
                installed_version,
                required_version,
            ) = check_version_compatibility(package_name)
...
def get_install_command(
    missing_packages: List[str], upgrade: bool = False
) -> str:
    """
...
def require_package(
    ...
    if min_version:
        ...
        installed_version = get_package_version(package_name)
        if installed_version and version.parse(
            installed_version
...
def optional_import(
    package_name: str, import_name: Optional[str] = None, warning: bool = True
) -> Optional[Any]:
    """
...
def _create_jinja2_fallback():
    ...
    logger.warning(
        '⚠️ Using simplified Jinja2 fallback. Install jinja2 for proper template rendering.'
    )
...
def _create_plotly_fallback():
    ...
    logger.warning(
        '⚠️ Using simplified Plotly fallback. Install plotly for interactive visualizations.'
    )
...
def print_dependency_status():
    ...
    if version_issues:
        prefix = (
            ...
            if (not all_installed or missing_optional)
            else 'There are'
        )
