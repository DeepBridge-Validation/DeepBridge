...
class RobustnessEvaluator:
    ...
    def __init__(
        ...
        if self.verbose:
            ...
            print(
                f'Performing {self.n_iterations} iterations per perturbation level'
            )
        ...
        if self._problem_type == 'classification':
            ...
            if hasattr(model, 'predict_proba'):
                ...
                if self.metric.upper() in ['AUC', 'ROC_AUC']:
                    ...
                    if (
                        len(y_pred_proba.shape) > 1
                        and y_pred_proba.shape[1] > 2
                    ):
    ...
    def _get_metric_score(
        self, y_true: pd.Series, y_pred: Union[pd.Series, np.ndarray]
    ) -> float:
        """
        ...
        if self._problem_type == 'classification':
            ...
            if self.metric.upper() in metric_map:
                ...
                if self.metric.upper() == 'RMSE':
                    ...
                    metric_func = getattr(
                        metrics, metric_map[self.metric.upper()]
                    )
                    return metric_func(y_true, y_pred)
            else:
    ...
    def evaluate_perturbation(
        ...
        for iteration in range(self.n_iterations):
            ...
            if iteration > 0:
                ...
                if hasattr(self.data_perturber.rng, 'seed'):
                    ...
                    self.data_perturber.set_random_state(
                        original_seed + seed_offset
                    )
            ...
            X_perturbed = self.data_perturber.perturb_data(
                X_full, perturb_method, level, perturb_features
            )

            # Calculate score on perturbed data
            perturbed_score = self.calculate_score_on_perturbed_data(
                X_perturbed, y
            )
        ...
        higher_is_better = not (
            ...
            and self.metric.upper() in ['MSE', 'MAE', 'RMSE']
        )
        ...
        if not higher_is_better:
            ...
            mean_impact = (mean_perturbed_score - base_score) / max(
                base_score, 1e-10
            )
        else:
            # For classification metrics, higher values mean better performance
            mean_impact = (base_score - mean_perturbed_score) / max(
                base_score, 1e-10
            )
    ...
    def calculate_score_on_perturbed_data(
        self, X_perturbed: pd.DataFrame, y: pd.Series
    ) -> float:
        """
        Calculate score on perturbed data.
    ...
    def evaluate_feature_importance(
        ...
        if self.verbose:
            print(
                f'Evaluating feature importance with {perturb_method} perturbation at level {level}'
            )
            print(
                f'Using {self.n_iterations} iterations per feature for statistical robustness'
            )
            if feature_subset:
        ...
        for i, feature in enumerate(features_to_test):
            ...
            for iteration in range(self.n_iterations):
                ...
                if iteration > 0:
                    seed_offset = iteration * 1000
                    if hasattr(self.data_perturber.rng, 'seed'):
                        original_seed = self.data_perturber.rng.get_state()[1][
                            0
                        ]
                        self.data_perturber.set_random_state(
                            original_seed + seed_offset
                ...
                X_perturbed = self.data_perturber.perturb_data(
                    X_full.copy(), perturb_method, level, [feature]
                )
                ...
                if not higher_is_better:
                    # For these metrics, higher values mean worse performance
                    impact = (perturbed_score - base_score) / max(
                        base_score, 1e-10
                    )
                else:
                    # For classification metrics, higher values mean better performance
                    impact = (base_score - perturbed_score) / max(
                        base_score, 1e-10
                    )
