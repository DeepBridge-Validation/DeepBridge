...
class TopFeaturesDistributionChart:
    ...
    @staticmethod
    def generate(
        data: Dict[str, Any], top_n: int = 3, **kwargs
    ) -> Optional[str]:
        """
        ...
        **kwargs : dict
        ...
        try:
            ...
            ) = TopFeaturesDistributionChart._extract_distribution_data(data)
            ...
            sorted_features = sorted(
                psi_values.items(), key=lambda x: x[1], reverse=True
            )
            ...
            fig, axes = plt.subplots(
                1, n_features, figsize=(fig_width, fig_height)
            )
            ...
            for idx, (feature_name, psi_value) in enumerate(top_features):
                ...
                (
                    reliable_values,
                    unreliable_values,
                ) = TopFeaturesDistributionChart._get_feature_distributions(
                    feature_name, feature_distributions
                )

                if reliable_values is None or unreliable_values is None:
                    logger.warning(
                        f'No distribution data for feature {feature_name}'
                    )
                    continue
                ...
                if show_kde:
                    try:
                        ...
                        if len(unreliable_values) > 1:
                            kde_unreliable = stats.gaussian_kde(
                                unreliable_values
                            )
                            ax.plot(
                                x_range,
                                kde_unreliable(x_range),
                                color='#c0392b',
                                linewidth=2.5,
                ...
                # Statistics box
                stats_text = (
                    TopFeaturesDistributionChart._generate_feature_stats(
                        reliable_values, unreliable_values
                    )
                )
                ...
                ax.axvline(
                    ...
                    alpha=0.7,
                    linewidth=1.5,
                    label=f'Mean Unrel.: {mean_unrel:.2f}',
                )
                ...
                separation_score = (
                    TopFeaturesDistributionChart._calculate_separation_score(
                        reliable_values, unreliable_values
                    )
                )
                ax.text(
            ...
            interpretation = (
                TopFeaturesDistributionChart._generate_overall_interpretation(
            ...
            logger.error(
                f'Error generating top features distribution: {str(e)}'
            )
            import traceback
                ...
                for result in results:
                    if 'reliability_analysis' in result:
                        ...
                        if 'feature_distributions' in reliability:
                            feature_distributions = reliability[
                                'feature_distributions'
                            ]
                        break
    ...
    def _get_feature_distributions(
        feature_name: str, feature_distributions: Dict
        ...
        if (
            'reliable' in feature_distributions
            and 'unreliable' in feature_distributions
        ):
        ...
        if reliable_values is None or unreliable_values is None:
            logger.info(
                f'Generating synthetic distribution data for {feature_name}'
            )
            np.random.seed(42)
    ...
    @staticmethod
    def _generate_feature_stats(
        reliable_values: np.ndarray, unreliable_values: np.ndarray
    ) -> str:
        """
    ...
    def _calculate_separation_score(
        ...
        pooled_std = np.sqrt(
            (np.var(reliable_values) + np.var(unreliable_values)) / 2
        )
    ...
    def _generate_overall_interpretation(
        top_features: List[Tuple[str, float]], feature_distributions: Dict
    ) -> str:
        """
        ...
        interpretation = (
            f'Analysis of {n_features} features with highest shift: '
        )

        if n_significant > 0:
            interpretation += (
                f'{n_significant} show significant change (PSI ≥ 0.25). '
            )
        else:
