...
class FeatureImportanceChart(BaseChartGenerator):
    ...
    def generate(
        ...
        title: str = 'Feature Importance for Uncertainty',
    ) -> str:
        """
        ...
        if self.chart_generator and hasattr(
            self.chart_generator, 'heatmap_chart'
        ):
            try:
                ...
                logger.error(
                    f'Error using chart generator for feature importance: {str(e)}'
                )

        # Fallback - implement direct charting
        try:
            # Process data based on detected structure
            processed_data = self._process_feature_importance_data(
                feature_importance_data, data_type
            )

            # Check if processed_data is valid
            if not processed_data:
                logger.warning(
                    'Failed to process feature importance data - returned None'
                )
                return ''

            # Check individual components
            has_matrix = (
                'matrix' in processed_data
                and processed_data['matrix'] is not None
            )
            has_x_labels = (
                'x_labels' in processed_data and processed_data['x_labels']
            )
            has_y_labels = (
                'y_labels' in processed_data and processed_data['y_labels']
            )
            ...
            # Set tick labels
            ax.set_xticklabels(
                processed_data['x_labels'], rotation=45, ha='right'
            )
    ...
    def _detect_data_structure(self, data):
        ...
        try:
            if all(
                ...
                or (hasattr(v, 'item') and callable(v.item))
        ...
        if all(isinstance(v, dict) for v in data.values()):
            ...
            if 'feature_importance' in sample_model and isinstance(
                sample_model['feature_importance'], dict
            ):
    ...
    def _process_feature_importance_data(self, data, data_type):
        ...
        if data_type == 'single_model_direct':
            ...
            sorted_features = sorted(
                converted_data.items(), key=lambda x: x[1], reverse=True
            )
            ...
            models_fi = {}
            for model_name, model_data in data.items():
                if 'feature_importance' in model_data and isinstance(
                    model_data['feature_importance'], dict
                ):
            ...
            for feature in all_features:
                values = [
                    model_fi.get(feature, 0) for model_fi in models_fi.values()
                ]
            ...
            for model_name, model_data in data.items():
                for ft in model_data.keys():
                    if isinstance(model_data[ft], dict) and all(
                        isinstance(v, (int, float))
                        for v in model_data[ft].values()
                    ):
                        feature_type = ft
                        break
                if feature_type:
            ...
            models_fi = {}
            for model_name, model_data in data.items():
                if feature_type in model_data and isinstance(
                    model_data[feature_type], dict
                ):
