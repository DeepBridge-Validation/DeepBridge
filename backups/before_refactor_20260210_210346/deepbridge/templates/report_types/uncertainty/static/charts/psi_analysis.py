...
class PSIAnalysisChart:
    ...
    def generate(data: Dict[str, Any], **kwargs) -> Optional[str]:
        ...
        try:
            ...
            if not psi_values:
                logger.warning(
                    f'No PSI values found in data. Data keys: {list(data.keys())}'
                )
                if 'reliability_analysis' in data:
                    logger.warning(
                        f"reliability_analysis keys: {list(data.get('reliability_analysis', {}).keys())}"
                    )
                return None
            ...
            ax.set_xticklabels(
                psi_df['Feature'].values, rotation=45, ha='right', fontsize=10
            )
            ...
            ax.set_title(
                'Distribution Shift Analysis: Unreliable vs Reliable Data',
                fontsize=14,
                fontweight='bold',
                pad=20,
            )
            ...
            # Add threshold lines
            ax.axhline(
                y=0.1, color='green', linestyle='--', alpha=0.5, linewidth=1.5
            )
            ax.axhline(
                y=0.25,
                color='orange',
    ...
    def _extract_psi_values(data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if not psi_values and 'reliability_analysis' in data:
            ...
            if 'psi_values' in reliability:
                logger.info(
                    f"Found psi_values in reliability_analysis with {len(reliability['psi_values'])} items"
                )
                psi_values = reliability['psi_values']
