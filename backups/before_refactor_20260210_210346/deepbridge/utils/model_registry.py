...
class StatsModelsGAM:
    ...
    def _create_bsplines(self, X):
        ...
        degree = [self.spline_order] * X.shape[
            1
        ]  # Same order for each feature
    ...
    def get_param_space(
        ...
        if mode == ModelMode.REGRESSION:
            ...
            if (
                ...
                and 'objective' in config.default_params
            ):
        ...
        if (
            ...
            and 'validation_fraction' in config.default_params
        ):
            param_space['validation_fraction'] = config.default_params[
                'validation_fraction'
            ]
...
class ModelFactory:
    ...
    def create_model(self, model_type, task_type='classification', **kwargs):
        ...
        return ModelRegistry.get_model(
            model_type, custom_params=kwargs, mode=mode
        )

    def get_model(
        self, model_type, custom_params=None, mode=ModelMode.CLASSIFICATION
    ):
        """
        ...
        """
        return ModelRegistry.get_model(
            model_type, custom_params=custom_params, mode=mode
        )
