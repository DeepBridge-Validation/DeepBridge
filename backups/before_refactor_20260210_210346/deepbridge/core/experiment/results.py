...
class BaseTestResult(TestResult):
    ...
    def __init__(
        self, name: str, results: dict, metadata: t.Optional[dict] = None
    ):
        """
    ...
    def clean_results_dict(self) -> dict:
        ...
        if 'initial_results' in self._results:
            cleaned['initial_results'] = copy.deepcopy(
                self._results['initial_results']
            )
...
class RobustnessResult(BaseTestResult):
    ...
    def clean_results_dict(self) -> dict:
        ...
        if 'alternative_models' in cleaned:
            for model_name, model_data in cleaned[
                'alternative_models'
            ].items():
    ...
    def _clean_model_data(self, model_data: dict) -> None:
        ...
        if 'metrics' in model_data and 'base_score' in model_data['metrics']:
            ...
            if model_data.get('base_score') == model_data['metrics'].get(
                'base_score'
            ):
...
class ExperimentResult:
    ...
    def save_html(
        ...
        if not result:
            raise TestResultNotFoundError(
                f'No {test_type} test results found. Run the test first.'
            )
        ...
        if test_type == 'robustness':
            ...
            if 'primary_model' in test_result:
                ...
                if 'weakspot_analysis' in test_result:
                    report_data['weakspot_analysis'] = test_result[
                        'weakspot_analysis'
                    ]
                if 'overfitting_analysis' in test_result:
                    report_data['overfitting_analysis'] = test_result[
                        'overfitting_analysis'
                    ]
            elif (
                'results' in test_result
                and 'primary_model' in test_result['results']
            ):
                ...
                report_data = {
                    'primary_model': {
                        ...
                        'avg_quantile_impact': test_result.get(
                            'avg_quantile_impact', 0
                        ),
                        'avg_overall_impact': test_result.get(
                            'avg_overall_impact', 0
                        ),
                        'robustness_score': 1.0
                        - test_result.get('avg_overall_impact', 0),
                        'feature_importance': test_result.get(
                            'feature_importance', {}
                        ),
                        'model_feature_importance': test_result.get(
                ...
                if 'feature_subset' in test_result:
                    report_data['feature_subset'] = test_result[
                        'feature_subset'
                    ]
        ...
        try:
            ...
            raise ReportGenerationError(
                f'HTML report generation for {test_type} tests is not implemented: {str(e)}'
            )
    ...
    def save_json(
        ...
        json_data = {
            'experiment_info': {
                ...
                'generation_time': self.generation_time.strftime(
                    '%Y-%m-%d %H:%M:%S'
                ),
                'config': self.config,
            },
        ...
        if 'initial_results' in self.results:
            if compact:
                ...
                initial_compact = self._compact_initial_results(
                    self.results['initial_results']
                )
                json_data['initial_model_evaluation'] = initial_compact
            else:
                json_data['initial_model_evaluation'] = self.results[
                    'initial_results'
                ]
    ...
    def _generate_robustness_summary(self, test_data: dict) -> dict:
        ...
        if 'primary_model' in test_data:
            ...
            robustness_score = primary.get(
                'robustness_score', 1.0 - primary.get('avg_overall_impact', 0)
            )
            summary['model_performance']['robustness_score'] = round(
                robustness_score, 4
            )
            ...
            summary['model_performance']['average_impacts'] = {
                ...
                'quantile_perturbation': round(
                    primary.get('avg_quantile_impact', 0), 4
                ),
            ...
            if robustness_score >= 0.9:
                summary['key_findings'].append(
                    'Model shows excellent robustness (score >= 0.9)'
                )
            elif robustness_score >= 0.8:
                summary['key_findings'].append(
                    'Model shows good robustness (score >= 0.8)'
                )
            elif robustness_score >= 0.7:
                summary['key_findings'].append(
                    'Model shows moderate robustness (score >= 0.7)'
                )
            else:
                summary['key_findings'].append(
                    f'Model shows low robustness (score: {robustness_score:.4f})'
                )

            # Add findings about feature sensitivity
            if 'feature_importance' in primary and len(features) > 0:
                high_impact_features = [
                    k for k, v in features.items() if v > 0.1
                ]
                if high_impact_features:
                    summary['key_findings'].append(
                        f'Found {len(high_impact_features)} highly sensitive features (impact > 0.1)'
                    )

            # Recommendations based on results
            if robustness_score < 0.8:
                summary['recommendations'].append(
                    'Consider model regularization to improve robustness'
                )

            if (
                len(
                    summary['feature_impacts'].get(
                        'most_sensitive_features', []
                    )
                )
                > 0
            ):
                top_feature = summary['feature_impacts'][
                    'most_sensitive_features'
                ][0]
                if top_feature['impact'] > 0.2:
                    summary['recommendations'].append(
                        f"Feature '{top_feature['feature']}' is highly sensitive - consider feature engineering or validation"
                    )

        # Analyze alternative models if available
        if 'alternative_models' in test_data:
            alt_models = {}
            for model_name, model_data in test_data[
                'alternative_models'
            ].items():
                if 'metrics' in model_data:
                    alt_models[model_name] = {
                        ...
                        'robustness_score': round(
                            1.0 - model_data.get('avg_overall_impact', 0), 4
                        ),
                    }
            ...
            if alt_models:
                ...
                if (
                    best_alt[1]['robustness_score']
                    > summary['model_performance']['robustness_score']
                ):
                    summary['key_findings'].append(
                        f"Alternative model '{best_alt[0]}' shows better robustness"
                    )
            ...
            if by_alpha:
                ...
                for alpha_key, alpha_data in by_alpha.items():
                    ...
                    calibration_errors.append(
                        abs(target_coverage - actual_coverage)
                    )
                ...
                summary['model_performance']['average_coverage'] = round(
                    avg_coverage, 4
                )
                summary['model_performance'][
                    'average_calibration_error'
                ] = round(avg_calibration_error, 4)
                summary['model_performance']['average_interval_width'] = round(
                    avg_width, 4
                )
                summary['model_performance']['uncertainty_score'] = round(
                    primary.get('uncertainty_quality_score', 0), 4
                )
                ...
                if avg_calibration_error < 0.05:
                    ...
                    summary['calibration_quality'][
                        'description'
                    ] = f'Calibration error: {avg_calibration_error:.4f}'
                ...
                if avg_calibration_error >= 0.10:
                    summary['key_findings'].append(
                        'Calibration needs improvement (error ≥ 0.10)'
                    )

                if avg_width > 0.5:
                    summary['key_findings'].append(
                        f'High uncertainty detected (avg width: {avg_width:.4f})'
                    )

                if avg_coverage < 0.90:
                    summary['key_findings'].append(
                        'Coverage below 90% - model may be overconfident'
                    )
                ...
                if avg_width > 0.5:
                    summary['recommendations'].append(
                        'Collect more training data to reduce prediction variance'
                    )
                    summary['recommendations'].append(
                ...
                if avg_coverage < 0.90:
                    summary['recommendations'].append(
                        'Recalibrate or increase interval width'
                    )

                # Per-alpha analysis
                summary['per_alpha_analysis'] = []
                for alpha_key in sorted(
                    by_alpha.keys(), key=lambda x: float(x)
                ):
    ...
    def _compact_uncertainty_data(self, test_data: dict) -> dict:
        ...
        if 'primary_model' in test_data:
            ...
            if 'uncertainty_quality_score' in primary:
                compact_primary['uncertainty_quality_score'] = primary[
            ...
            if 'feature_importance' in primary:
                ...
                sorted_features = sorted(
                    feat_imp.items(), key=lambda x: abs(x[1]), reverse=True
                )[:10]
                compact_primary['feature_importance_top10'] = dict(
                    sorted_features
                )
    ...
    def _compact_resilience_data(self, test_data: dict) -> dict:
        ...
        if 'primary_model' in test_data:
            ...
            if 'resilience_score' in primary:
                compact_primary['resilience_score'] = primary[
                    'resilience_score'
                ]
            ...
            if 'drift_analysis' in primary:
                ...
                if 'data_drift_score' in drift:
                    compact_drift['data_drift_score'] = drift[
                        'data_drift_score'
                    ]
                if 'concept_drift_score' in drift:
                    compact_drift['concept_drift_score'] = drift[
                        'concept_drift_score'
                    ]
                ...
                if 'per_feature_drift' in drift:
                    ...
                    compact_drift['per_feature_drift_top10'] = dict(
                        sorted_features
                    )
            ...
            if 'distribution_shift' in primary:
                ...
                if 'by_alpha' in dist_shift:
                    compact_by_alpha = {}
                    for alpha_key, alpha_data in dist_shift[
                        'by_alpha'
                    ].items():
                        if 'results' in alpha_data and alpha_data['results']:
                            ...
                            compact_result = {
                                ...
                                'distance_metric': first_result.get(
                                    'distance_metric'
                                ),
                                'worst_metric': first_result.get(
                                    'worst_metric'
                                ),
                                'remaining_metric': first_result.get(
                                    'remaining_metric'
                                ),
                                'performance_gap': first_result.get(
                                    'performance_gap'
                                ),
                                'worst_sample_count': first_result.get(
                                    'worst_sample_count'
                                ),
                                'remaining_sample_count': first_result.get(
                            ...
                            if (
                                'feature_distances' in first_result
                                and 'top_features'
                                in first_result['feature_distances']
                            ):
                                top_features = first_result[
                                    'feature_distances'
                                ]['top_features']
    ...
    def to_dict(self) -> dict:
        ...
        metadata = {
            ...
            'tests_performed': list(
                k for k in self.results.keys() if k != 'initial_results'
            ),
        }
    ...
    def from_dict(cls, results_dict: dict) -> 'ExperimentResult':
        ...
        for key in required_keys:
            if key not in results_dict:
                raise ValueError(
                    f'Missing required key in results_dict: {key}'
                )

        experiment_type = results_dict.get(
        ...
        if 'initial_results' in results_dict:
            ...
            instance.results['initial_results'] = results_dict[
                'initial_results'
            ]
...
def create_test_result(
    ...
    result_class = test_classes.get(
        test_type,
        lambda name, results, metadata: BaseTestResult(
            name.capitalize(), results, metadata
        ),
    )
