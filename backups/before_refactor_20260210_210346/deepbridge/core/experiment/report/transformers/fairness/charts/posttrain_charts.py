...
class DisparateImpactGaugeChart(BaseChart):
    ...
    def create(self, data: Dict[str, Any]) -> str:
        ...
        for attr in protected_attrs:
            if attr in posttrain_metrics:
                ...
                if isinstance(di_metric, dict) and 'ratio' in di_metric:
                    ...
                    gauge_data.append(
                        {'attribute': attr, 'ratio': ratio, 'passes': passes}
                    )
        ...
        fig = make_subplots(
            ...
            cols=n_attrs,
            subplot_titles=[
                f"{format_attribute_name(d['attribute'])}" for d in gauge_data
            ],
        ...
        for i, item in enumerate(gauge_data, start=1):
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    delta={
                        'reference': 0.8,
                        'increasing': {'color': self.COLOR_SUCCESS},
...
class DisparityComparisonChart(BaseChart):
    ...
    def create(self, data: Dict[str, Any]) -> str:
        ...
        for attr in protected_attrs:
            if attr in posttrain_metrics:
                sp_metric = posttrain_metrics[attr].get(
                    'statistical_parity', {}
                )
        ...
        colors = [
            self._get_color_for_status(status) for status in df['status']
        ]
    ...
    def create(self, data: Dict[str, Any]) -> str:
        ...
        for attr in protected_attrs:
            ...
            for metric in POSTTRAIN_MAIN_METRICS:
                if (
                    attr in posttrain_metrics
                    and metric in posttrain_metrics[attr]
                ):
        ...
        fig = go.Figure(
            data=go.Heatmap(
                z=matrix,
                x=[
                    METRIC_SHORT_LABELS.get(m, m)
                    for m in POSTTRAIN_MAIN_METRICS
                ],
                y=attr_labels,
                text=text_matrix,
        ...
        self._apply_common_layout(
            ...
            xaxis=dict(autorange=True, tickfont={'size': 11}, side='top'),
            yaxis=dict(autorange=True, tickfont={'size': 12}),
        )
