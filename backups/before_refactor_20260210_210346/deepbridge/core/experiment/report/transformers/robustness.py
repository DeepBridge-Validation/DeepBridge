    ...
    def transform(
        ...
        if 'primary_model' in report_data:
            ...
            if 'feature_importance' in primary_data:
                logger.info(
                    f"Found feature_importance in primary_model with {len(primary_data['feature_importance'])} features"
                )
                report_data['feature_importance'] = primary_data[
                    'feature_importance'
                ]

            if 'model_feature_importance' in primary_data:
                logger.info(
                    f"Found model_feature_importance in primary_model with {len(primary_data['model_feature_importance'])} features"
                )
                report_data['model_feature_importance'] = primary_data[
        ...
        # Use model_type from primary_model if available
            ...
            'primary_model' in report_data
            and 'model_type' in report_data['primary_model']
        ):
            report_data['model_type'] = report_data['primary_model'][
                'model_type'
            ]
        # Use type from initial_results if available
            ...
            and 'primary_model' in report_data['initial_results']['models']
            and 'type'
            in report_data['initial_results']['models']['primary_model']
        ):
            report_data['model_type'] = report_data['initial_results'][
                'models'
            ]['primary_model']['type']
        else:
        ...
        if 'results' in report_data:
            logger.info(
                'Checking for feature importance in nested results structure'
            )
            if 'robustness' in report_data['results']:
                rob_results = report_data['results']['robustness']
                logger.info(
                    f'Found robustness key with keys: {list(rob_results.keys())}'
                )

                # Check in direct robustness object
                if (
                    'feature_importance' in rob_results
                    and rob_results['feature_importance']
                ):
                    logger.info(
                        f"Found feature_importance directly in results.robustness with {len(rob_results['feature_importance'])} features"
                    )
                    report_data['feature_importance'] = rob_results[
                        'feature_importance'
                    ]

                if (
                    'model_feature_importance' in rob_results
                    and rob_results['model_feature_importance']
                ):
                    logger.info(
                        f"Found model_feature_importance directly in results.robustness with {len(rob_results['model_feature_importance'])} features"
                    )
                    report_data['model_feature_importance'] = rob_results[
                        'model_feature_importance'
                    ]
                ...
                if 'results' in rob_results:
                    ...
                    logger.info(
                        f'Found nested results with keys: {list(nested_results.keys())}'
                    )

                    # Check for alternative models in nested structure
                    if (
                        'alternative_models' in nested_results
                        and 'alternative_models' not in report_data
                    ):
                        logger.info(
                            'Found alternative_models in nested structure'
                        )
                        report_data['alternative_models'] = nested_results[
                            'alternative_models'
                        ]
                    ...
                    if 'primary_model' in nested_results:
                        ...
                        logger.info(
                            'Found primary_model in nested results.robustness.results'
                        )

                        if (
                            'feature_importance' in primary_model
                            and primary_model['feature_importance']
                        ):
                            logger.info(
                                f"Found feature_importance in nested results with {len(primary_model['feature_importance'])} features"
                            )
                            report_data['feature_importance'] = primary_model[
                                'feature_importance'
                            ]

                        if (
                            'model_feature_importance' in primary_model
                            and primary_model['model_feature_importance']
                        ):
                            logger.info(
                                f"Found model_feature_importance in nested results with {len(primary_model['model_feature_importance'])} features"
                            )
                            report_data[
                                'model_feature_importance'
                            ] = primary_model['model_feature_importance']
        ...
        if 'robustness_score' not in report_data:
            if 'avg_overall_impact' in report_data:
                report_data['robustness_score'] = float(
                    1.0 - report_data['avg_overall_impact']
                )
                logger.info(
                    'Calculated robustness_score from avg_overall_impact'
                )
            else:
                logger.warning(
                    'robustness_score not found in results and cannot be calculated'
                )
        ...
        if 'avg_quantile_impact' not in report_data:
            if (
                'quantile' in report_data
                and 'overall' in report_data['quantile']
            ):
                report_data['avg_quantile_impact'] = (
                    report_data['quantile']
                    .get('overall', {})
                    .get('avg_impact', 0.0)
                )
            else:
        ...
        if 'raw' in report_data and 'by_level' in report_data['raw']:
            levels = sorted(
                [
                    float(level)
                    for level in report_data['raw']['by_level'].keys()
                ]
            )
            scores = []
            worst_scores = []
            ...
            for level in levels:
                ...
                if 'overall_result' in level_data:
                    ...
                    if 'all_features' in overall_result:
                        ...
                        score = all_features_result.get(
                            'mean_score'
                        )  # CORREÇÃO: Usar mean_score ao invés de perturbed_score
                        worst_score = all_features_result.get('worst_score')
                    else:
                        score = overall_result.get(
                            'mean_score'
                        )  # CORREÇÃO: Usar mean_score ao invés de perturbed_score
                ...
                worst_scores.append(
                    worst_score if worst_score is not None else 0.0
                )
            ...
            if 'alternative_models' in report_data:
                alt_models_data = {}
                for alt_name, alt_data in report_data[
                    'alternative_models'
                ].items():
                    if 'raw' in alt_data and 'by_level' in alt_data['raw']:
                        ...
                        for level in levels:
                            level_str = str(level)
                            level_data = alt_data['raw']['by_level'].get(
                                level_str, {}
                            )
                            ...
                            if 'overall_result' in level_data:
                                overall_result = level_data['overall_result']
                                if 'all_features' in overall_result:
                                    all_features_result = overall_result[
                                        'all_features'
                                    ]
                                    alt_score = all_features_result.get(
                                        'mean_score'
                                    )  # CORREÇÃO: Usar mean_score ao invés de perturbed_score
                                    alt_worst = all_features_result.get(
                                        'worst_score'
                                    )
                                else:
                                    alt_score = overall_result.get(
                                        'mean_score'
                                    )  # CORREÇÃO: Usar mean_score ao invés de perturbed_score
                                    alt_worst = overall_result.get(
                                        'worst_score'
                                    )
                            ...
                            if alt_score is None and 'runs' in level_data:
                                if 'all_features' in level_data['runs']:
                                    all_features_runs = level_data['runs'][
                                        'all_features'
                                    ]
                                    if (
                                        all_features_runs
                                        and len(all_features_runs) > 0
                                    ):
                                        first_run = all_features_runs[0]
                                        alt_score = first_run.get(
                                            'perturbed_score'
                                        )
                                        alt_worst = first_run.get(
                                            'worst_score'
                                        )

                            alt_scores.append(
                                alt_score if alt_score is not None else 0.0
                            )
                            alt_worst_scores.append(
                                alt_worst if alt_worst is not None else 0.0
                            )
        ...
        if report_data['feature_subset']:
            report_data['feature_subset_display'] = ', '.join(
                report_data['feature_subset']
            )
        else:
        ...
        if 'alternative_models' in report_data:
            ...
            for alt_model_name, model_data in report_data[
                'alternative_models'
            ].items():
                ...
                if 'base_score' not in model_data:
                    logger.warning(
                        f'base_score not found in alternative model {alt_model_name}'
                    )
                ...
                if 'raw' in model_data and isinstance(model_data['raw'], dict):
                    if (
                        'avg_raw_impact' not in model_data
                        and 'overall' in model_data['raw']
                    ):
                        model_data['avg_raw_impact'] = (
                            model_data['raw']
                            .get('overall', {})
                            .get('avg_impact', 0.0)
                        )
                    ...
                    logger.warning(
                        f'raw data not found in alternative model {alt_model_name}'
                    )
                    model_data['raw'] = {}
                    model_data['avg_raw_impact'] = 0.0

                if 'quantile' in model_data and isinstance(
                    model_data['quantile'], dict
                ):
                    if (
                        'avg_quantile_impact' not in model_data
                        and 'overall' in model_data['quantile']
                    ):
                        model_data['avg_quantile_impact'] = (
                            model_data['quantile']
                            .get('overall', {})
                            .get('avg_impact', 0.0)
                        )
                    ...
                    logger.warning(
                        f'quantile data not found in alternative model {alt_model_name}'
                    )
                    model_data['quantile'] = {}
                ...
                if 'robustness_score' not in model_data:
                    if 'avg_overall_impact' in model_data:
                        model_data['robustness_score'] = float(
                            1.0 - model_data.get('avg_overall_impact', 0.0)
                        )
                    else:
                        logger.warning(
                            f'robustness_score not found in alternative model {alt_model_name}'
                        )
