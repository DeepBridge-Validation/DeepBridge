...
class ResilienceDataTransformerSimple:
    ...
    def transform(
        ...
        if 'test_results' in results:
            ...
            logger.info(
                "Using nested format: results['test_results']['primary_model']"
            )
        ...
        logger.info(
            f"Transformation complete. {transformed['metadata']['total_scenarios']} scenarios, "
    ...
    def _transform_scenarios(
        ...
        logger.info(
            f'Raw scenarios from distribution_shift.all_results: {len(scenarios)}'
        )
        ...
        for i, scenario in enumerate(scenarios):
            ...
            logger.info(
                f"Transformed scenario {i+1}: is_valid={transformed['is_valid']}, "
        ...
        logger.info(
            f"Total valid scenarios: {sum(1 for s in transformed_scenarios if s['is_valid'])}"
        )
        logger.info('=' * 70)
    ...
    def _transform_worst_sample(self, primary_model: Dict) -> Dict[str, Any]:
        ...
        for alpha, data in worst_sample_data.get('by_alpha', {}).items():
            by_alpha[str(alpha)] = {
                'avg_performance_gap': float(
                    data.get('avg_performance_gap', 0)
                ),
    ...
    def _transform_worst_cluster(self, primary_model: Dict) -> Dict[str, Any]:
        ...
        by_n_clusters = {}
        for n_clusters, data in worst_cluster_data.get(
            'by_n_clusters', {}
        ).items():
    ...
    def _transform_hard_sample(self, primary_model: Dict) -> Dict[str, Any]:
        ...
        by_threshold = {}
        for threshold, data in hard_sample_data.get(
            'by_threshold', {}
        ).items():
    ...
    def _prepare_charts(
        ...
        logger.info(
            f'Checking for missing data information in primary_model...'
        )
        ...
        if 'missing_data' in primary_model:
            ...
            logger.info(
                f"  missing_data keys: {list(primary_model['missing_data'].keys())}"
            )
        else:
        ...
        charts = {
            ...
            'test_type_comparison': self._chart_test_type_comparison(
                primary_model
            ),
        ...
        if 'score_distribution' in charts:
            ...
            if isinstance(score_dist, dict):
                logger.info(
                    f"score_distribution has 'data': {'data' in score_dist}"
                )
                logger.info(
                    f"score_distribution has 'layout': {'layout' in score_dist}"
                )
                if 'data' in score_dist:
                    logger.info(
                        f"score_distribution data length: {len(score_dist.get('data', []))}"
                    )
                    logger.info(
                        f"score_distribution data sample: {score_dist.get('data', [])[:1]}"
                    )

        return charts

    def _chart_overview(
        self, primary_model: Dict, scenarios: List[Dict]
    ) -> Dict[str, Any]:
        ...
        for i, scenario in enumerate(scenarios):
            logger.info(
                f"Scenario {i+1}: name={scenario.get('name')}, "
        ...
        for scenario in scenarios:
            ...
            if scenario['is_valid']:
                ...
                logger.info(
                    f"Added to chart: metric={metric}, alpha={scenario['alpha']}, "
        ...
        for metric, data in metrics.items():
            if data['alphas']:
                logger.info(
                    f"Creating trace for metric '{metric}': "
    ...
    def _chart_scenarios_by_alpha(
        ...
        layout = {
            ...
            'xaxis': {
                'title': 'Alpha (Shift Strength)',
                'tickmode': 'array',
                'tickvals': sorted_alphas,
            },
            'yaxis': {
                'title': 'Accuracy',
                'side': 'left',
    ...
    def _chart_scenarios_by_metric(
        self, scenarios: List[Dict]
    ) -> Dict[str, Any]:
        ...
        for metric, gaps in metrics.items():
            if gaps:
                traces.append(
                    {'y': gaps, 'type': 'box', 'name': metric, 'boxmean': 'sd'}
                )
    ...
    def _chart_feature_importance(self, features: Dict) -> Dict[str, Any]:
        ...
        for i, f in enumerate(top_features, 1):
            logger.info(
                f"Feature {i}: {f['name']} = {f['importance_abs']:.4f}"
            )
        ...
        logger.info(
            f'Feature importance chart created with {len(top_features)} features'
        )
        logger.info('=' * 80)
    ...
    def _chart_worst_cluster(self, worst_cluster: Dict) -> Dict[str, Any]:
        ...
        n_clusters_list = sorted(
            list(set([r['n_clusters'] for r in valid_results]))
        )
        gaps_by_n_clusters = []

        for n in n_clusters_list:
            cluster_results = [
                r for r in valid_results if r['n_clusters'] == n
            ]
    ...
    def _chart_hard_sample(self, hard_sample: Dict) -> Dict[str, Any]:
        ...
        valid_results = [
            r for r in results if r['is_valid'] and not r.get('skipped', False)
        ]
        ...
        thresholds = sorted(
            list(set([r['disagreement_threshold'] for r in valid_results]))
        )
        gaps_by_threshold = []
        ...
        for threshold in thresholds:
            threshold_results = [
                ...
                if r['disagreement_threshold'] == threshold
            ]
            avg_gap = np.mean(
    ...
    def _chart_test_type_comparison(
        self, primary_model: Dict
    ) -> Dict[str, Any]:
    ...
    def _chart_by_level(self, primary_model: Dict) -> Dict[str, Any]:
        ...
        if missing_data or by_level:
            ...
            for level_str, data in sorted(
                level_data.items(), key=lambda x: float(x[0])
            ):
                ...
                # Extract metrics
                mean_score = data.get(
                    'mean_score', data.get('avg_performance', 0)
                )
                worst_score = data.get(
                    'worst_score', data.get('min_performance', 0)
                )
                ...
                logger.info(
                    f'Level {level}%: mean={mean_score}, worst={worst_score}'
                )
            ...
            logger.info(
                f'Created by_level chart with {len(levels)} data points'
            )
            logger.info('=' * 80)
            ...
            logger.warning(
                "The 'By Missing %' tab in the report will be empty"
            )
            logger.warning(
                'This is expected if the resilience test focused on distribution shift'
            )
            logger.info('=' * 80)
    ...
    def _chart_missing_patterns(self, primary_model: Dict) -> Dict[str, Any]:
        ...
        if missing_patterns:
            logger.info(
                f'Found patterns data: {list(missing_patterns.keys())}'
            )
            ...
            for pattern_name, pattern_data in missing_patterns.items():
                ...
                score = pattern_data.get(
                    'performance', pattern_data.get('score', 0)
                )
            ...
            logger.info(
                f'Created missing patterns chart with {len(pattern_names)} patterns'
            )
            logger.info('=' * 80)
            ...
            logger.warning(
                'This is expected if the resilience test did not include missing data pattern analysis'
            )
            logger.info('=' * 80)
    ...
    def _chart_imputation_strategies(
        self, primary_model: Dict
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f'imputation_strategies available: {bool(imputation_strategies)}'
        )

        if imputation_strategies:
            logger.info(
                f'Found strategies data: {list(imputation_strategies.keys())}'
            )
            ...
            for strategy_name, strategy_data in imputation_strategies.items():
                ...
                score = strategy_data.get(
                    'performance', strategy_data.get('score', 0)
                )
                performance_scores.append(float(score))
            ...
            logger.info(
                f'Created imputation strategies chart with {len(strategy_names)} strategies'
            )
            logger.info('=' * 80)
            ...
            logger.warning(
                'No imputation strategies data found in primary_model'
            )
            logger.warning(
                'This is expected if the resilience test did not include imputation analysis'
            )
            logger.info('=' * 80)
    ...
    def _count_all_scenarios(self, primary_model: Dict) -> int:
        ...
        total = 0
        total += len(
            primary_model.get('distribution_shift', {}).get('all_results', [])
        )
        total += len(
            primary_model.get('worst_sample', {}).get('all_results', [])
        )
        total += len(
            primary_model.get('worst_cluster', {}).get('all_results', [])
        )
        total += len(
            primary_model.get('outer_sample', {}).get('all_results', [])
        )
        total += len(
            primary_model.get('hard_sample', {}).get('all_results', [])
        )
        return total
