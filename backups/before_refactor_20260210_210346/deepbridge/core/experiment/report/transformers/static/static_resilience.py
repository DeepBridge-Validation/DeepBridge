...
class StaticResilienceTransformer:
    ...
    def transform(
        self, data: Dict[str, Any], model_name: str = 'Model'
    ) -> Dict[str, Any]:
        """
        ...
        try:
            transformed_data = self.base_transformer.transform(
                data, model_name
            )
            logger.info(
                f'Base transformer produced data with keys: {list(transformed_data.keys() if isinstance(transformed_data, dict) else [])}'
            )
        ...
        output = {
            'model_name': model_name,
            'test_type': 'resilience',
            'timestamp': transformed_data.get(
                'timestamp',
                datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            ),
            'model_type': transformed_data.get(
                'model_type', data.get('model_type', 'Classification')
            ),
            'features': transformed_data.get(
        ...
        if 'avg_performance_gap' in transformed_data:
            output['avg_performance_gap'] = transformed_data[
                'avg_performance_gap'
            ]
        ...
        if 'feature_importance' in transformed_data and isinstance(
            transformed_data['feature_importance'], dict
        ):
            output['feature_importance'] = transformed_data[
                'feature_importance'
            ]
        elif 'feature_importance' in data and isinstance(
        ...
        if 'feature_distances' in transformed_data and isinstance(
            transformed_data['feature_distances'], dict
        ):
            output['feature_distances'] = transformed_data['feature_distances']
        elif 'feature_distances' in data and isinstance(
            data['feature_distances'], dict
        ):
        ...
        for source in [transformed_data, data]:
            if 'distribution_shift' in source and isinstance(
                source['distribution_shift'], dict
            ):
        ...
        performance_metrics = {}
        for source in [transformed_data, data]:
            if 'performance_metrics' in source and isinstance(
                source['performance_metrics'], dict
            ):
        ...
        for sample_type in ['worst_samples', 'remaining_samples']:
            if sample_type in transformed_data and isinstance(
                transformed_data[sample_type], dict
            ):
        ...
        if 'feature_correlations' in transformed_data and isinstance(
            transformed_data['feature_correlations'], dict
        ):
            output['feature_correlations'] = transformed_data[
                'feature_correlations'
            ]
        elif 'feature_correlations' in data and isinstance(
            data['feature_correlations'], dict
        ):
        ...
        for key in ['by_alpha', 'alpha_results', 'perturbation_levels']:
            if key in transformed_data and isinstance(
        ...
        if 'alternative_models' in transformed_data and isinstance(
            transformed_data['alternative_models'], dict
        ):
            output['alternative_models'] = transformed_data[
                'alternative_models'
            ]
        elif 'alternative_models' in data and isinstance(
            data['alternative_models'], dict
        ):
            output['alternative_models'] = data['alternative_models']

        # Extract model feature importance if available (for comparison)
        if 'model_feature_importance' in transformed_data and isinstance(
            transformed_data['model_feature_importance'], dict
        ):
            output['model_feature_importance'] = transformed_data[
                'model_feature_importance'
            ]
        elif 'model_feature_importance' in data and isinstance(
            data['model_feature_importance'], dict
        ):
            output['model_feature_importance'] = data[
                'model_feature_importance'
            ]

        # Extract sensitive features if available
        if 'sensitive_features' in transformed_data:
            output['sensitive_features'] = transformed_data[
                'sensitive_features'
            ]
        ...
        if 'most_affected_scenario' in transformed_data:
            output['most_affected_scenario'] = transformed_data[
        ...
        if (
            not output.get('features')
            and 'feature_distances' in output
            and isinstance(output['feature_distances'], dict)
        ):
            output['features'] = list(output['feature_distances'].keys())
        elif (
            not output.get('features')
            and 'feature_importance' in output
            and isinstance(output['feature_importance'], dict)
        ):
            output['features'] = list(output['feature_importance'].keys())
        elif (
            not output.get('features')
            and 'distribution_shift' in output
            and isinstance(output['distribution_shift'], dict)
        ):
            dist_shift = output['distribution_shift']
            if 'by_distance_metric' in dist_shift and isinstance(
                dist_shift['by_distance_metric'], dict
            ):
                for metric_name, metric_data in dist_shift[
                    'by_distance_metric'
                ].items():
                    if isinstance(metric_data, dict):
                        if (
                            'avg_feature_distances' in metric_data
                            and isinstance(
                                metric_data['avg_feature_distances'], dict
                            )
                        ):
                            output['features'] = list(
                                metric_data['avg_feature_distances'].keys()
                            )
                            break
                        elif 'top_features' in metric_data and isinstance(
                            metric_data['top_features'], dict
                        ):
                            output['features'] = list(
                                metric_data['top_features'].keys()
                            )
                            break

        logger.info(
            f"Transformed resilience data for static report: resilience_score={output.get('resilience_score', 'N/A')}, avg_performance_gap={output.get('avg_performance_gap', 'N/A')}"
        )

        # Generate and add visualizations to the output data
        if (
            HAS_VISUALIZATION_LIBS
            and HAS_CHART_GENERATOR
            and self.chart_generator
        ):
            logger.info(
                f'Starting chart generation with modular chart generator'
            )
            try:
                logger.info(
                    f'Input data for charts has keys: {list(output.keys())}'
                )

                # Log important data fields for debugging
                if 'resilience_score' in output:
                    logger.info(
                        f"Resilience score: {output['resilience_score']}"
                    )
                if 'avg_performance_gap' in output:
                    logger.info(
                        f"Average performance gap: {output['avg_performance_gap']}"
                    )
                if 'feature_importance' in output:
                    logger.info(
                        f"Feature importance available with {len(output['feature_importance'])} features"
                    )
                if 'feature_distances' in output:
                    logger.info(
                        f"Feature distances available with {len(output['feature_distances'])} features"
                    )
                if 'performance_metrics' in output:
                    logger.info(
                        f"Performance metrics available with keys: {list(output['performance_metrics'].keys())}"
                    )

                output['charts'] = self._generate_charts(output)
                logger.info(
                    f"Generated {len(output['charts'])} charts for resilience visualization"
                )
                ...
                for chart_name in output['charts']:
                    ...
                    logger.info(
                        f"Chart '{chart_name}' generated with data size: {chart_size} bytes"
                    )
            except Exception as e:
            ...
            if not HAS_VISUALIZATION_LIBS:
                logger.error(
                    'Cannot generate charts: visualization libraries not available'
                )
                logger.error('Required libraries: matplotlib, seaborn, pandas')
            if not HAS_CHART_GENERATOR:
                logger.error(
                    'Cannot generate charts: chart generator not available'
                )
                logger.error(
                    'Required module: templates.report_types.resilience.static.charts.ResilienceChartGenerator'
                )
            output['charts'] = {}
    ...
    def _generate_charts(self, report_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        logger.info(
            f'Running _generate_charts in transformer with data keys: {list(report_data.keys())}'
        )
        ...
        try:
            ...
            if resilience_score is not None or performance_gap is not None:
                ...
                if resilience_score is not None:
                    try:
                        metrics_data['Resilience Score'] = float(
                            resilience_score
                        )
                ...
                if performance_gap is not None:
                    try:
                        metrics_data['Performance Gap'] = float(
                            performance_gap
                        )
                    except (ValueError, TypeError):
                        pass
                ...
                if metrics_data:
                    charts[
                        'resilience_score_chart'
                    ] = self.chart_generator.generate_model_resilience_scores(
        ...
        try:
            ...
            logger.info(
                f'Feature distances data present: {feature_distances is not None}'
            )
            ...
            if not feature_distances:
                ...
                if distribution_shift and isinstance(distribution_shift, dict):
                    logger.info(
                        f'Distribution shift data keys: {list(distribution_shift.keys())}'
                    )

                    # Check by_distance_metric
                    if (
                        'by_distance_metric' in distribution_shift
                        and isinstance(
                            distribution_shift['by_distance_metric'], dict
                        )
                    ):
                        by_distance_metric = distribution_shift[
                            'by_distance_metric'
                        ]
                        logger.info(
                            f'Found by_distance_metric with keys: {list(by_distance_metric.keys())}'
                        )
                        ...
                        for (
                            metric_name,
                            metric_data,
                        ) in by_distance_metric.items():
                            if (
                                isinstance(metric_data, dict)
                                and 'avg_feature_distances' in metric_data
                            ):
                                feature_distances = metric_data[
                                    'avg_feature_distances'
                                ]
                                logger.info(
                                    f'Extracted avg_feature_distances from {metric_name}: {feature_distances is not None}'
                                )
                            ...
                            if (
                                not feature_distances
                                and isinstance(metric_data, dict)
                                and 'top_features' in metric_data
                            ):
                                feature_distances = metric_data['top_features']
                                logger.info(
                                    f'Extracted top_features from {metric_name}: {feature_distances is not None}'
                                )
                                if feature_distances:
                                    break
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        if isinstance(fd, dict):
                                            ...
                                            if feature_distances:
                                                logger.info(
                                                    f'Extracted feature distances from {metric_name} results: {feature_distances is not None}'
                                                )
                                                break
                    ...
                    if not feature_distances:
                        ...
                        if 'feature_distances' in distribution_shift:
                            feature_distances = distribution_shift[
                                'feature_distances'
                            ]
                            logger.info(
                                f'Found feature_distances in distribution_shift: {feature_distances is not None}'
                            )
                        # Try known field names
                        elif 'features' in distribution_shift:
                            feature_distances = distribution_shift['features']
                            logger.info(
                                "Using 'features' field from distribution_shift"
                            )
                        elif 'distances' in distribution_shift:
                            feature_distances = distribution_shift['distances']
                            logger.info(
                                "Using 'distances' field from distribution_shift"
                            )
                        # Try by pattern
                        else:
                            for key in distribution_shift.keys():
                                if (
                                    ...
                                    distribution_shift[key], dict
                                ):
                                    feature_distances = distribution_shift[key]
                                    logger.info(
                                        f"Using '{key}' from distribution_shift"
                                    )
                                    break
            ...
            if not feature_distances:
                for field in [
                    ...
                    if field in report_data and isinstance(
                        report_data[field], dict
                    ):
                        logger.info(
                            f"Checking '{field}' for feature distances"
                        )
                        ...
                        for key, value in potential_source.items():
                            if isinstance(value, dict) and len(value) > 0:
                                ...
                                if not first_key.startswith(
                                    '_'
                                ) and not first_key.startswith('__'):
                                    feature_distances = value
                                    logger.info(
                                        f"Using '{field}.{key}' as feature distances"
                                    )
                                    break

            if feature_distances and isinstance(feature_distances, dict):
                logger.info(
                    f'Feature distances data has {len(feature_distances)} features'
                )
                ...
                try:
                    ...
                    if chart_data:
                        ...
                        logger.info(
                            f'Generated feature distribution shift chart: {len(chart_data)} bytes'
                        )
                    else:
                        logger.error(
                            'generate_feature_distribution_shift returned empty result'
                        )
                except Exception as inner_e:
                    logger.error(
                        f'Error in generate_feature_distribution_shift: {str(inner_e)}'
                    )
                    logger.error(f'Traceback: {traceback.format_exc()}')
            else:
                logger.warning(
                    'No valid feature distances data found for chart'
                )
        except Exception as e:
            logger.error(
                f'Error generating feature distribution shift chart: {str(e)}'
            )
            logger.error(f'Traceback: {traceback.format_exc()}')
        ...
        try:
            ...
            logger.info(
                f'Performance metrics present: {performance_metrics is not None}'
            )
            ...
            if not performance_metrics:
                ...
                if 'distribution_shift' in report_data and isinstance(
                    report_data['distribution_shift'], dict
                ):
                    ...
                    if 'by_distance_metric' in dist_shift and isinstance(
                        ...
                        logger.info(
                            'Checking distribution_shift.by_distance_metric for performance metrics'
                        )
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    if isinstance(last_result, dict):
                                        ...
                                        worst_keys = [
                                            k
                                            for k in constructed_metrics.keys()
                                            if k.startswith('worst_')
                                        ]
                                        remaining_keys = [
                                            k
                                            for k in constructed_metrics.keys()
                                            if k.startswith('remaining_')
                                        ]
                                        ...
                                        if worst_keys and remaining_keys:
                                            ...
                                            logger.info(
                                                f'Constructed performance metrics from {metric_name} results with {len(performance_metrics)} keys'
                                            )
                                            break
                ...
                if not performance_metrics:
                    ...
                    for field in [
                        ...
                        if field in report_data and isinstance(
                            ...
                            logger.info(
                                f"Checking '{field}' for performance metrics"
                            )
                            ...
                            if worst_keys or remaining_keys:
                                logger.info(
                                    f"Found potential performance metrics in '{field}'"
                                )
                            ...
                            for subfield, value in metrics_data.items():
                                if isinstance(value, dict):
                                    ...
                                    if worst_keys or remaining_keys:
                                        logger.info(
                                            f"Found potential performance metrics in '{field}.{subfield}'"
                                        )
            ...
            if performance_metrics and isinstance(performance_metrics, dict):
                logger.info(
                    f'Performance metrics keys: {list(performance_metrics.keys())}'
                )
                logger.info(
                    f'Performance metrics values: {performance_metrics}'
                )
                ...
                for key in performance_metrics.keys():
                    if key.startswith('worst_'):
                        ...
                        if (
                            base_metric.endswith('_count')
                            or base_metric == 'sample_count'
                        ):
                            continue
                        ...
                        if remaining_key in performance_metrics:
                            paired_metrics.append(
                                (key, remaining_key, base_metric)
                            )

                logger.info(
                    f'Found {len(paired_metrics)} paired metrics: {[m[2] for m in paired_metrics]}'
                )
                ...
                if paired_metrics:
                    ...
                    if any(
                        key in performance_metrics
                        for key in ['worst_mse', 'worst_mae', 'worst_r2']
                    ):
                    ...
                    try:
                        ...
                        chart_data = (
                            self.chart_generator.generate_performance_gap(
                                performance_metrics, task_type=task_type
                            )
                        )
                        if chart_data:
                            charts['performance_gap_chart'] = chart_data
                            logger.info(
                                f'Generated performance gap chart: {len(chart_data)} bytes'
                            )
                        else:
                            logger.error(
                                'generate_performance_gap returned empty result'
                            )
                    except Exception as inner_e:
                        logger.error(
                            f'Error in generate_performance_gap: {str(inner_e)}'
                        )
                        logger.error(f'Traceback: {traceback.format_exc()}')
                else:
                    logger.warning(
                        'No paired metrics (worst_X and remaining_X) found'
                    )
            else:
        ...
        try:
            ...
            if feature_importance and (worst_samples or remaining_samples):
                feature_list = list(feature_importance.keys())[
                    :5
                ]  # Get up to 5 top features

                charts[
                    'critical_feature_distributions'
                ] = self.chart_generator.generate_critical_feature_distributions(
            ...
            logger.error(
                f'Error generating critical feature distributions chart: {str(e)}'
            )
        ...
        try:
            ...
            if feature_correlations and isinstance(feature_correlations, dict):
                ...
                ] = self.chart_generator.generate_feature_residual_correlation(
                    feature_correlations
                )
                logger.info('Generated feature-residual correlation chart')
        except Exception as e:
            logger.error(
                f'Error generating feature-residual correlation chart: {str(e)}'
            )
        ...
        try:
            ...
            if residuals or worst_residuals or remaining_residuals:
                ...
                ] = self.chart_generator.generate_residual_distribution(
            ...
            logger.error(
                f'Error generating residual distribution chart: {str(e)}'
            )
        ...
        try:
            feature_importance = report_data.get('feature_importance')
            logger.info(
                f'Feature importance present: {feature_importance is not None}'
            )
            ...
            if not feature_importance:
                ...
                if 'distribution_shift' in report_data and isinstance(
                    ...
                    if 'by_distance_metric' in dist_shift and isinstance(
                        ...
                        logger.info(
                            'Checking distribution_shift.by_distance_metric for feature importance'
                        )
                        ...
                        for metric_name, metric_data in dist_shift[
                            ...
                            if isinstance(metric_data, dict):
                                ...
                                if (
                                    'top_features' in metric_data
                                    and isinstance(
                                        metric_data['top_features'], dict
                                    )
                                ):
                                    feature_importance = metric_data[
                                        'top_features'
                                    ]
                                    logger.info(
                                        f'Using top_features from {metric_name} as feature importance'
                                    )
                                    break
                                ...
                                if (
                                    ...
                                    and 'avg_feature_distances' in metric_data
                                    and isinstance(
                                        metric_data['avg_feature_distances'],
                                        dict,
                                    )
                                ):
                                    feature_importance = metric_data[
                                    ...
                                    logger.info(
                                        f'Using avg_feature_distances from {metric_name} as feature importance'
                                    )
                                    break
                                ...
                                if (
                                    ...
                                    and 'results' in metric_data
                                    and isinstance(
                                        metric_data['results'], list
                                    )
                                    and len(metric_data['results']) > 0
                                ):
                                    for result in metric_data['results']:
                                        if (
                                            isinstance(result, dict)
                                            and 'feature_distances' in result
                                        ):
                                            ...
                                            if isinstance(fd, dict):
                                                if (
                                                    ...
                                                    feature_importance = fd[
                                                        'top_features'
                                                    ]
                                                    logger.info(
                                                        f'Using feature_distances.top_features from {metric_name} results as feature importance'
                                                    )
                                                    break
                                                elif (
                                                    'all_feature_distances'
                                                    ...
                                                    logger.info(
                                                        f'Using feature_distances.all_feature_distances from {metric_name} results as feature importance'
                                                    )
                                                    break
                ...
                if not feature_importance:
                    ...
                    for field in [
                        ...
                        if field in report_data and isinstance(
                            ...
                            logger.info(
                                f"Checking '{field}' for feature importance"
                            )

                            # Look for direct 'feature_importance' key
                            if (
                                'feature_importance' in field_data
                                and isinstance(
                                    field_data['feature_importance'], dict
                                )
                            ):
                                feature_importance = field_data[
                                    'feature_importance'
                                ]
                                logger.info(
                                    f"Found feature importance in '{field}.feature_importance'"
                                )
                                break
                            ...
                            for key in field_data.keys():
                                if (
                                    'feature' in key.lower()
                                    and 'import' in key.lower()
                                    and isinstance(field_data[key], dict)
                                ):
                                    feature_importance = field_data[key]
                                    logger.info(
                                        f"Found feature importance in '{field}.{key}'"
                                    )
                                    break
                            ...
                            if not feature_importance:
                                for subfield, subvalue in field_data.items():
                                    if isinstance(subvalue, dict):
                                        ...
                                        if (
                                            'feature_importance' in subvalue
                                            and isinstance(
                                                subvalue['feature_importance'],
                                                dict,
                                            )
                                        ):
                                        ...
                                        if len(subvalue) > 3 and not any(
                                            ...
                                            if numeric:
                                                feature_importance = subvalue
                                                logger.info(
                                                    f"Found potential feature importance in '{field}.{subfield}'"
                                                )
                                                break

            if feature_importance and isinstance(feature_importance, dict):
                logger.info(
                    f'Feature importance has {len(feature_importance)} features'
                )
                ...
                try:
                    ...
                    if chart_data:
                        ...
                        logger.info(
                            f'Generated feature importance chart: {len(chart_data)} bytes'
                        )
                    else:
                        logger.error(
                            'generate_feature_importance_chart returned empty result'
                        )
                except Exception as inner_e:
                    logger.error(
                        f'Error in generate_feature_importance_chart: {str(inner_e)}'
                    )
                    logger.error(f'Traceback: {traceback.format_exc()}')
                ...
                logger.info(
                    f'Model feature importance present: {model_feature_importance is not None}'
                )

                if model_feature_importance and isinstance(
                    model_feature_importance, dict
                ):
                    try:
                        # Create comparison data for both importance metrics
                        comparison_data = {}
                        for feature in set(feature_importance.keys()) | set(
                            model_feature_importance.keys()
                        ):
                            if (
                                feature in feature_importance
                                or feature in model_feature_importance
                            ):
                                comparison_data[feature] = {
                                    'Model Importance': float(
                                        model_feature_importance.get(
                                            feature, 0
                                        )
                                    ),
                                    'Resilience Impact': float(
                        ...
                        if comparison_data:
                            ...
                            if comparison_chart:
                                ...
                                logger.info(
                                    f'Generated feature comparison chart: {len(comparison_chart)} bytes'
                                )
                            else:
                                logger.error(
                                    'Feature comparison chart generation returned empty result'
                                )
                    except Exception as inner_e:
                        logger.error(
                            f'Error generating feature comparison chart: {str(inner_e)}'
                        )
                        logger.error(f'Traceback: {traceback.format_exc()}')
            else:
        ...
        try:
            ...
            logger.info(
                f'Alternative models present: {alternative_models is not None}'
            )
            ...
            if alternative_models and isinstance(alternative_models, dict):
                logger.info(
                    f'Alternative models has {len(alternative_models)} entries'
                )
            ...
            if by_alpha and isinstance(by_alpha, dict):
                ...
                if sample_alphas:
                    ...
                    if isinstance(first_alpha_data, dict):
                        logger.info(
                            f'  Alpha data contains keys: {list(first_alpha_data.keys())}'
                        )
                        if 'score' in first_alpha_data:
                            logger.info(
                                f"  Alpha data contains score field: {first_alpha_data['score']}"
                            )
            ...
            if not by_alpha:
                ...
                if 'distribution_shift' in report_data and isinstance(
                    ...
                    if 'by_alpha' in dist_shift and isinstance(
                        dist_shift['by_alpha'], dict
                    ):
                        by_alpha = dist_shift['by_alpha']
                        logger.info(
                            f'Found by_alpha in distribution_shift with {len(by_alpha)} entries'
                        )
                ...
                if (
                    not by_alpha
                    and 'distribution_shift' in report_data
                    and isinstance(report_data['distribution_shift'], dict)
                ):
                    dist_shift = report_data['distribution_shift']
                            ...
                            if (
                                ...
                                for result in metric_data.get('results', []):
                                    if (
                                        ...
                                        if (
                                            'worst_metric' in result
                                            and 'remaining_metric' in result
                                        ):
                                            alpha_dict[alpha_key][
                                                'score'
                                            ] = result['remaining_metric']

                                if alpha_dict:
                                    by_alpha = alpha_dict
                                    logger.info(
                                        f'Constructed by_alpha from distribution_shift.by_distance_metric.{metric_name} with {len(by_alpha)} entries'
                                    )
                                    break
            ...
            if (
                alternative_models
                and by_alpha
                and isinstance(alternative_models, dict)
                and isinstance(by_alpha, dict)
            ):
                try:
                    ...
                    # Add primary model
                    primary_model_name = report_data.get(
                        'model_name', 'Primary Model'
                    )
                    primary_resilience_score = report_data.get(
                        'resilience_score'
                    )
                    ...
                    for alt_name, alt_data in alternative_models.items():
                        if 'by_alpha' in alt_data and isinstance(
                            alt_data['by_alpha'], dict
                        ):
                            alt_alpha_levels = []
                            alt_scores = []

                            for alpha_str, alpha_data in sorted(
                                alt_data['by_alpha'].items()
                            ):
                                try:
                                    alpha = float(alpha_str)
                                    score = alpha_data.get('score')
                            ...
                            if alt_alpha_levels:
                                ...
                                if 'resilience_score' in alt_data:
                                    try:
                                        alt_resilience_score = float(
                                            alt_data['resilience_score']
                                        )
                                    except (ValueError, TypeError):
                                        pass
                    ...
                    if (
                        ...
                        if chart_data:
                            ...
                            logger.info(
                                f'Generated model comparison chart: {len(chart_data)} bytes'
                            )
                        else:
                            logger.error(
                                'generate_model_comparison returned empty result'
                            )
                except Exception as inner_e:
                    logger.error(
                        f'Error in generate_model_comparison: {str(inner_e)}'
                    )
                    logger.error(f'Traceback: {traceback.format_exc()}')
            else:
                logger.warning(
                    'Missing required data for model comparison chart'
                )
        except Exception as e:
        ...
        try:
            ...
            if 'performance_metrics' in report_data and isinstance(
                report_data['performance_metrics'], dict
            ):
            ...
            if (
                primary_resilience_score is not None
                and primary_accuracy is not None
            ):
            ...
            if alternative_models and isinstance(alternative_models, dict):
                for model_name, model_data in alternative_models.items():
                    ...
                    if 'performance_metrics' in model_data and isinstance(
                        model_data['performance_metrics'], dict
                    ):
                    ...
                    if (
                        alt_resilience_score is not None
                        and alt_accuracy is not None
                    ):
            ...
            if len(models_accuracy_resilience) >= 2:
                try:
                    ...
                    scatter_chart = (
                        self.chart_generator.generate_model_comparison_scatter(
                    ...
                    if scatter_chart:
                        ...
                        logger.info(
                            f'Generated model comparison scatter plot: {len(scatter_chart)} bytes'
                        )
                    else:
                        logger.error(
                            'generate_model_comparison_scatter returned empty result'
                        )
                except Exception as inner_e:
                    logger.error(
                        f'Error in generate_model_comparison_scatter: {str(inner_e)}'
                    )
                    logger.error(f'Traceback: {traceback.format_exc()}')
            else:
                logger.warning(
                    'Not enough models with both accuracy and resilience scores for scatter plot'
                )
        except Exception as e:
            logger.error(
                f'Error generating model comparison scatter plot: {str(e)}'
            )
            logger.error(f'Traceback: {traceback.format_exc()}')
                ...
                if 'by_distance_metric' in dist_shift and isinstance(
                    ...
                    if alpha_levels:
                        ...
                        if metrics_data and len(metrics_data) > 1:
                            try:
                                logger.info(
                                    'Calling generate_distance_metrics_comparison'
                                )
                                ...
                                if metrics_chart:
                                    ...
                                    logger.info(
                                        f'Generated distance metrics comparison chart: {len(metrics_chart)} bytes'
                                    )
                                else:
                                    logger.error(
                                        'generate_distance_metrics_comparison returned empty result'
                                    )
                            except Exception as inner_e:
                                logger.error(
                                    f'Error in generate_distance_metrics_comparison: {str(inner_e)}'
                                )
                                logger.error(
                            ...
                            logger.warning(
                                'Not enough metrics data for distance metrics comparison chart'
                            )
                    else:
                        logger.warning(
                            'No valid alpha levels found for distance metrics comparison'
                        )
                else:
                    logger.warning(
                        'No by_distance_metric data available for distance metrics comparison'
                    )
            else:
                logger.warning(
                    'No distribution_shift data available for distance metrics comparison'
                )
        except Exception as e:
            logger.error(
                f'Error generating distance metrics comparison chart: {str(e)}'
            )
            logger.error(f'Traceback: {traceback.format_exc()}')
        ...
        try:
            ...
            if 'distribution_shift' in report_data and isinstance(
                ...
                if 'by_distance_metric' in dist_shift and isinstance(
                    ...
                    for metric_name, metric_data in by_distance_metric.items():
                        if isinstance(metric_data, dict):
                            ...
                            if (
                                ...
                                feature_distances_by_metric[
                                    metric_name
                                ] = metric_data['top_features']
                            # Check in results
                            elif (
                                'results' in metric_data
                                and isinstance(metric_data['results'], list)
                                and len(metric_data['results']) > 0
                            ):
                                ...
                                if (
                                    ...
                                    and 'feature_distances' in last_result
                                ):
                                    fd = last_result['feature_distances']
                                    if isinstance(fd, dict):
                                        if (
                                            'all_feature_distances' in fd
                                            and isinstance(
                                                fd['all_feature_distances'],
                                                dict,
                                            )
                                        ):
                                            feature_distances_by_metric[
                    ...
                    if (
                        feature_distances_by_metric
                        and len(feature_distances_by_metric) > 1
                    ):
                        try:
                            logger.info(
                                'Calling generate_feature_distance_heatmap'
                            )
                            ...
                            if heatmap_chart:
                                ...
                                logger.info(
                                    f'Generated feature distance heatmap: {len(heatmap_chart)} bytes'
                                )
                            else:
                                logger.error(
                                    'generate_feature_distance_heatmap returned empty result'
                                )
                        except Exception as inner_e:
                            logger.error(
                                f'Error in generate_feature_distance_heatmap: {str(inner_e)}'
                            )
                            logger.error(
                                f'Traceback: {traceback.format_exc()}'
                            )
                        ...
                        logger.warning(
                            'Not enough metrics with feature distances for heatmap'
                        )
                else:
                    logger.warning(
                        'No by_distance_metric data available for feature distance heatmap'
                    )
            else:
                logger.warning(
                    'No distribution_shift data available for feature distance heatmap'
                )
        except Exception as e:
            logger.error(
                f'Error generating feature distance heatmap: {str(e)}'
            )
            logger.error(f'Traceback: {traceback.format_exc()}')
            ...
            if models_resilience:
                try:
                    ...
                    if bar_chart:
                        ...
                        logger.info(
                            f'Generated model resilience scores bar chart: {len(bar_chart)} bytes'
                        )
                    else:
                        logger.error(
                            'generate_model_resilience_scores returned empty result'
                        )
                except Exception as inner_e:
                    logger.error(
                        f'Error in generate_model_resilience_scores: {str(inner_e)}'
                    )
                    logger.error(f'Traceback: {traceback.format_exc()}')
            else:
                logger.warning(
                    'No models with resilience scores available for bar chart'
                )
        except Exception as e:
            logger.error(
                f'Error generating model resilience scores bar chart: {str(e)}'
            )
            logger.error(f'Traceback: {traceback.format_exc()}')
        ...
        try:
            ...
            def extract_performance_gaps(model_data):
                ...
                if (
                    ...
                    or 'distribution_shift' not in model_data
                ):
                    return None

                dist_shift = model_data['distribution_shift']
                if (
                    not isinstance(dist_shift, dict)
                    or 'by_alpha' not in dist_shift
                ):
                    return None
                ...
                for alpha in alpha_keys:
                    alpha_key = (
                        str(alpha) if isinstance(alpha, float) else alpha
                    )
                    ...
                    # Try to extract from results
                        ...
                        'results' in alpha_data
                        and isinstance(alpha_data['results'], list)
                        and len(alpha_data['results']) > 0
                    ):
                        ...
                        if isinstance(results, dict):
                            ...
                            # Worst and remaining metrics
                                ...
                                'worst_metric' in results
                                and 'remaining_metric' in results
                            ):
                ...
                if not performance_gaps and worst_values and remaining_values:
                    performance_gaps = [
                        abs(r - w)
                        for w, r in zip(worst_values, remaining_values)
                    ]
            ...
            if 'distribution_shift' in report_data and isinstance(
                ...
                primary_data = {
                    'distribution_shift': report_data['distribution_shift']
                }
            # Or from primary_model field
            elif 'primary_model' in report_data and isinstance(
            ...
            if alpha_levels and models_perf_data:
                try:
                    logger.info(
                        f'Starting performance_gap_by_alpha chart with {len(models_perf_data)} models and {len(alpha_levels)} alpha levels'
                    )
                    ...
                    try:
                        ...
                        for model_name, model_data in models_perf_data.items():
                            if 'gaps' in model_data and model_data['gaps']:
                                ...
                                model_alphas = model_data.get(
                                    'alphas',
                                    alpha_levels[: len(model_data['gaps'])],
                                )

                                # Ensure alphas and gaps have the same length
                                n_points = min(
                                    len(model_alphas), len(model_data['gaps'])
                                )
                        ...
                        if all_alphas and all_gaps and all_models:
                            ...
                            ax.set_title(
                                'Performance Gap by Alpha Level', fontsize=16
                            )
                            ax.set_xlabel(
                                'Alpha (Perturbation Intensity)', fontsize=14
                            )
                            ...
                            logger.info(
                                f"Generated performance gap by alpha chart: {len(charts['performance_gap_by_alpha'])} bytes"
                            )
                        else:
                            logger.warning(
                                'Failed to prepare data for performance gap chart'
                            )
                    except ImportError as imp_err:
                        logger.error(
                            f'Cannot generate chart - required libraries missing: {str(imp_err)}'
                        )

                        # Try to use chart generator as fallback
                        if self.chart_generator and hasattr(
                            self.chart_generator,
                            'generate_performance_gap_by_alpha',
                        ):
                            ...
                            if chart_data:
                                ...
                                logger.info(
                                    f'Generated performance gap chart using chart generator: {len(chart_data)} bytes'
                                )
                except Exception as inner_e:
                    logger.error(
                        f'Error generating performance gap chart: {str(inner_e)}'
                    )
                    logger.error(f'Traceback: {traceback.format_exc()}')
            else:
                logger.warning(
                    'Not enough data for performance gap by alpha chart'
                )
        except Exception as e:
            logger.error(
        ...
        logger.info(
            f'Generated {len(charts)} charts for resilience visualization'
        )
        return charts
