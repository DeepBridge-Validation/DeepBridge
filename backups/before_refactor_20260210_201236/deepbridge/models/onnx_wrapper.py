...
class ONNXModelWrapper:
    ...
    def __init__(
        ...
        if onnx_session is not None:
            ...
            raise ValueError(
                'Either onnx_path or onnx_session must be provided'
            )
        ...
        self.n_features = (
            input_info.shape[1] if len(input_info.shape) > 1 else None
        )
        ...
        if self.task_type == 'classification':
            ...
            if self.class_names is None:
                self.class_names = [
                    f'class_{i}' for i in range(self.n_classes_)
                ]
    ...
    def predict_proba(self, X: Union[np.ndarray, pd.DataFrame]) -> np.ndarray:
        ...
        if len(probabilities.shape) == 1 or probabilities.shape[1] == 1:
            ...
            if not np.allclose(probabilities.sum(axis=1), 1.0, rtol=1e-3):
                ...
                exp_scores = np.exp(
                    probabilities
                    - np.max(probabilities, axis=1, keepdims=True)
                )
    ...
    def feature_importances_(self) -> np.ndarray:
        ...
        raise AttributeError(
            'ONNX models do not provide feature importances directly'
        )

    def __repr__(self) -> str:
        """String representation of the wrapper."""
        return (
            f"ONNXModelWrapper(task_type='{self.task_type}', "
...
def load_onnx_model(
    onnx_path: str, task_type: str = 'classification', **kwargs
) -> ONNXModelWrapper:
    """
