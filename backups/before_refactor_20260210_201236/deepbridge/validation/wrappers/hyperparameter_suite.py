...
class HyperparameterSuite:
    ...
    _CONFIG_TEMPLATES = {
        'quick': [
            {
                'method': 'importance',
                'params': {'cv': 3, 'n_subsamples': 5, 'subsample_size': 0.5},
            }
        ],
        'medium': [
            {
                'method': 'importance',
                'params': {'cv': 5, 'n_subsamples': 10, 'subsample_size': 0.5},
            }
        ],
        'full': [
            {
                'method': 'importance',
                'params': {'cv': 5, 'n_subsamples': 20, 'subsample_size': 0.5},
            },
            {
                'method': 'importance',
                'params': {'cv': 5, 'n_subsamples': 10, 'subsample_size': 0.7},
            },
        ],
    }
    ...
    def _get_param_grid(self):
        ...
        """
        if (
            hasattr(self.dataset, 'param_grid')
            and self.dataset.param_grid is not None
        ):
        ...
        if (
            ...
            or model_type == 'RandomForestRegressor'
        ):
            return {
            ...
            or model_type == 'GradientBoostingRegressor'
        ):
            return {
                'n_estimators': [50, 100, 200],
            ...
            if self.verbose:
                print(
                    f'Could not infer parameter grid for model type: {model_type}'
                )
    ...
    def config(
        ...
        if self.verbose:
            ...
            for i, test in enumerate(self.current_config, 1):
                ...
                display_params = {
                    k: v for k, v in params.items() if k != 'feature_subset'
                }
                param_str = ', '.join(
                    f'{k}={v}' for k, v in display_params.items()
                )
    ...
    def evaluate_importance(self, method: str, params: Dict) -> Dict[str, Any]:
        ...
        for i in range(n_subsamples):
            ...
            for param_name, param_values in self.param_grid.items():
                ...
                for value in param_values:
                    ...
                    default_params = {
                        p: v[0] for p, v in self.param_grid.items()
                    }
                    ...
                    performance = self._evaluate_model(
                        X_sub, y_sub, default_params, cv=cv
                    )
                    ...
                    performance_data[param_name][str(value)].append(
                        performance
                    )
        ...
        raw_importance_scores = {
            param: np.mean(variations)
            for param, variations in param_variations.items()
        }

        # Normalize scores to sum to 1
        total = (
            sum(raw_importance_scores.values()) or 1.0
        )  # Avoid division by zero
        normalized_importance = {
            param: score / total
            for param, score in raw_importance_scores.items()
        }
        ...
        for param, values_dict in performance_data.items():
            avg_performance[param] = {
                value: np.mean(perfs) for value, perfs in values_dict.items()
            }
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for test_config in self.current_config:
            ...
            if method == 'importance':
                ...
                if self.verbose:
                    print(
                        f'Running hyperparameter importance analysis with cv={cv}, '
        ...
        avg_importance = {
            param: np.mean(scores)
            for param, scores in all_importance_scores.items()
        }

        # Normalize average scores to sum to 1
        total = sum(avg_importance.values()) or 1.0  # Avoid division by zero
        normalized_avg_importance = {
            param: score / total for param, score in avg_importance.items()
        }

        # Sort parameters by average importance
        sorted_avg_importance = dict(
            sorted(
                normalized_avg_importance.items(),
                key=lambda x: x[1],
                reverse=True,
            )
        )
        ...
        if self.verbose:
            ...
            for i, param in enumerate(results['tuning_order'], 1):
                print(
                    f'  {i}. {param} (importance: {sorted_avg_importance[param]:.4f})'
                )
    ...
    def save_report(self, output_path: str) -> None:
        ...
        for config_key, config_results in (
            test_results.get('importance', {}).get('by_config', {}).items()
        ):
            report_lines.append(f'\n### Configuration: {config_key}')
            report_lines.append(f"CV folds: {config_results.get('cv')}")
            report_lines.append(
                f"Subsamples: {config_results.get('n_subsamples')}"
            )
            report_lines.append(
                f"Subsample size: {config_results.get('subsample_size')}"
            )

            report_lines.append('\nImportance scores:')
            for param, score in config_results.get(
                'sorted_importance', {}
            ).items():
                report_lines.append(f'- {param}: {score:.4f}')

            # Add performance data for most important parameter
            if config_results.get('sorted_importance') and config_results.get(
                'performance_data'
            ):
                top_param = next(
                    iter(config_results.get('sorted_importance', {})), None
                )
                if top_param:
                    report_lines.append(
                        f'\nPerformance for different values of {top_param}:'
                    )
                    perf_data = config_results.get('performance_data', {}).get(
                        top_param, {}
                    )
