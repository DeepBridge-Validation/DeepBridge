...
class DBDataset:
    ...
    def __init__(
        ...
        # Validate input data
        self._validator.validate_data_input(
            data, train_data, test_data, target_column
        )

        # Validate that only one of model_path, model, or prob_cols is provided
        params_provided = sum(
            param is not None for param in [model_path, model, prob_cols]
        )
        if params_provided > 1:
            raise ValueError(
                "You must provide only one of 'model_path', 'model', or 'prob_cols' parameters. "
        ...
        if data is not None:
            self._process_unified_data(
                data, target_column, features, prob_cols, test_size
            )
        else:
            self._process_split_data(
                train_data, test_data, target_column, features, prob_cols
            )
        ...
        if model_path is not None:
            ...
            if self._train_data is not None or self._test_data is not None:
                try:
                    ...
                    print(
                        f'Warning: Could not generate predictions using the provided model: {str(e)}'
                    )
    ...
    def _process_unified_data(
        ...
        if not isinstance(data, pd.DataFrame):
            ...
            try:
                ...
                raise ValueError(
                    f'Could not convert input data to DataFrame: {str(e)}'
                )
        ...
        self._features = self._validator.validate_features(
            features, data, target_column, prob_cols
        )
    ...
    def _process_split_data(
        ...
        if not isinstance(train_data, pd.DataFrame):
            try:
                # Handle scikit-learn Bunch object
                if hasattr(train_data, 'data') and hasattr(
                    train_data, 'target'
                ):
                    ...
                    # Create DataFrame from data and target
                    train_feature_data = pd.DataFrame(
                        train_data.data, columns=features
                    )
                    train_target_data = pd.Series(
                        train_data.target, name=target_column
                    )
                    train_data = pd.concat(
                        [train_feature_data, train_target_data], axis=1
                    )
                else:
                    # Try to convert any other object to DataFrame
            ...
            try:
                ...
                if hasattr(test_data, 'data') and hasattr(test_data, 'target'):
                    ...
                    test_feature_data = pd.DataFrame(
                        test_data.data, columns=features
                    )
                    test_target_data = pd.Series(
                        test_data.target, name=target_column
                    )
                    test_data = pd.concat(
                        [test_feature_data, test_target_data], axis=1
                    )
                else:
                    # Try to convert any other object to DataFrame
        ...
        for df, name in [(train_data, 'train'), (test_data, 'test')]:
            if target_column not in df.columns:
                raise ValueError(
                    f"Target column '{target_column}' not found in {name} data"
                )
    ...
    def _validate_categorical_features(
        ...
        if invalid_features:
            raise ValueError(
                f'Categorical features {invalid_features} not found in features list'
            )
    ...
    def get_feature_data(self, dataset: str = 'train') -> pd.DataFrame:
        ...
        data = (
            self._train_data if dataset.lower() == 'train' else self._test_data
        )
        ...
        if isinstance(model_or_path, (str, Path)):
            ...
            if self._train_data is not None or self._test_data is not None:
                try:
                    ...
                    print(
                        f'Warning: Could not generate predictions for the new model: {str(e)}'
                    )
