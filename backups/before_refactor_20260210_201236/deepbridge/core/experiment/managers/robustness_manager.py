...
class RobustnessManager(BaseManager):
    ...
    def run_tests(self, config_name='quick', **kwargs) -> dict:
        ...
        """
        from deepbridge.validation.wrappers.robustness_suite import (
            RobustnessSuite,
        )
        ...
        self.log(
            f'Running robustness tests with {n_iterations} iterations per perturbation level...'
        )

        # Initialize results storage
        results = {'main_model': {}, 'alternative_models': {}}
    ...
    def compare_models_robustness(self, robustness_results) -> dict:
        ...
        if 'main_model' in robustness_results:
            ...
            if 'robustness_scores' in robustness_results['main_model']:
                main_score = robustness_results['main_model'][
                    'robustness_scores'
                ].get('overall_score', 0)
        ...
        if 'alternative_models' in robustness_results:
            for model_name, model_results in robustness_results[
                'alternative_models'
            ].items():
                model_score = None
                # Try different possible paths to find the robustness score
                if 'robustness_scores' in model_results:
                    model_score = model_results['robustness_scores'].get(
                        'overall_score', 0
                    )
