...
class StaticUncertaintyRenderer:
        ...
        try:
            ...
            if not template_path:
                raise FileNotFoundError(
                    f'No suitable template found for uncertainty report'
                )
            ...
            logger.info(
                'Starting data transformation with standard transformer'
            )
            report_data = self.data_transformer.transform(results, model_name)
            logger.info(
                f'Standard transformer produced data with keys: {list(report_data.keys() if isinstance(report_data, dict) else [])}'
            )

            # Then apply additional transformations for static reports
            report_data = self.static_transformer.transform(
                results, model_name
            )
            logger.info(
                f'Static transformer produced data with keys: {list(report_data.keys() if isinstance(report_data, dict) else [])}'
            )

            # Transform initial results data if available
            if 'initial_results' in results:
                logger.info(
                    'Found initial_results in results, transforming...'
                )
                initial_results = self.initial_results_transformer.transform(
                    results.get('initial_results', {})
                )
                report_data['initial_results'] = initial_results

            # Create the context for the template
            context = self.base_renderer._create_static_context(
                report_data, 'uncertainty', css_content
            )
            ...
            try:
                ...
                try:
                    ...
                    if (
                        'marginal_bandwidth' in charts
                        and 'interval_widths_comparison' not in charts
                    ):
                        logger.info(
                            'Adding marginal_bandwidth → interval_widths_comparison mapping'
                        )
                        charts['interval_widths_comparison'] = charts[
                            'marginal_bandwidth'
                        ]
                    if (
                        'interval_widths_comparison' in charts
                        and 'marginal_bandwidth' not in charts
                    ):
                        logger.info(
                            'Adding interval_widths_comparison → marginal_bandwidth mapping'
                        )
                        charts['marginal_bandwidth'] = charts[
                            'interval_widths_comparison'
                        ]

                context['charts'] = charts
                logger.info(
                    f'Generated {len(charts)} charts: {list(charts.keys())}'
                )
            ...
            (
                feature_subset,
                feature_subset_display,
            ) = self._extract_feature_subset(report_data)

            # Add uncertainty-specific context with default values for None
            context.update(
                {
                    # Core metrics
                    'uncertainty_score': report_data.get(
                        'uncertainty_score', 0
                    ),
                    'coverage': report_data.get('coverage', 0),
                    'mean_width': report_data.get('mean_width', 0),
                    # Feature importance data
                    'feature_importance': report_data.get(
                        'feature_importance', {}
                    ),
                    'model_feature_importance': report_data.get(
                        'model_feature_importance', {}
                    ),
                    'has_feature_importance': bool(
                        report_data.get('feature_importance')
                    ),
                    'has_model_feature_importance': bool(
                        report_data.get('model_feature_importance')
                    ),
                    # Test metadata
            ...
            if 'alternative_models' in report_data and isinstance(
                report_data['alternative_models'], dict
            ):
                for model_name, model_data in report_data[
                    'alternative_models'
                ].items():
            ...
            if 'charts' in context and context['charts']:
                ...
                for old_name, new_name in chart_name_mapping.items():
                    if old_name in context['charts']:
                        ...
                        logger.info(
                            f"Mapped chart '{old_name}' to '{new_name}' for template compatibility"
                        )

                        # Also add the original chart directly if it's not already present
                        if old_name not in mapped_charts:
                            mapped_charts[old_name] = context['charts'][
                                old_name
                            ]
                ...
                for old_name, new_name in chart_name_mapping.items():
                    ...
                    if (
                        new_name in context['charts']
                        and old_name not in mapped_charts
                    ):
                        mapped_charts[old_name] = context['charts'][new_name]
                        logger.info(
                            f"Created reverse mapping from '{new_name}' to '{old_name}'"
                        )
                ...
                template_expected_charts = [
                    ...
                    'distance_metrics_comparison',
                    'feature_distance_heatmap',
                    'model_resilience_scores',
                    'interval_widths_boxplot',
                ]

                # Log which expected charts are missing and check for mismatches between naming conventions
                missing_charts = [
                    chart
                    for chart in template_expected_charts
                    if chart not in mapped_charts
                ]
                if missing_charts:
                    logger.info(
                        f'Missing charts needed by template: {missing_charts}'
                    )
                ...
                for chart_name in template_expected_charts:
                    logger.info(
                        f"Chart '{chart_name}': {'Available' if chart_name in mapped_charts else 'MISSING'}"
                    )
                logger.info('===================================')

                # Special case for marginal_bandwidth / interval_widths_comparison
                if (
                    'marginal_bandwidth' in context['charts']
                    and 'interval_widths_comparison' not in mapped_charts
                ):
                    logger.info(
                        "Found 'marginal_bandwidth' but not mapped to 'interval_widths_comparison'. Adding mapping."
                    )
                    mapped_charts['interval_widths_comparison'] = context[
                        'charts'
                    ]['marginal_bandwidth']
                elif (
                    'interval_widths_comparison' in context['charts']
                    and 'marginal_bandwidth' not in mapped_charts
                ):
                    logger.info(
                        "Found 'interval_widths_comparison' but not mapped to 'marginal_bandwidth'. Adding mapping."
                    )
                    mapped_charts['marginal_bandwidth'] = context['charts'][
                        'interval_widths_comparison'
                    ]
                ...
                try:
                    ...
                    chart_references = re.findall(
                        r'{{[\s]*charts\.([\w\d_]+)[\s]*}}', template_content
                    )

                    # Find all {% if charts.NAME %} patterns in the template
                    chart_conditions = re.findall(
                        r'{%[\s]*if[\s]*charts\.([\w\d_]+)[\s]*%}',
                        template_content,
                    )

                    # Combine and deduplicate
                    all_chart_refs = list(
                        set(chart_references + chart_conditions)
                    )

                    logger.info(
                        f'Found {len(all_chart_refs)} chart references in the template: {all_chart_refs}'
                    )
                    ...
                    missing_template_refs = [
                        ...
                        if ref not in mapped_charts
                    ]
                    if missing_template_refs:
                        logger.warning(
                            f'Template contains references to charts that were not generated: {missing_template_refs}'
                        )

                    # Check which generated charts are not referenced in the template
                    unused_charts = [
                        chart
                        for chart in mapped_charts.keys()
                        if chart not in all_chart_refs
                    ]
                    if unused_charts:
                        logger.info(
                            f'Generated charts not referenced in the template: {unused_charts}'
                        )
                except Exception as e:
                    logger.error(
                        f'Error analyzing template chart references: {str(e)}'
                    )
            else:
            ...
            report_path = self.base_renderer._write_report(
                rendered_html, file_path
            )
    ...
    def _extract_feature_list(self, report_data: Dict[str, Any]) -> List[str]:
        ...
        if 'features' in report_data and isinstance(
            report_data['features'], list
        ):
            return report_data['features']

        # If features not found but feature_importance is available, use those keys
        elif 'feature_importance' in report_data and isinstance(
            report_data['feature_importance'], dict
        ):
    ...
    def _generate_placeholder_chart(
        self, title, output_path, chart_type='scatter'
    ):
        """
        ...
        try:
            ...
            plt.figtext(
                0.5,
                0.01,
                'Example data - Generated for demonstration purposes',
                ha='center',
                fontsize=8,
                color='gray',
            )
    ...
    def _save_base64_to_file(
        ...
        try:
            ...
            if base64_data.startswith('data:image/png;base64,'):
                ...
                logger.error(
                    f'Invalid base64 image format for chart {chart_name}'
                )
                return base64_data
        except Exception as e:
    ...
    def _generate_charts(
        self, report_data: Dict[str, Any], save_chart: bool = True
    ) -> Dict[str, str]:
        """
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            f'Error importing module {module_name}: {str(module_error)}'
                        )

                    # Now try to import and instantiate the chart class
                    from deepbridge.templates.report_types.uncertainty.static.charts.enhanced_charts import (
                        EnhancedUncertaintyCharts,
                    )

                    enhanced_charts = EnhancedUncertaintyCharts(
                    ...
                    available_methods = [
                        ...
                        if hasattr(enhanced_charts, method)
                    ]
                    logger.info(
                        f'Successfully loaded enhanced uncertainty charts with methods: {available_methods}'
                    )

                    if set(available_methods) != set(expected_methods):
                        missing_methods = set(expected_methods) - set(
                            available_methods
                        )
                        logger.warning(
                            f'Enhanced charts is missing expected methods: {missing_methods}'
                        )
                except ImportError as e:
                    enhanced_charts = None
                    logger.error(
                        f'Enhanced uncertainty charts not available: {str(e)}'
                    )
                except Exception as e:
                    enhanced_charts = None
                    logger.error(
                        f'Error loading enhanced uncertainty charts: {str(e)}'
                    )
                    import traceback
                ...
                chart_generator = UncertaintyChartGenerator(
                    self.chart_generator
                )
                logger.info(
                    'Using new modular chart generator system for uncertainty visualization'
                )
                ...
                try:
                    ...
                    if 'calibration_results' in report_data:
                        logger.info(
                            f"  - calibration_results disponível: {report_data['calibration_results'].keys() if isinstance(report_data['calibration_results'], dict) else 'não é um dicionário'}"
                        )
                        if isinstance(
                            report_data['calibration_results'], dict
                        ):
                            for key, value in report_data[
                                ...
                                if isinstance(value, (list, tuple)):
                                    logger.info(
                                        f'  - {key}: {len(value)} valores'
                                    )
                                else:
                                    logger.info(f'  - {key}: {type(value)}')
                    else:
                        logger.error(
                            '  - ERRO: calibration_results não está disponível nos dados'
                        )

                    # Check alpha values
                    if 'alpha_levels' in report_data:
                        logger.info(
                            f"  - alpha_levels: {report_data['alpha_levels']}"
                        )
                    else:
                        logger.error(
                            '  - ERRO: alpha_levels não está disponível nos dados'
                        )
                    ...
                    try:
                        ...
                        if (
                            'calibration_results' in report_data
                            and isinstance(
                                report_data['calibration_results'], dict
                            )
                            and 'alpha_values'
                            in report_data['calibration_results']
                            and 'coverage_values'
                            in report_data['calibration_results']
                            and 'expected_coverages'
                            in report_data['calibration_results']
                        ):

                            # Log data for debugging
                            alpha_values = report_data['calibration_results'][
                                'alpha_values'
                            ]
                            coverage_values = report_data[
                                'calibration_results'
                            ]['coverage_values']
                            expected_coverages = report_data[
                                'calibration_results'
                            ]['expected_coverages']

                            logger.info(
                                f"Alpha values: {alpha_values} (type: {type(alpha_values)}, len: {len(alpha_values) if hasattr(alpha_values, '__len__') else 'NA'})"
                            )
                            logger.info(
                                f"Coverage values: {coverage_values} (type: {type(coverage_values)}, len: {len(coverage_values) if hasattr(coverage_values, '__len__') else 'NA'})"
                            )
                            logger.info(
                                f"Expected coverages: {expected_coverages} (type: {type(expected_coverages)}, len: {len(expected_coverages) if hasattr(expected_coverages, '__len__') else 'NA'})"
                            )

                            # Make sure data is lists, not numpy arrays
                            if hasattr(alpha_values, 'tolist') and callable(
                                getattr(alpha_values, 'tolist')
                            ):
                                report_data['calibration_results'][
                                    'alpha_values'
                                ] = alpha_values.tolist()
                            if hasattr(coverage_values, 'tolist') and callable(
                                getattr(coverage_values, 'tolist')
                            ):
                                report_data['calibration_results'][
                                    'coverage_values'
                                ] = coverage_values.tolist()
                            if hasattr(
                                expected_coverages, 'tolist'
                            ) and callable(
                                getattr(expected_coverages, 'tolist')
                            ):
                                report_data['calibration_results'][
                                    'expected_coverages'
                                ] = expected_coverages.tolist()
                            ...
                            # Now generate chart
                            coverage_chart = (
                                chart_generator.generate_coverage_vs_expected(
                                    report_data
                                )
                            )
                            if coverage_chart:
                                ...
                                logger.info(
                                    'Generated coverage vs expected coverage chart'
                                )
                            ...
                            logger.warning(
                                'Missing required data for coverage vs expected chart'
                            )
                    except Exception as e:
                        logger.error(
                            f'Error in coverage chart generation: {str(e)}',
                            exc_info=True,
                        )
                except Exception as e:
                    logger.error(
                        f'Error generating coverage vs expected chart: {str(e)}'
                    )
                ...
                try:
                    ...
                    if 'calibration_results' in report_data:
                        ...
                        if isinstance(
                            ...
                            if (
                                ...
                                logger.info(
                                    f"  - width_values: {len(report_data['calibration_results']['width_values'])} valores"
                                )
                            else:
                                logger.error(
                                    '  - ERRO: width_values não está disponível em calibration_results'
                                )
                            ...
                            if (
                                ...
                                logger.info(
                                    f"  - coverage_values: {len(report_data['calibration_results']['coverage_values'])} valores"
                                )
                            else:
                                logger.error(
                                    '  - ERRO: coverage_values não está disponível em calibration_results'
                                )
                    else:
                        logger.error(
                            '  - ERRO: calibration_results não está disponível nos dados'
                    ...
                    try:
                        ...
                        if (
                            ...
                            # Log data for debugging
                            width_values = report_data['calibration_results'][
                                'width_values'
                            ]
                            coverage_values = report_data[
                                'calibration_results'
                            ...
                            logger.info(
                                f"Width values: {width_values} (type: {type(width_values)}, len: {len(width_values) if hasattr(width_values, '__len__') else 'NA'})"
                            )
                            logger.info(
                                f"Coverage values: {coverage_values} (type: {type(coverage_values)}, len: {len(coverage_values) if hasattr(coverage_values, '__len__') else 'NA'})"
                            )
                            ...
                            # Make sure data is lists, not numpy arrays
                            if hasattr(width_values, 'tolist') and callable(
                                getattr(width_values, 'tolist')
                            ):
                                report_data['calibration_results'][
                                    'width_values'
                                ] = width_values.tolist()
                            ...
                            # Now generate chart
                            width_chart = (
                                chart_generator.generate_width_vs_coverage(
                                    report_data
                                )
                            )
                            if width_chart:
                                ...
                                logger.info(
                                    'Generated width vs coverage chart'
                                )
                            ...
                            logger.warning(
                                'Missing required data for width vs coverage chart'
                            )
                    except Exception as e:
                        logger.error(
                            f'Error in width vs coverage chart generation: {str(e)}',
                            exc_info=True,
                        )
                except Exception as e:
                    logger.error(
                        f'Error generating width vs coverage chart: {str(e)}'
                    )
                ...
                try:
                    ...
                    if 'uncertainty_score' in report_data:
                        logger.info(
                            f"  - uncertainty_score: {report_data['uncertainty_score']}"
                        )
                    else:
                        logger.error(
                            '  - ERRO: uncertainty_score não está disponível nos dados'
                        )
                    ...
                    if 'coverage' in report_data:
                        ...
                        logger.error(
                            '  - ERRO: coverage não está disponível nos dados'
                        )

                    if 'mean_width' in report_data:
                        logger.info(
                            f"  - mean_width: {report_data['mean_width']}"
                        )
                    else:
                        logger.error(
                            '  - ERRO: mean_width não está disponível nos dados'
                        )

                    # Check metrics dictionary
                    if 'metrics' in report_data:
                        logger.info(
                            f"  - metrics disponível: {list(report_data['metrics'].keys()) if isinstance(report_data['metrics'], dict) else 'não é um dicionário'}"
                        )
                    else:
                        logger.error(
                            '  - ERRO: metrics não está disponível nos dados'
                        )
                    ...
                    if self.uncertainty_chart_generator:
                        metrics_chart = self.uncertainty_chart_generator.generate_uncertainty_metrics(
                            report_data, title='Uncertainty Metrics'
                        )
                    else:
                        logger.warning(
                            'UncertaintyChartGenerator not available, skipping uncertainty_metrics chart'
                        )
                        metrics_chart = None
                    if metrics_chart:
                    ...
                    logger.error(
                        f'Error generating uncertainty metrics chart: {str(e)}'
                    )
                ...
                if 'feature_importance' in report_data:
                    ...
                    logger.info(
                        f"  - feature_importance type: {type(report_data['feature_importance'])}"
                    )
                    if isinstance(report_data['feature_importance'], dict):
                        logger.info(
                            f"  - feature_importance tem {len(report_data['feature_importance'])} características"
                        )
                        # Log some features as examples
                        for i, (feature, importance) in enumerate(
                            report_data['feature_importance'].items()
                        ):
                            if i < 5:  # Show first 5 features only
                                logger.info(
                                    f'  - Exemplo: {feature}: {importance}'
                                )
                            else:
                                break
                    else:
                        logger.error(
                            f"  - ERRO: feature_importance não é um dicionário: {type(report_data['feature_importance'])}"
                        )
                    ...
                    try:
                        ...
                        if self.uncertainty_chart_generator:
                            ...
                            importance_chart = (
                                chart_generator.generate_feature_importance(
                                    report_data
                                )
                            )
                        if importance_chart:
                        ...
                        logger.error(
                            f'Error generating feature importance chart: {str(e)}'
                        )
                else:
                    logger.error(
                        '  - ERRO: feature_importance não está disponível nos dados'
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        if 'primary_model' in report_data and isinstance(
                            report_data['primary_model'], dict
                        ):
                    ...
                    try:
                        ...
                        if (
                            y_prob is None
                            and 'model' in report_data
                            and 'X_test' in report_data
                        ):
                            ...
                            if hasattr(model, 'predict_proba'):
                                ...
                                logger.info(
                                    f'  - Generated predictions from model.predict_proba'
                                )
                            elif hasattr(model, 'predict'):
                                y_prob = model.predict(X_test)
                                logger.info(
                                    f'  - Generated predictions from model.predict'
                                )
                    except Exception as e:
                        logger.warning(
                    ...
                    if y_prob is None and 'test_predictions' in report_data:
                        ...
                        logger.info(
                            f'  - Found test_predictions in report_data'
                        )
                    ...
                    if y_true is not None and y_prob is not None:
                        ...
                        logger.info(
                            f'  - Primary model data prepared: y_true shape {y_true.shape}, y_prob shape {y_prob.shape}'
                        )
                    else:
                        logger.warning(
                            f'  - Missing data: y_true is None: {y_true is None}, y_prob is None: {y_prob is None}'
                        )
                    ...
                    if 'alternative_models' in report_data and isinstance(
                        ...
                        for model_name, model_data in report_data[
                            ...
                            if (
                                'y_true' in model_data
                                and 'y_prob' in model_data
                            ):
                                ...
                                logger.info(
                                    f'  - Alternative model {model_name} has reliability data'
                                )

                    if reliability_data:
                        reliability_bandwidth_chart = (
                            chart_generator.generate_reliability_bandwidth(
                                reliability_data,
                                n_bins=10,
                                confidence=0.95,
                        ...
                        if reliability_bandwidth_chart:
                            ...
                            logger.info(
                                'Generated reliability bandwidth chart'
                            )
                        ...
                        logger.warning(
                            'No data available for reliability bandwidth chart'
                        )

                except Exception as e:
                    logger.error(
                        f'Error generating reliability bandwidth chart: {str(e)}'
                    )
                ...
                try:
                    ...
                    if 'reliability_data' in locals() and reliability_data:
                        reliability_perf_chart = (
                            chart_generator.generate_reliability_performance(
                                reliability_data,
                                n_bins=10,
                                metric='accuracy',
                        ...
                        if reliability_perf_chart:
                            ...
                            logger.info(
                                'Generated reliability performance chart'
                            )
                        ...
                        logger.warning(
                            'No data available for reliability performance chart'
                        )

                except Exception as e:
                    logger.error(
                        f'Error generating reliability performance chart: {str(e)}'
                    )
                ...
                try:
                    ...
                    if 'alternative_models' in report_data:
                        ...
                        logger.info(
                            f"  - alternative_models: {len(alt_models) if isinstance(alt_models, dict) else 'não é um dicionário'}"
                        )

                        if isinstance(alt_models, dict):
                            # Log model names
                            logger.info(
                                f'  - Modelos alternativos: {list(alt_models.keys())}'
                            )
                            ...
                            for model_name, model_data in alt_models.items():
                                ...
                                if isinstance(model_data, dict):
                                    if 'uncertainty_score' in model_data:
                                        logger.info(
                                            f"    - uncertainty_score: {model_data['uncertainty_score']}"
                                        )
                                    else:
                                        logger.error(
                                            f'    - ERRO: uncertainty_score não disponível para {model_name}'
                                        )

                                    if 'coverage' in model_data:
                                        logger.info(
                                            f"    - coverage: {model_data['coverage']}"
                                        )
                                    else:
                                        logger.error(
                                            f'    - ERRO: coverage não disponível para {model_name}'
                                        )

                                    if 'mean_width' in model_data:
                                        logger.info(
                                            f"    - mean_width: {model_data['mean_width']}"
                                        )
                                    else:
                                        logger.error(
                                            f'    - ERRO: mean_width não disponível para {model_name}'
                                        )
                                else:
                                    logger.error(
                                        f'    - ERRO: dados do modelo não são um dicionário: {type(model_data)}'
                                    )
                        else:
                            logger.error(
                                f'  - ERRO: alternative_models não é um dicionário: {type(alt_models)}'
                            )
                    else:
                        logger.error(
                            '  - ERRO: alternative_models não está disponível nos dados'
                        )

                    comparison_chart = (
                        chart_generator.generate_model_comparison(report_data)
                    )
                    if comparison_chart:
                    ...
                    logger.error(
                        f'Error generating model comparison chart: {str(e)}'
                    )
                ...
                try:
                    ...
                    if 'calibration_results' in report_data:
                        ...
                        logger.info(
                            f"  - calibration_results disponível: {cal_results.keys() if isinstance(cal_results, dict) else 'não é um dicionário'}"
                        )

                        # Check for alpha and coverage arrays
                        if isinstance(cal_results, dict):
                            if (
                                'alpha_values' in cal_results
                                and 'coverage_values' in cal_results
                            ):
                                logger.info(
                                    f"  - alpha_values: {len(cal_results['alpha_values'])} valores"
                                )
                                logger.info(
                                    f"  - coverage_values: {len(cal_results['coverage_values'])} valores"
                                )
                                logger.info(
                                    f"  - expected_coverages: {len(cal_results.get('expected_coverages', []))} valores"
                                )
                            else:
                                logger.error(
                                    '  - ERRO: alpha_values, coverage_values ou expected_coverages ausentes em calibration_results'
                                )
                    else:
                        logger.error(
                            '  - ERRO: calibration_results não está disponível nos dados'
                    ...
                    try:
                        ...
                        sig = inspect.signature(
                            UncertaintyChartGenerator.generate_performance_gap_by_alpha
                        )
                        logger.info(
                            f'  - Argumentos necessários para generate_performance_gap_by_alpha: {list(sig.parameters.keys())}'
                        )
                    except Exception as import_err:
                        logger.error(
                            f'  - Não foi possível inspecionar a assinatura do método: {str(import_err)}'
                        )

                    performance_gap_chart = (
                        chart_generator.generate_performance_gap_by_alpha(
                            models_data=report_data,
                    ...
                    logger.error(
                        f'Error generating performance gap by alpha chart: {str(e)}'
                    )
                ...
                if enhanced_charts:
                    ...
                    try:
                        ...
                        if 'reliability_analysis' in report_data:
                            logger.info(
                                'Attempting to generate reliability distribution chart'
                            )
                            logger.info(
                                f"reliability_analysis keys: {list(report_data['reliability_analysis'].keys())}"
                            )
                            ...
                            for key, value in report_data[
                                ...
                                if isinstance(value, dict):
                                    logger.info(
                                        f'  - {key} is a dictionary with keys: {list(value.keys())}'
                                    )
                                elif isinstance(value, list):
                                    logger.info(
                                        f'  - {key} is a list with {len(value)} items'
                                    )
                                    if (
                                        value and len(value) < 10
                                    ):  # Only log small lists
                            ...
                            if (
                                ...
                                in report_data['reliability_analysis']
                            ):
                                logger.info(
                                    f"feature_distributions available with types: {list(report_data['reliability_analysis']['feature_distributions'].keys())}"
                                )

                                for dist_type, features in report_data[
                                    'reliability_analysis'
                                ]['feature_distributions'].items():
                                    logger.info(
                                        f'Type {dist_type} has features: {list(features.keys())}'
                                    )
                                    ...
                                    if features:
                                        ...
                                        if isinstance(feature_data, dict):
                                            logger.info(
                                                f"Sample data for feature '{first_feature}': {list(feature_data.keys())}"
                                            )
                                        else:
                                            logger.info(
                                                f"Sample data for feature '{first_feature}' is type: {type(feature_data)}"
                                            )
                            else:
                                logger.warning(
                                    'feature_distributions not available in reliability_analysis'
                                )

                            # Get top feature from feature importance
                            top_feature = None
                            if (
                                'feature_importance' in report_data
                                and report_data['feature_importance']
                            ):
                                ...
                                logger.info(
                                    f'Selected top feature: {top_feature}'
                                )
                            else:
                                logger.warning(
                                    'No feature_importance available to select top feature'
                                )

                            # Ensure enhanced_charts exists and has the required method
                            if enhanced_charts and hasattr(
                                enhanced_charts,
                                'generate_reliability_distribution',
                            ):
                                logger.info(
                                    'Calling generate_reliability_distribution'
                                )
                                reliability_chart = enhanced_charts.generate_reliability_distribution(
                                    report_data, top_feature
                                )
                                ...
                                if reliability_chart:
                                    ...
                                    logger.info(
                                        'Successfully generated reliability distribution chart'
                                    )
                                    ...
                                    logger.warning(
                                        'generate_reliability_distribution returned None'
                                    )
                            else:
                                logger.error(
                                    'Enhanced charts not available or missing generate_reliability_distribution method'
                                )
                        else:
                            logger.warning(
                                'reliability_analysis not available in report_data'
                            )
                    except Exception as e:
                        logger.error(
                            f'Error generating reliability distribution chart: {str(e)}'
                        )
                        import traceback
                    ...
                    try:
                        ...
                        if 'marginal_bandwidth' in report_data:
                            logger.info(
                                'Attempting to generate marginal bandwidth chart'
                            )
                            logger.info(
                                f"marginal_bandwidth type: {type(report_data['marginal_bandwidth'])}"
                            )

                            # Verify data structure and log more details
                            if isinstance(
                                report_data['marginal_bandwidth'], dict
                            ):
                                logger.info(
                                    f"marginal_bandwidth keys: {list(report_data['marginal_bandwidth'].keys())}"
                                )

                                # Check if feature_impacts is directly in marginal_bandwidth
                                if (
                                    'feature_impacts'
                                    in report_data['marginal_bandwidth']
                                ):
                                    feature_impacts = report_data[
                                        'marginal_bandwidth'
                                    ]['feature_impacts']
                                    logger.info(
                                        f'feature_impacts found directly in marginal_bandwidth: {type(feature_impacts)}'
                                    )

                                    if isinstance(feature_impacts, dict):
                                        logger.info(
                                            f'Features in feature_impacts: {list(feature_impacts.keys())}'
                                        )
                                        ...
                                        for feature in sample_features:
                                            logger.info(
                                                f'  - {feature}: {feature_impacts[feature]}'
                                            )
                                ...
                                if report_data['marginal_bandwidth']:
                                    ...
                                    keys = [
                                        ...
                                        if k != 'feature_impacts'
                                    ]
                                    if keys:
                                        top_feature = keys[0]
                                        logger.info(
                                            f'Selected feature for bandwidth chart: {top_feature}'
                                        )

                                        # Log the structure of data for this feature
                                        feature_data = report_data[
                                            'marginal_bandwidth'
                                        ][top_feature]
                                        if isinstance(feature_data, dict):
                                            logger.info(
                                                f'Feature data keys: {list(feature_data.keys())}'
                                            )
                                            for (
                                                key,
                                                value,
                                                ...
                                                if isinstance(
                                                    ...
                                                    if (
                                                        len(value) < 5
                                                    ):  # Only show values for small arrays
                                                        logger.info(
                                                            f'    Values: {value}'
                                                        )
                                                else:
                                            ...
                                            logger.info(
                                                f'Feature data is not a dictionary but a {type(feature_data)}'
                                            )
                                            if isinstance(
                                                feature_data,
                                                (int, float, str, bool),
                                                ...
                                                logger.info(
                                                    f'Value: {feature_data}'
                                                )
                                    else:
                                        logger.warning(
                                            'marginal_bandwidth has no feature keys'
                                        )
                                else:
                                    logger.warning(
                                        'marginal_bandwidth dictionary is empty'
                                    )
                            else:
                                logger.warning(
                                    f"marginal_bandwidth is not a dictionary but a {type(report_data['marginal_bandwidth'])}"
                                )
                            ...
                            if enhanced_charts and hasattr(
                                ...
                                logger.info(
                                    'Calling generate_marginal_bandwidth_chart'
                                )
                                bandwidth_chart = enhanced_charts.generate_marginal_bandwidth_chart(
                                    report_data, top_feature
                                )
                                ...
                                if bandwidth_chart:
                                    ...
                                    logger.info(
                                        'Successfully generated marginal bandwidth chart'
                                    )
                                    ...
                                    logger.warning(
                                        'generate_marginal_bandwidth_chart returned None'
                                    )
                            else:
                                logger.error(
                                    'Enhanced charts not available or missing generate_marginal_bandwidth_chart method'
                                )
                        else:
                            logger.warning(
                                'marginal_bandwidth not available in report_data'
                            )
                    except Exception as e:
                        logger.error(
                            f'Error generating marginal bandwidth chart: {str(e)}'
                        )
                        import traceback
                    ...
                    try:
                        ...
                        if 'interval_widths' in report_data:
                            logger.info(
                                'Attempting to generate interval widths boxplot'
                            )

                            # Log information about the interval_widths data
                            if isinstance(
                                report_data['interval_widths'], list
                            ):
                                logger.info(
                                    f"interval_widths is a list with {len(report_data['interval_widths'])} elements"
                                )
                                if report_data[
                                    'interval_widths'
                                ] and isinstance(
                                    report_data['interval_widths'][0], list
                                ):
                                    logger.info(
                                        f"First element is a list with {len(report_data['interval_widths'][0])} values"
                                    )
                            elif isinstance(
                                report_data['interval_widths'], dict
                            ):
                                logger.info(
                                    f"interval_widths is a dictionary with keys: {list(report_data['interval_widths'].keys())}"
                                )
                                for model, widths in report_data[
                                    'interval_widths'
                                ].items():
                                    if isinstance(widths, list):
                                        logger.info(
                                            f'  - {model}: {len(widths)} values'
                                        )
                            else:
                                logger.warning(
                                    f"interval_widths has unexpected type: {type(report_data['interval_widths'])}"
                                )
                            ...
                            if enhanced_charts and hasattr(
                                ...
                                logger.info(
                                    'Calling generate_interval_widths_boxplot'
                                )
                                interval_chart = enhanced_charts.generate_interval_widths_boxplot(
                                    report_data
                                )
                                ...
                                if interval_chart:
                                    ...
                                    logger.info(
                                        'Successfully generated interval widths boxplot'
                                    )
                                    ...
                                    logger.warning(
                                        'generate_interval_widths_boxplot returned None'
                                    )
                            else:
                                logger.error(
                                    'Enhanced charts not available or missing generate_interval_widths_boxplot method'
                                )
                        else:
                            logger.warning(
                                'interval_widths not available in report_data'
                            )
                    except Exception as e:
                        logger.error(
                            f'Error generating interval widths boxplot: {str(e)}'
                        )
                        import traceback
                    ...
                    try:
                        ...
                        metrics_chart = (
                            enhanced_charts.generate_model_metrics_comparison(
                                report_data
                            )
                        )
                        if metrics_chart:
                        ...
                        logger.error(
                            f'Error generating model metrics comparison: {str(e)}'
                        )

            except ImportError as e:
                logger.warning(
                    f'Could not import modular chart generator: {str(e)}'
                )
                ...
                if 'interval_widths' in report_data:
                    try:
                        ...
                        from ...utils.uncertainty_charts import (
                            generate_interval_widths_comparison,
                        )
                        ...
                        # Generate the chart
                        chart_path = generate_interval_widths_comparison(
                            report_data, chart_dir
                        )
                        ...
                        if chart_path and os.path.exists(chart_path):
                            ...
                            if save_chart and charts_dir:
                                charts[
                                    'interval_widths_comparison'
                                ] = f'./{charts_subdir}/{chart_filename}'
                            else:
                            ...
                            logger.info(
                                f'Successfully generated interval widths comparison chart at {chart_path}'
                            )
                    except Exception as e:
                        logger.error(
                            f'Error generating interval widths chart: {str(e)}'
                        )

                # Generate feature reliability chart - legacy method
                if 'feature_reliability' in report_data:
                    try:
                        feature_data = {'Feature': [], 'PSI': []}

                        # Extract top features by PSI
                        sorted_features = sorted(
                            [
                                (feature, data.get('psi', 0))
                                for feature, data in report_data[
                                    'feature_reliability'
                                ].items()
                            ],
                            key=lambda x: x[1],
                        ...
                        if sorted_features:
                            ...
                            feature_data['PSI'] = [
                                score for _, score in sorted_features
                            ]

                            charts[
                                'feature_reliability'
                            ] = self.chart_generator.feature_psi_chart(
                        ...
                        logger.error(
                            f'Error generating feature reliability chart: {str(e)}'
                        )
                ...
                try:
                    ...
                    from ...utils.uncertainty_report_charts import (
                        generate_all_uncertainty_charts,
                    )
                    ...
                    logger.info(
                        'Generating static uncertainty charts from real data only'
                    )
                    additional_charts = generate_all_uncertainty_charts(
                        additional_charts_dir
                    )
                    ...
                    if additional_charts:
                        ...
                        if save_chart and charts_dir:
                            for (
                                chart_name,
                                chart_data,
                            ) in additional_charts.items():
                                if isinstance(
                                    chart_data, str
                                ) and chart_data.startswith(
                                    ...
                                    if not os.path.exists(chart_path):
                                        try:
                                            ...
                                            logger.info(
                                                f'Saved additional chart {chart_filename} to {charts_dir}'
                                            )
                                        except Exception as e:
                                            logger.error(
                                                f'Error saving additional chart {chart_name}: {str(e)}'
                                            )

                        charts.update(additional_charts)
                        logger.info(
                            f'Added {len(additional_charts)} additional uncertainty charts to {additional_charts_dir}'
                        )
                    else:
                    ...
                    logger.error(
                        f'Error generating additional uncertainty charts: {str(e)}'
                    )
                    logger.error(traceback.format_exc())
        ...
        try:
            ...
            try:
                ...
                if (
                    not boxplot_data['results']
                    and 'calibration_results' in report_data
                ):
                    calib = report_data['calibration_results']
                    if 'alpha_values' in calib and 'width_values' in calib:
                        logger.info(
                            'Using calibration_results for boxplot data'
                        )
                ...
                if (
                    not boxplot_data['results']
                    and 'results' in report_data
                    and isinstance(report_data['results'], list)
                ):
                ...
                if boxplot_data['results']:
                    interval_boxplot_chart = (
                        chart_generator.generate_interval_boxplot(boxplot_data)
                    )
                    if interval_boxplot_chart:
                        charts['interval_boxplot'] = interval_boxplot_chart
                        logger.info(
                            '✅ Successfully generated Interval Boxplot chart'
                        )
                        ...
                        logger.warning(
                            'Interval Boxplot chart generation returned None'
                        )
                else:
                    logger.warning(
                        'No data available for Interval Boxplot chart'
                    )

            except Exception as e:
                logger.error(
                    f'Error generating Interval Boxplot chart: {str(e)}'
                )
                logger.error(traceback.format_exc())
            ...
            try:
                ...
                if 'reliability_analysis' in report_data:
                    if 'psi_values' in report_data['reliability_analysis']:
                        psi_data = {
                            'reliability_analysis': report_data[
                                'reliability_analysis'
                            ]
                        }
                # Alternative: check in results
                    ...
                    if isinstance(report_data['results'], dict):
                        ...
                        isinstance(report_data['results'], list)
                        and report_data['results']
                    ):
                        # Check first result
                ...
                if psi_data:
                    ...
                    if psi_chart:
                        ...
                        logger.info(
                            '✅ Successfully generated PSI Analysis chart'
                        )
                        ...
                        logger.warning(
                            'PSI Analysis chart generation returned None'
                        )
                else:
                    logger.warning(
                        'No PSI data available for PSI Analysis chart'
                    )
            ...
            try:
                ...
                if 'reliability_analysis' in report_data:
                    if (
                        'psi_values' in report_data['reliability_analysis']
                        and 'feature_distributions'
                        in report_data['reliability_analysis']
                    ):
                        dist_data = {
                            'reliability_analysis': report_data[
                ...
                if dist_data:
                    dist_chart = (
                        chart_generator.generate_top_features_distribution(
                            dist_data, top_n=3
                        )
                    )
                    if dist_chart:
                        ...
                        logger.info(
                            '✅ Successfully generated Top Features Distribution chart'
                        )
                        ...
                        logger.warning(
                            'Top Features Distribution chart generation returned None'
                        )
                else:
                    logger.warning(
                        'No distribution data available for Top Features Distribution chart'
                    )

            except Exception as e:
                logger.error(
                    f'Error generating Top Features Distribution chart: {str(e)}'
                )
                logger.error(traceback.format_exc())

            logger.info('=' * 50)
            logger.info(
                f'Custom charts generation completed. Total charts: {len(charts)}'
            )
            logger.info('=' * 50)
