...
class DistillationDataTransformer:
    ...
    def transform(
        self, results: Dict[str, Any], config: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        transformed = {
            'experiment_summary': self._create_experiment_summary(
                results_df, experiment_config
            ),
            'original_model': self._transform_original_metrics(
                original_metrics
            ),
            ...
            'performance_comparison': self._create_performance_comparison(
                results_df, original_metrics
            ),
            'tradeoff_analysis': self._analyze_tradeoffs(
                results_df, original_metrics
            ),
            'frequency_distributions': self._collect_frequency_distributions(
                results_df, results.get('dataset')
            ),
            'ks_distributions': self._collect_ks_distributions(
                results_df, original_metrics
            ),
        ...
        logger.info(
            f'Transformation complete. Processed {len(results_df)} model configurations'
        )
        return transformed

    def _create_experiment_summary(
        self, results_df: pd.DataFrame, config: Dict
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            'model_types': list(results_df['model_type'].unique())
            if 'model_type' in results_df
            else [],
            'temperature_range': {
                'min': float(results_df['temperature'].min())
                if 'temperature' in results_df
                else 0,
                'max': float(results_df['temperature'].max())
                if 'temperature' in results_df
                else 0,
                'values': sorted(results_df['temperature'].unique().tolist())
                if 'temperature' in results_df
                else [],
            },
            'alpha_range': {
                'min': float(results_df['alpha'].min())
                if 'alpha' in results_df
                else 0,
                'max': float(results_df['alpha'].max())
                if 'alpha' in results_df
                else 0,
                'values': sorted(results_df['alpha'].unique().tolist())
                if 'alpha' in results_df
                else [],
            },
            ...
            'average_training_time': float(results_df['training_time'].mean())
            if 'training_time' in results_df
            else 0,
    ...
    def _transform_original_metrics(
        self, original_metrics: Dict
    ) -> Dict[str, Any]:
        ...
        if original_metrics:
            if 'train' in original_metrics:
                transformed['train_metrics'] = self._clean_metrics(
                    original_metrics['train']
                )
            if 'test' in original_metrics:
                transformed['test_metrics'] = self._clean_metrics(
                    original_metrics['test']
                )
            transformed['has_metrics'] = bool(
                transformed['train_metrics'] or transformed['test_metrics']
            )

        return transformed

    def _transform_best_model(
        self, best_model: Dict, results_df: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        if best_model:
            return {
                ...
                'model_complexity': float(
                    best_model.get('model_complexity', 0)
                ),
    ...
    def _extract_model_metrics(self, model_data: Any) -> Dict[str, float]:
        ...
        metric_keys = [
            ...
            'test_f1_score',
            'train_f1',
            'test_f1',  # Support both f1_score and f1 naming
            'train_auc_roc',
            'test_auc_roc',
            'train_auc_pr',
            'test_auc_pr',
    ...
    def _analyze_hyperparameters(
        ...
        if 'temperature' in results_df:
            ...
            analysis['temperature_impact'] = {
                ...
                'best': float(
                    results_df.loc[
                        results_df['test_accuracy'].idxmax(), 'temperature'
                    ]
                ),
            }
        ...
        if 'alpha' in results_df:
            ...
            analysis['alpha_impact'] = {
                ...
                'best': float(
                    results_df.loc[
                        results_df['test_accuracy'].idxmax(), 'alpha'
                    ]
                ),
            }
    ...
    def _create_performance_comparison(
        self, results_df: pd.DataFrame, original_metrics: Dict
    ) -> Dict[str, Any]:
        ...
        for metric in key_metrics:
            ...
            if test_metric in results_df:
                ...
                metric_data = {
                    ...
                    'improvement': float(
                        results_df[test_metric].max() - original_value
                    )
                    if original_value
                    else 0,
                }
        ...
        for metric in ranking_metrics:
            if metric in results_df:
                ...
                ranked = (
                    results_df.nsmallest(5, metric)
                    if ascending
                    else results_df.nlargest(5, metric)
                )
        ...
        if 'test_accuracy' in results_df and original_test.get('accuracy'):
            improved_models = results_df[
                results_df['test_accuracy'] > original_test['accuracy']
            ]
            comparison['improvement_stats'] = {
                'models_improved': len(improved_models),
                'models_total': len(results_df),
                'improvement_rate': len(improved_models) / len(results_df)
                if len(results_df) > 0
                else 0,
                'max_improvement': float(
                    (
                        results_df['test_accuracy'].max()
                        - original_test['accuracy']
                    )
                    * 100
                ),
                'avg_improvement': float(
                    (
                        improved_models['test_accuracy'].mean()
                        - original_test['accuracy']
                    )
                    * 100
                )
    ...
    def _analyze_tradeoffs(
        ...
        if 'model_complexity' in results_df and 'training_time' in results_df:
            ...
            for _, row in results_df.iterrows():
                ...
                norm_complexity = (
                    row['model_complexity'] / max_complexity
                    if max_complexity > 0
                    else 1
                )
                norm_time = (
                    row['training_time'] / max_time if max_time > 0 else 1
                )

                efficiency = row['test_accuracy'] / (
                    norm_complexity * norm_time + 0.001
    ...
    def _generate_recommendations(
            ...
            recommendations.append(
                {
                    ...
                    'metrics': {
                        ...
                        'f1_score': float(
                            best_accuracy.get('test_f1_score', 0)
                        ),
                        'training_time': float(
                            best_accuracy.get('training_time', 0)
                        ),
                        'complexity': float(
                            best_accuracy.get('model_complexity', 0)
                        ),
                    },
                }
            )
        ...
        if 'model_complexity' in results_df and 'test_accuracy' in results_df:
            results_df['efficiency'] = results_df['test_accuracy'] / (
                results_df['model_complexity'] + 0.001
            )
            ...
            recommendations.append(
                {
                    ...
                    'metrics': {
                        ...
                        'f1_score': float(
                            most_efficient.get('test_f1_score', 0)
                        ),
                        'training_time': float(
                            most_efficient.get('training_time', 0)
                        ),
                        'complexity': float(
                            most_efficient.get('model_complexity', 0)
                        ),
        ...
        if 'training_time' in results_df and 'test_accuracy' in results_df:
            ...
            good_models = results_df[
                results_df['test_accuracy'] >= median_accuracy
            ]
                ...
                recommendations.append(
                    {
                        ...
                        'metrics': {
                            ...
                            'complexity': float(
                                fastest.get('model_complexity', 0)
                            ),
                        },
                    }
                )
        ...
        if all(
            col in results_df
            for col in ['test_accuracy', 'test_f1_score', 'model_complexity']
        ):
            # Normalize metrics
            max_complexity = results_df['model_complexity'].max()
            ...
            results_df['production_score'] = (
                ...
                + 0.3
                * (
                    1 - results_df['model_complexity'] / max_complexity
                    if max_complexity > 0
                    else 0
                )
            )

            best_production = results_df.nlargest(1, 'production_score').iloc[
                0
            ]
            recommendations.append(
                {
                    ...
                    'metrics': {
                        ...
                        'f1_score': float(
                            best_production.get('test_f1_score', 0)
                        ),
                        'training_time': float(
                            best_production.get('training_time', 0)
                        ),
                        'complexity': float(
                            best_production.get('model_complexity', 0)
                        ),
                        'production_score': float(
                            best_production['production_score']
                        ),
                    },
                }
            )
        ...
        if original_metrics.get('test', {}).get('accuracy'):
            ...
            better_models = results_df[
                results_df['test_accuracy'] > original_acc
            ]
            ...
            if not better_models.empty:
                ...
                if 'model_complexity' in better_models:
                    simplest_better = better_models.nsmallest(
                        1, 'model_complexity'
                    ).iloc[0]
                    recommendations.append(
                        {
                            'scenario': 'Simplest Model Better Than Original',
                            'model': simplest_better.get(
                                'model_type', 'Unknown'
                            ),
                            ...
                            'metrics': {
                                'accuracy': float(
                                    simplest_better['test_accuracy']
                                ),
                                'f1_score': float(
                                    simplest_better.get('test_f1_score', 0)
                                ),
                                'training_time': float(
                                    simplest_better.get('training_time', 0)
                                ),
                                'complexity': float(
                                    simplest_better.get('model_complexity', 0)
                                ),
                                'improvement': float(
                                    simplest_better['test_accuracy']
                                    - original_acc
    ...
    def _identify_available_metrics(
        self, results_df: pd.DataFrame
    ) -> List[str]:
        ...
        for metric in possible_metrics:
            if (
                f'test_{metric}' in results_df
                or f'train_{metric}' in results_df
            ):
                metrics.append(metric)

        return metrics

    def _collect_frequency_distributions(
        self, results_df: pd.DataFrame, dataset=None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if dataset is not None:
                ...
                if (
                    hasattr(dataset, 'test_predictions')
                    and dataset.test_predictions is not None
                ):
                    test_preds = dataset.test_predictions
                    logger.info(
                        f'Found test_predictions with shape: {test_preds.shape}'
                    )
                    ...
                    if (
                        'proba0' in test_preds.columns
                        and 'proba1' in test_preds.columns
                    ):
                        confidence = np.maximum(
                            test_preds['proba0'].values,
                            test_preds['proba1'].values,
                        )
                ...
                if (
                    hasattr(dataset, 'train_predictions')
                    and dataset.train_predictions is not None
                ):
                    train_preds = dataset.train_predictions
                    logger.info(
                        f'Found train_predictions with shape: {train_preds.shape}'
                    )
                    ...
                    try:
                        ...
                        if X_test is not None and len(X_test) > 0:
                            ...
                            if len(X_test.shape) > 1:
                                ...
                                distributions['feature_importance'] = [
                                    float(np.mean(feature_importance))
                                ] * len(X_test)

                            # Calculate activation norm (L2 norm of features)
                            activation_norm = np.linalg.norm(
                                X_test, axis=1 if len(X_test.shape) > 1 else 0
                            )
                            distributions[
                                'activation_norm'
                            ] = activation_norm.tolist()
            ...
            logger.info(
                f'Collected frequency distributions: {list(distributions.keys())}'
            )
    ...
    def _collect_ks_distributions(
        ...
        try:
            ...
            if 'test_ks_statistic' in results_df.columns:
                ks_stats = results_df[
                    ['model_type', 'temperature', 'alpha', 'test_ks_statistic']
                ].copy()
                ...
                if not ks_stats.empty:
                    ...
                    for idx, row in ks_stats.iterrows():
                        ...
                        if 'test_ks_pvalue' in row and pd.notna(
                            row['test_ks_pvalue']
                        ):
                            ...
                            p_value = (
                                0.01
                                if ks_stat > 0.3
                                else (0.05 if ks_stat > 0.2 else 0.5)
                            )
                    ...
                    ks_data['significance_summary'] = {
                        ...
                        'marginal': int(
                            sum((ks_values >= 0.1) & (ks_values < 0.2))
                        ),
                    ...
                    if not ks_stats.empty:
                        ...
                        student_cdf = stats.norm.cdf(
                            x_values, loc=shift, scale=1.1
                        )
                    ...
                    ks_data['teacher_data'] = np.random.normal(
                        0, 1, 500
                    ).tolist()
                    ks_data['student_data'] = np.random.normal(
                        0.1, 1.1, 500
                    ).tolist()
                    ...
                    sample_quantiles = (
                        theoretical_quantiles + np.random.normal(0, 0.1, 100)
                    )
                    ks_data['qq_plot'] = {
                    ...
                    empirical_prob = np.clip(
                        theoretical_prob + np.random.normal(0, 0.02, 100), 0, 1
                    )
                    ks_data['pp_plot'] = {
                    ...
                    if len(ks_stats) > 1:
                        layers = [
                            f'Layer_{i}' for i in range(min(7, len(ks_stats)))
                        ]
                        models_data = []
                        for model_name in ks_stats['model_type'].unique()[
                            :3
                        ]:  # Top 3 models
                            model_ks_values = np.random.uniform(
                                0.05, 0.25, len(layers)
                            )
                            models_data.append(
                                {
                                    'name': str(model_name),
                                    'values': {
                                        layer: float(val)
                                        for layer, val in zip(
                                            layers, model_ks_values
                                        )
                                    },
                                }
                            )
            ...
            logger.info(
                f'Collected KS distributions with keys: {list(ks_data.keys())}'
            )
