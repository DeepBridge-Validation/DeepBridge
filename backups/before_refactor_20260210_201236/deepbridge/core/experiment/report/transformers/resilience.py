...
class ResilienceDataTransformer(DataTransformer):
    ...
    def transform(
        ...
        if 'test_results' in report_data:
            ...
            logger.info(
                f"Found 'test_results' key with keys: {list(test_results.keys())}"
            )
            ...
            if 'primary_model' in test_results:
                ...
                logger.info(
                    f'Found primary_model in test_results with keys: {list(primary_model.keys())}'
                )
                ...
                if 'metrics' in primary_model and not report_data.get(
                    'base_score'
                ):
                    ...
                    for metric_name in [
                        ...
                        if metric_name in metrics:
                            ...
                            logger.info(
                                f'Set base_score to {metrics[metric_name]} from {metric_name}'
                            )
                            break
        ...
        if 'initial_model_evaluation' in report_data:
            ...
            if (
                ...
                if (
                    'feature_importance' in initial_primary
                    and not report_data.get('feature_importance')
                ):
                    report_data['feature_importance'] = initial_primary[
                        'feature_importance'
                    ]
                    logger.info(
                        f"Extracted {len(initial_primary['feature_importance'])} features from initial_model_evaluation"
                    )

                # Use as model_feature_importance if not set
                if (
                    'feature_importance' in initial_primary
                    and not report_data.get('model_feature_importance')
                ):
                    report_data['model_feature_importance'] = initial_primary[
                        'feature_importance'
                    ]
        ...
        if 'primary_model' in report_data:
            logger.info(
                "Found 'primary_model' key at root level, extracting data..."
            )
            primary_data = report_data['primary_model']

            # Extract feature importance data
            if 'feature_importance' in primary_data and not report_data.get(
                'feature_importance'
            ):
                logger.info(
            ...
            if (
                'model_feature_importance' in primary_data
                and not report_data.get('model_feature_importance')
            ):
                logger.info(
        ...
        if 'metric' not in report_data:
            report_data['metric'] = next(
                iter(report_data.get('metrics', {}).keys()), 'score'
            )
        ...
        if 'results' in report_data:
            if 'resilience' in report_data['results']:
                ...
                logger.info(
                    f'Found resilience key with keys: {list(resilience_results.keys())}'
                )

                # Check in direct resilience object
                if (
                    'feature_importance' in resilience_results
                    and resilience_results['feature_importance']
                ):
                    logger.info(
                        f"Found feature_importance directly in results.resilience with {len(resilience_results['feature_importance'])} features"
                    )
                    report_data['feature_importance'] = resilience_results[
                        'feature_importance'
                    ]

                if (
                    'model_feature_importance' in resilience_results
                    and resilience_results['model_feature_importance']
                ):
                    logger.info(
                        f"Found model_feature_importance directly in results.resilience with {len(resilience_results['model_feature_importance'])} features"
                    )
                    report_data[
                        'model_feature_importance'
                    ] = resilience_results['model_feature_importance']
                ...
                if 'results' in resilience_results:
                    ...
                    if 'primary_model' in nested_results:
                        ...
                        logger.info(
                            'Found primary_model in nested results.resilience.results'
                        )
        ...
        if 'distribution_shift_results' not in report_data:
            ...
            if 'test_results' in report_data and isinstance(
                report_data['test_results'], list
            ):
                report_data['distribution_shift_results'] = report_data[
                    'test_results'
                ]
            elif (
                'distribution_shift' in report_data
                and 'all_results' in report_data['distribution_shift']
            ):
        ...
        if 'alphas' not in report_data:
            ...
            report_data['alphas'] = (
                sorted(list(alphas)) if alphas else [0.1, 0.2, 0.3]
            )
        ...
        if 'avg_performance_gap' not in report_data:
            ...
            if performance_gaps:
                report_data['avg_performance_gap'] = sum(
                    performance_gaps
                ) / len(performance_gaps)
            elif 'resilience_score' in report_data:
                # If we have resilience score but no average gap, calculate gap from score
                report_data['avg_performance_gap'] = (
                    1.0 - report_data['resilience_score']
                )
            else:
        ...
        try:
            ...
            if (
                ...
                for result in report_data['distribution_shift']['all_results']:
                    ...
                    score = result.get('remaining_metric') or result.get(
                        'target_performance'
                    )
            ...
            if not scores:
                logger.warning(
                    'No performance scores found for boxplot generation'
                )
                return {}
            ...
            logger.info(
                f'Generated boxplot data: median={median:.4f}, iqr={iqr:.4f}, outliers={len(outliers)}'
            )
            return boxplot_data
