...
class StaticRobustnessTransformer:
    ...
    def _enhance_for_static_report(
        ...
        if 'raw' in enhanced and 'by_level' in enhanced['raw']:
            ...
            perturbation_levels = sorted(
                [float(level) for level in raw_data.keys()]
            )
        ...
        if 'alternative_models' in enhanced:
            ...
            for model_name, model_data in enhanced[
                'alternative_models'
            ].items():
                model_iterations = {}
        ...
        if 'feature_importance' in enhanced and enhanced['feature_importance']:
            ...
            for feature, value in enhanced['feature_importance'].items():
                try:
                    ...
                    logger.warning(
                        f'Non-numeric feature importance for {feature}: {value}, setting to 0'
                    )
            ...
            sorted_importance = dict(
                sorted(
                    clean_importance.items(), key=lambda x: x[1], reverse=True
                )
            )
            ...
            for feature, impact in clean_importance.items():
                ...
                individual_impacts[
                    feature
                ] = -impact  # Invert to show drop as negative

            enhanced['individual_feature_impacts'] = individual_impacts
            logger.info(
                f'Prepared individual_feature_impacts with {len(individual_impacts)} features'
            )

        # Same for model feature importance
        if (
            'model_feature_importance' in enhanced
            and enhanced['model_feature_importance']
        ):
            # Ensure all values are numbers
            clean_importance = {}
            for feature, value in enhanced['model_feature_importance'].items():
                try:
                    ...
                    logger.warning(
                        f'Non-numeric model feature importance for {feature}: {value}, setting to 0'
                    )
                    clean_importance[feature] = 0.0
        ...
        if method_comparison_data:
            ...
            logger.info(
                'Prepared method comparison data for raw vs quantile visualization'
            )

        # Prepare selected features comparison data if both all_features and feature_subset results exist
        selected_features_comparison = (
            self._prepare_selected_features_comparison_data(enhanced)
        )
        if selected_features_comparison:
            ...
            logger.info(
                'Prepared selected features comparison data for all vs selected visualization'
            )

        # Prepare detailed distribution data for enhanced boxplot
        detailed_distribution_data = self._prepare_detailed_distribution_data(
            enhanced
        )
        if detailed_distribution_data:
            enhanced['detailed_distribution'] = detailed_distribution_data
            logger.info(
                'Prepared detailed distribution data for enhanced boxplot visualization'
            )
        ...
        if distribution_grid_data:
            ...
            logger.info(
                f"Prepared distribution grid data for {distribution_grid_data['n_models']} models across {distribution_grid_data['n_levels']} levels"
            )

        # Add additional information that might be needed for static visualizations
        enhanced['visualization_type'] = 'static'
        enhanced['has_iterations'] = bool(
            iteration_data and any(iteration_data.values())
        )
        ...
        if 'features' not in enhanced or not enhanced['features']:
            ...
            if (
                'feature_importance' in enhanced
                and enhanced['feature_importance']
            ):
                enhanced['features'] = list(
                    enhanced['feature_importance'].keys()
                )
                logger.info(
                    f"Extracted {len(enhanced['features'])} features from feature_importance"
                )
            # Ou de primary_model se disponível
            elif 'primary_model' in enhanced:
                if (
                    'features' in enhanced['primary_model']
                    and enhanced['primary_model']['features']
                ):
                    enhanced['features'] = enhanced['primary_model'][
                        'features'
                    ]
                    logger.info(
                        f"Extracted {len(enhanced['features'])} features from primary_model.features"
                    )
                elif (
                    'feature_importance' in enhanced['primary_model']
                    and enhanced['primary_model']['feature_importance']
                ):
                    enhanced['features'] = list(
                        enhanced['primary_model']['feature_importance'].keys()
                    )
                    logger.info(
                        f"Extracted {len(enhanced['features'])} features from primary_model.feature_importance"
                    )
                else:
                    enhanced['features'] = []
            else:
        ...
        if 'alternative_models' in data and 'alt_iterations_by_level' in data:
            for model_name, model_data in data['alternative_models'].items():
                ...
                if model_name in data['alt_iterations_by_level']:
                    for level_scores in data['alt_iterations_by_level'][
                        model_name
                    ].values():
                        if level_scores:
    ...
    def _prepare_method_comparison_data(
        self, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if (
            'raw' not in data
            or 'by_level' not in data['raw']
            or 'quantile' not in data
            or 'by_level' not in data['quantile']
        ):
            return {}
        ...
        if not common_levels:
            logger.warning(
                'No common perturbation levels found between raw and quantile methods'
            )
            return {}
        ...
        for level in sorted_levels:
            ...
            if (
                ...
                and 'all_features' in raw_data[level_str]['overall_result']
            ):
                raw_result = raw_data[level_str]['overall_result'][
                    'all_features'
                ]
                raw_scores.append(raw_result.get('mean_score', 0))
                raw_worst_scores.append(
                    raw_result.get(
                        'worst_score', raw_result.get('min_score', 0)
                    )
                )
            else:
                raw_scores.append(0)
            ...
            if (
                level_str in quantile_data
                and 'overall_result' in quantile_data[level_str]
                and 'all_features'
                in quantile_data[level_str]['overall_result']
            ):
                quantile_result = quantile_data[level_str]['overall_result'][
                    'all_features'
                ]
                quantile_scores.append(quantile_result.get('mean_score', 0))
                quantile_worst_scores.append(
                    quantile_result.get(
                        'worst_score', quantile_result.get('min_score', 0)
                    )
                )
            else:
        ...
        logger.info(
            f'Prepared method comparison data for {len(sorted_levels)} perturbation levels'
        )
    ...
    def _prepare_selected_features_comparison_data(
        self, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if 'raw' in data and 'by_level' in data['raw']:
            ...
            for level_key, level_data in raw_data.items():
                if (
                    'overall_result' in level_data
                    and 'all_features' in level_data['overall_result']
                    and 'feature_subset' in level_data['overall_result']
                ):
        ...
        if not comparison_data and (
            'quantile' in data and 'by_level' in data['quantile']
        ):
            quantile_data = data['quantile']['by_level']
            # Check if any level has both all_features and feature_subset
        ...
        if not comparison_data:
            logger.warning(
                'No comparison data found with both all_features and feature_subset results'
            )
            return {}
        ...
        if not levels_with_both:
            logger.warning(
                'No perturbation levels found with both all_features and feature_subset data'
            )
            return {}
        ...
        for level in sorted_levels:
            ...
            all_features_scores.append(
                all_features_result.get('mean_score', 0)
            )
            all_features_worst.append(
                all_features_result.get(
                    'worst_score', all_features_result.get('min_score', 0)
                )
            )

            # Selected features scores
            selected_features_result = level_data['feature_subset']
            selected_features_scores.append(
                selected_features_result.get('mean_score', 0)
            )
            selected_features_worst.append(
                selected_features_result.get(
                    'worst_score', selected_features_result.get('min_score', 0)
                )
            )
        ...
        if not any(all_features_scores) or not any(selected_features_scores):
            logger.warning(
                'No valid scores found for selected features comparison'
            )
            return {}

        logger.info(
            f'Prepared selected features comparison data for {len(sorted_levels)} perturbation levels using {method_name} method'
        )
    ...
    def _prepare_detailed_distribution_data(
        self, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if not source_data:
            logger.warning(
                'No perturbation data available for detailed distribution'
            )
            return {}
        ...
        for level_key, level_data in source_data.items():
            ...
            if level_scores:
                ...
                logger.debug(
                    f'Collected {len(level_scores)} scores for level {level_float}'
                )

        if not perturbation_data:
            logger.warning(
                'No iteration scores found for detailed distribution'
            )
            return {}

        # Verify we have enough data points for meaningful distribution
        total_scores = sum(
            len(scores) for scores in perturbation_data.values()
        )
        if total_scores < 10:  # Minimum threshold for meaningful distribution
            logger.warning(
                f'Insufficient data points ({total_scores}) for detailed distribution'
            )
            return {}

        logger.info(
            f'Prepared detailed distribution data for {len(perturbation_data)} perturbation levels '
    ...
    def _prepare_distribution_grid_data(
        self, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Prepare distribution grid data for comprehensive matrix visualization
        ...
        if primary_data:
            ...
            model_distributions = self._extract_model_distributions(
                primary_data
            )
        ...
        if 'alternative_models' in data:
            for alt_model_name, alt_model_data in data[
                'alternative_models'
            ].items():
                model_distributions = self._extract_model_distributions(
                    alt_model_data
                ...
                if model_distributions:
                    ...
                    baseline_scores[alt_model_name] = alt_model_data.get(
                        'base_score', 0
                    )

        # Also check for other model patterns
        for key, value in data.items():
            if key.startswith('alt_model_') or (
                key == 'alternative_model'
                and key not in ['alternative_models']
            ):
                if isinstance(value, dict) and 'raw' in value:
                    # Extract model name from key or use a default
                    model_name = key.replace('alt_model_', 'Model ').replace(
                    ...
                    if model_distributions:
                        models_data[model_name] = model_distributions
                        baseline_scores[model_name] = value.get(
                            'base_score', 0
                        )
        ...
        logger.info(
            f'Prepared distribution grid data for {total_models} models '
        ...
        return {
            ...
            'metric': data.get(
                'metric', data.get('primary_model', {}).get('metric', 'Score')
            ),
        }

    def _extract_model_distributions(
