...
app = typer.Typer(
    help='DeepBridge CLI - Tools for Model Validation and Distillation'
)
...
def train_distilled_model(
    model_type: DistillerModelType = typer.Argument(
        ..., help='Type of model to use'
    ),
    original_predictions: Path = typer.Argument(
        ..., help='Path to original model predictions CSV'
    ),
    features_data: Path = typer.Argument(
        ..., help='Path to features data CSV'
    ),
    save_path: Optional[Path] = typer.Option(
        None, '--save', '-s', help='Path to save the model'
    ),
    params_file: Optional[Path] = typer.Option(
        None, '--params', '-p', help='JSON file with model parameters'
    ),
    test_size: float = typer.Option(
        0.2, '--test-size', '-t', help='Test set size for validation'
    ),
):
...
def predict_with_model(
    ...
    try:
        with console.status(
            '[bold green]Loading model and making predictions...'
        ):
            # Load model and data
            distiller = AutoDistiller.load(model_path)
        ...
        if output:
            pd.DataFrame(predictions, columns=['probability']).to_csv(
                output, index=False
            )
...
def evaluate_model(
    ...
    distilled_predictions: Path = typer.Argument(
        ..., help='Path to distilled model predictions CSV'
    ),
    output_format: str = typer.Option(
        'table', '--format', '-f', help='Output format (table or json)'
    ),
    ...
    try:
        ...
        with console.status('[bold green]Calculating metrics...'):
            # Calculate metrics
            metrics = AutoDistiller.calculate_detailed_metrics(
