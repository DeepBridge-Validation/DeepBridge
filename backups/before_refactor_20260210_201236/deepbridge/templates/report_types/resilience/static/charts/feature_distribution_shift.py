...
class FeatureDistributionShiftChart(BaseChartGenerator):
    ...
    def generate(
        ...
        sorted_features = sorted(
            clean_distances.items(), key=lambda x: x[1], reverse=True
        )
        top_features = dict(
            sorted_features[: min(top_n, len(sorted_features))]
        )
            ...
            try:
                ...
                if hasattr(self.chart_generator, 'feature_psi_chart'):
                    return self.chart_generator.feature_psi_chart(
                        psi_data=feature_data, title=title
                    )
                # Fall back to bar_chart
                    ...
                    return self.chart_generator.bar_chart(
                        data=dist_data, title=title
                    )
            except Exception as e:
                logger.error(
                    f'Error using chart generator for feature distribution shift: {str(e)}'
                )
