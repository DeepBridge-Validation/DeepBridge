...
class FeatureResidualCorrelationChart(BaseChartGenerator):
    ...
    def generate(
        ...
        if not self._validate_data(feature_correlations):
            logger.warning(
                'Invalid or empty feature correlations data for chart'
            )
            return ''
        ...
        sorted_correlations = sorted(
            clean_correlations.items(), key=lambda x: abs(x[1]), reverse=True
        )

        # Take top N
        top_correlations = sorted_correlations[
            : min(top_n, len(sorted_correlations))
        ]
        ...
        if self.chart_generator:
            ...
            if correlation_matrix and feature_names:
                try:
                    ...
                    logger.error(
                        f'Error using chart generator for correlation chart: {str(e)}'
                    )
        ...
        try:
            ...
            if not features:
                logger.warning(
                    'No valid correlations found for correlation chart'
                )
                return ''
            ...
            bars = self.sns.barplot(
                y=features, x=correlations, palette='coolwarm', ax=ax
            )

            # Add value labels
            for i, v in enumerate(correlations):
                ax.text(
                    v + 0.01 if v >= 0 else v - 0.06,
                    i,
                    f'{v:.3f}',
                    va='center',
                    fontsize=10,
