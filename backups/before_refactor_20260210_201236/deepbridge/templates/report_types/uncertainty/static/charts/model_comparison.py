...
class ModelComparisonChart(BaseChartGenerator):
    ...
    def _validate_data(self, models_data):
        ...
        for metric in standard_metrics:
            if metric in models_data:
                logger.info(
                    f"Found primary model metric '{metric}': {models_data[metric]}"
                )
                has_primary_metrics = True

        # Check metrics dictionary if present
        if 'metrics' in models_data and isinstance(
            models_data['metrics'], dict
        ):
            logger.info(f"metrics keys: {list(models_data['metrics'].keys())}")
            for metric in standard_metrics:
                if metric in models_data['metrics']:
                    logger.info(
                        f"Found primary model metric '{metric}' in metrics: {models_data['metrics'][metric]}"
                    )
                    has_primary_metrics = True
        ...
        if 'alternative_models' in models_data and isinstance(
            ...
            logger.info(
                f"Found {len(models_data['alternative_models'])} alternative models"
            )
                ...
                if isinstance(model_data, dict):
                    for metric in standard_metrics:
                        if metric in model_data:
                            logger.info(
                                f"Found metric '{metric}' in alternative model '{model_name}'"
                            )
                            has_alternative_models = True
                            break

                    if 'metrics' in model_data and isinstance(
                        model_data['metrics'], dict
                    ):
                        for metric in standard_metrics:
                            if metric in model_data['metrics']:
                                logger.info(
                                    f"Found metric '{metric}' in metrics of alternative model '{model_name}'"
                                )
                                has_alternative_models = True
                                break
    ...
    def generate(
        ...
        title: str = 'Model Uncertainty Metrics Comparison',
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if not self._validate_data(models_data):
                logger.warning(
                    'Invalid data provided for model comparison chart'
                )
                return None
            ...
            for metric in metrics:
                if metric in models_data:
                    ...
                    'metrics' in models_data
                    and metric in models_data['metrics']
                ):
                ...
                for model_name, model_data in models_data[
                    ...
                    for metric in metrics:
                        if metric in model_data:
                            ...
                            'metrics' in model_data
                            and metric in model_data['metrics']
                        ):
                            model_metrics[metric] = model_data['metrics'][
                                metric
                            ]
            ...
            if not all_models:
                logger.warning(
                    'No models with valid metrics found for comparison chart'
                )
                return None
