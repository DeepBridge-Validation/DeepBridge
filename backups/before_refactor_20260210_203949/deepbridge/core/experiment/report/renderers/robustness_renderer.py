...
⚠️ DEPRECATED: This renderer is deprecated and will be removed in a future version.
Please use the new unified API:
...
warnings.warn(
    "RobustnessRenderer is deprecated and will be removed in a future version. "
    "Please use deepbridge.core.experiment.report.ReportGenerator instead. "
        ...
        try:
            ...
            if not template_path:
                raise FileNotFoundError(
                    f'No template found for robustness report in: {template_paths}'
                )
            ...
            if 'initial_results' in results:
                ...
                logger.info(
                    f"Initial results transformed for {len(initial_results.get('models', {}))} models"
                )
            else:
                # If no initial_results, still transform to get default data structure
                logger.info(
                    'No initial_results found, using transformer default'
                )
                initial_results = self.initial_results_transformer.transform(
                    {}
                )
                logger.info(
                    f"Using default initial results with {len(initial_results.get('models', {}))} models"
                )
            ...
            if initial_results:
                ...
                if 'models' in initial_results:
                    for model_id, model_data in initial_results[
                        'models'
                    ].items():
                ...
                chart_data['radar_chart_data'] = {'models': models_metrics}

            # Create template context
            context = self.base_renderer._create_context(
                report_data, 'robustness', css_content, js_content, report_type
            )
            ...
            if initial_results:
                ...
                logger.info(
                    f"Added initial_results to report_data with {len(initial_results.get('models', {}))} models"
                )
            ...
            if 'weakspot_analysis' in results:
                ...
                logger.info(
                    f"Added weakspot_analysis with {weakspot_analysis.get('summary', {}).get('total_weakspots', 0)} weakspots"
                )
            ...
            if not test_sample_count and 'raw' in report_data:
                ...
                if 'sample_count' in report_data['raw']:
                    ...
                    if 'runs' in report_data['raw']['by_level'][first_level]:
                        test_sample_count = report_data['raw']['by_level'][
                            first_level
                        ].get('sample_count', 0)
            ...
            if feature_importance:
                ...
                critical_features = sum(
                    1 for imp in feature_importance.values() if imp > 0.05
                )
            ...
            context.update(
                {
                    ...
                    'chart_data_json': self._sanitize_json(
                        chart_data
                    ),  # Use new sanitizer
                    ...
                    'weakspot_analysis_json': self._sanitize_json(
                        weakspot_analysis
                    )
                    if weakspot_analysis
                    else '{}',
                    'has_overfitting_analysis': bool(overfitting_analysis),
                    'overfitting_analysis': overfitting_analysis or {},
                    'overfitting_analysis_json': self._sanitize_json(
                    ...
                    ),  # Default to Scikit-learn if not specified
                }
            )

            # Log debug info for troubleshooting
            logger.info(
                f"robustness_score: {report_data.get('robustness_score')}"
            )
            if 'alternative_models' in report_data:
                for name, model_data in report_data[
                    'alternative_models'
                ].items():
                    logger.info(
                        f"Alternative model {name} robustness_score: {model_data.get('robustness_score')}"
                    )
            ...
            try:
                ...
                if not os.path.exists(template_partials_dir):
                    ...
                    logger.info(
                        f'Created partials directory: {template_partials_dir}'
                    )
            except Exception as e:
                logger.warning(
                    f'Could not create partials directory: {str(e)}'
                )
        ...
        <table class="data-table">
            ...
            <tbody id="importance-comparison-data">
                <!-- Will be populated by JavaScript -->
                    ...
                    logger.info(
                        f'Created importance comparison template: {importance_comparison_path}'
                    )
                except Exception as e:
                    logger.warning(
                        f'Could not create importance comparison template: {str(e)}'
                    )
            ...
            standard_report_path = os.path.join(
                templates_dir, 'report_robustness.html'
            )

            logger.info(
                f'Also writing report to standard path: {standard_report_path}'
            )
            return self.base_renderer._write_report(
                rendered_html, standard_report_path
            )
            ...
            if os.path.exists(details_controller_path):
                try:
                    with open(details_controller_path, 'r') as file:
                        ...
                        details_controller_content = self._process_js_content(
                            details_controller_content, details_controller_path
                        )
                        additional_content += f'\n// Details Controller\n{details_controller_content}\n'
                        logger.info(
                            f'Added details controller from {details_controller_path}'
                        )
                except Exception as file_error:
                    logger.error(
                        f'Error reading details controller: {str(file_error)}'
                    )
            ...
            if os.path.exists(details_charts_path):
                try:
                    with open(details_charts_path, 'r') as file:
                        ...
                        # Process to remove any ES6 imports/exports
                        details_charts_content = self._process_js_content(
                            details_charts_content, details_charts_path
                        )
                        additional_content += (
                            f'\n// Details Charts\n{details_charts_content}\n'
                        )
                        logger.info(
                            f'Added details charts from {details_charts_path}'
                        )
                except Exception as file_error:
                    logger.error(
                        f'Error reading details charts: {str(file_error)}'
                    )
            ...
            if os.path.exists(error_interceptor_path):
                try:
                    with open(error_interceptor_path, 'r') as file:
                        ...
                        logger.info(
                            f'Added error interceptor from {error_interceptor_path}'
                        )
                except Exception as e:
    ...
    def _sanitize_json(self, data: Any) -> str:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if (
                    'iterations_by_level' not in data
                    or not data['iterations_by_level']
                ):
                    data['iterations_by_level'] = {}
                    # If we have perturbation levels, create empty arrays for them
                    if (
                        'perturbation_levels' in data
                        and data['perturbation_levels']
                    ):
                ...
                if 'alternative_models_iterations' not in data:
                    ...
                    if (
                        'alternative_models' in data
                        and data['alternative_models']
                    ):
                ...
                elif 'alt_iterations_by_level' in data and isinstance(
                    data['alt_iterations_by_level'], dict
                ):
                    # Copy from alt_iterations_by_level to standard format
                    data['alternative_models_iterations'] = data[
                        'alt_iterations_by_level'
                    ]
                    # Ensure all keys are strings
                    for model_name, levels in data[
                        'alternative_models_iterations'
                    ].items():
                ...
                if (
                    'boxplot_data' in data
                    and 'models' in data['boxplot_data']
                    and len(data['boxplot_data']['models']) > 0
                ):
                    for model in data['boxplot_data']['models']:
                        if (
                            'scores' not in model
                            or not model['scores']
                            or len(model['scores']) == 0
                        ):
                            logger.warning(
                                f"Model {model.get('name', 'unknown')} has no scores in boxplot data"
                            )

                            # If this is the primary model, try to get scores from iterations_by_level
                            if (
                                model.get('name') == data.get('model_name')
                                and 'iterations_by_level' in data
                            ):
                                all_scores = []
                                for level_scores in data[
                                    'iterations_by_level'
                                ].values():
                                    if (
                                        isinstance(level_scores, list)
                                        and level_scores
                                    ):
                                ...
                                if all_scores:
                                    logger.info(
                                        f'Found {len(all_scores)} scores in iterations_by_level for primary model'
                                    )
                            ...
                            # If this is an alternative model, try to get scores from alternative_models_iterations
                                ...
                                if (
                                    model_name
                                    in data['alternative_models_iterations']
                                ):
                                    all_scores = []
                                    for level_scores in data[
                                        'alternative_models_iterations'
                                    ...
                                    if all_scores:
                                        logger.info(
                                            f'Found {len(all_scores)} scores in alternative_models_iterations for {model_name}'
                                        )
                                        model['scores'] = all_scores
                ...
                if (
                    ...
                    for model_id, model_data in data['initial_results'][
                        'models'
                    ].items():
                        ...
                        if (
                            model_name in existing_model_names
                            or model_data.get('is_primary', False)
                        ):
                            continue
                        ...
                        scores = []
                        if (
                            'evaluation_results' in model_data
                            and 'scores' in model_data['evaluation_results']
                        ):
                            if isinstance(
                                model_data['evaluation_results']['scores'],
                                list,
                            ):
                                scores = model_data['evaluation_results'][
                                    'scores'
                                ]
                            else:
                                # Convert to list if not already a list
                        ...
                        if not scores:
                            logger.warning(
                                f'No scores found in initial_results for model {model_name}'
                            )
                            scores = []
                        ...
                        logger.info(
                            f'Adding model {model_name} from initial_results to boxplot data with {len(scores)} scores'
                        )
        ...
        try:
            ...
            for line in lines:
                ...
                if (
                    ...
                    or line_stripped == 'export default'
                    or line_stripped.startswith('export default ')
                ):
                    continue
    ...
    def _prepare_chart_data(
        ...
        if 'raw' in report_data and 'by_level' in report_data['raw']:
            ...
            for level in levels:
                level_str = str(level)
                if (
                    level_str in raw_data
                    and 'overall_result' in raw_data[level_str]
                ):
                    ...
                    if 'all_features' in result:
                        perturbed_scores.append(
                            result['all_features'].get('mean_score', None)
                        )
                        worst_scores.append(
                            result['all_features'].get('worst_score', None)
                        )
                    else:
                    ...
                    if 'feature_subset' in result:
                        feature_subset_scores.append(
                            result['feature_subset'].get('mean_score', None)
                        )
                    else:
                    ...
                    if 'all_features' in runs and runs['all_features']:
                        for run in runs['all_features']:
                            if (
                                ...
                                if (
                                    iteration_scores
                                    and len(iteration_scores) > 0
                                ):
                                    logger.info(
                                        f'Level {level_str}: Found {len(iteration_scores)} iteration scores'
                                    )
                                    all_iteration_scores.extend(
                                        iteration_scores
                                    )
                                else:
                                    logger.warning(
                                        f'Level {level_str}: No iteration scores found'
                                    )
                                ...
                                if (
                                    str(level)
                                    not in chart_data['iterations_by_level']
                                ):
                                    chart_data['iterations_by_level'][
                                        str(level)
                                    ] = []
        ...
        boxplot_data = {'models': [primary_model_boxplot]}
        ...
        if (
            'alternative_models' in report_data
            and report_data['alternative_models']
        ):
            alt_models = {}
            ...
            for model_name, model_data in report_data[
                ...
                if 'raw' in model_data and 'by_level' in model_data['raw']:
                    ...
                    logger.info(
                        f'Collected levels from model {model_name}: {alt_levels}'
                    )

            # Sort all collected levels
            all_levels_sorted = sorted(all_perturbation_levels)
            logger.info(
                f'All combined perturbation levels: {all_levels_sorted}'
            )
            ...
            for model_name, model_data in report_data[
                ...
                if 'raw' in model_data and 'by_level' in model_data['raw']:
                    ...
                    for level in all_levels_sorted:
                        ...
                        if (
                            level_str in raw_data
                            and 'runs' in raw_data[level_str]
                        ):
                            runs = raw_data[level_str]['runs']
                            if 'all_features' in runs and runs['all_features']:
                                for run in runs['all_features']:
                                    if (
                                        ...
                                        iteration_scores = run['iterations'][
                                            'scores'
                                        ]
                                        ...
                                        if (
                                            ...
                                            logger.info(
                                                f'Alternative model {model_name}, level {level_str}: Found {len(iteration_scores)} iteration scores'
                                            )
                                            alt_model_iteration_scores.extend(
                                                iteration_scores
                                            )
                                        else:
                                            logger.warning(
                                                f'Alternative model {model_name}, level {level_str}: No iteration scores found'
                                            )

                                        # Store level-specific scores for each alternative model
                                        if (
                                            not 'alt_iterations_by_level'
                                            in chart_data
                                        ):
                                            chart_data[
                                                'alt_iterations_by_level'
        ...
        if 'raw' in report_data and 'by_level' in report_data['raw']:
            ...
            for level in perturbation_levels:
                ...
                if (
                    ...
                    # All features scores
                    if 'all_features' in result:
                        updated_perturbed_scores.append(
                            result['all_features'].get('mean_score', None)
                        )
                        updated_worst_scores.append(
                            result['all_features'].get('worst_score', None)
                        )
                    else:
                    ...
                    # Feature subset scores
                    if 'feature_subset' in result:
                        updated_feature_subset_scores.append(
                            result['feature_subset'].get('mean_score', None)
                        )
                    else:
                ...
                if 'raw' in model_data and 'by_level' in model_data['raw']:
                    ...
                    for level in perturbation_levels:
                        ...
                        if (
                            ...
                            if 'all_features' in result:
                                alt_model_scores.append(
                                    result['all_features'].get(
                                        'mean_score', None
                                    )
                                )
                                alt_model_worst_scores.append(
                                    result['all_features'].get(
                                        'worst_score', None
                                    )
                                )
        ...
        if (
            'iterations_by_level' not in chart_data
            and 'raw' in report_data
            and 'by_level' in report_data['raw']
        ):
            iteration_data = {}
            raw_data = report_data['raw']['by_level']
            ...
            for level_str, level_data in raw_data.items():
                ...
                if (
                    'runs' in level_data
                    and 'all_features' in level_data['runs']
                ):
                    for run in level_data['runs']['all_features']:
                        if (
                            ...
                            iterations_by_level.extend(
                                run['iterations']['scores']
                            )
        ...
        if 'alt_iterations_by_level' in chart_data:
            chart_data['alternative_models_iterations'] = chart_data[
                'alt_iterations_by_level'
            ]
