...
class TestRunner(ITestRunner):
    ...
    def __init__(
        ...
        y_test,
        verbose: bool = False,
        feature_subset: t.Optional[
            t.List[str]
        ] = None,  # Consistently use feature_subset instead of features_select
    ...
    def run_test(
        ...
        if test_type not in self.tests:
            raise ValueError(
                f"Test type '{test_type}' not in configured tests: {self.tests}"
            )
        ...
        if self.alternative_models:
            for model_name, model in self.alternative_models.items():
                ...
                alt_result = self._run_model_test(
                    test_type, alt_dataset, config
                )
                ...
                if hasattr(model, 'predict_proba'):
                    ...
                    if y_prob.shape[1] > 1:  # Multi-class
                        metrics['roc_auc'] = roc_auc_score(
                            self.y_test, y_prob, multi_class='ovr'
                        )
                    else:  # Binary
                        metrics['roc_auc'] = roc_auc_score(
                            self.y_test, y_prob[:, 1]
                        )
            except:
