...
class ResilienceChartGenerator:
    ...
    def __init__(self, seaborn_chart_generator=None):
        ...
        self.feature_distribution_shift = FeatureDistributionShiftChart(
            seaborn_chart_generator
        )
        self.performance_gap = PerformanceGapChart(seaborn_chart_generator)
        self.critical_feature_distributions = (
            CriticalFeatureDistributionsChart(seaborn_chart_generator)
        )
        self.feature_residual_correlation = FeatureResidualCorrelationChart(
            seaborn_chart_generator
        )
        self.residual_distribution = ResidualDistributionChart(
            seaborn_chart_generator
        )
        self.model_comparison = ModelComparisonChart(seaborn_chart_generator)
        self.model_comparison_scatter = ModelComparisonScatterChart(
            seaborn_chart_generator
        )
        self.distance_metrics_comparison = DistanceMetricsComparisonChart(
            seaborn_chart_generator
        )
        self.feature_distance_heatmap = FeatureDistanceHeatmapChart(
            seaborn_chart_generator
        )
        self.model_resilience_scores = ModelResilienceScoresChart(
            seaborn_chart_generator
        )
    ...
    def generate_feature_distribution_shift(
        self, feature_distances, title='Feature Distribution Shift', top_n=10
    ):
        """Generate a chart showing the distribution shift for different features."""
        return self.feature_distribution_shift.generate(
            feature_distances, title, top_n
        )

    def generate_performance_gap(
        self,
        performance_metrics,
        ...
        return self.performance_gap.generate(
            performance_metrics, title, task_type
        )

    def generate_critical_feature_distributions(
        self,
        worst_samples,
        remaining_samples,
        top_features,
        ...
        return self.critical_feature_distributions.generate(
            worst_samples, remaining_samples, top_features, title
        )

    def generate_feature_residual_correlation(
        self,
        ...
        return self.feature_residual_correlation.generate(
            feature_correlations, title, top_n
        )

    def generate_residual_distribution(
        self,
        worst_residuals=None,
        remaining_residuals=None,
        all_residuals=None,
        title='Model Residual Distribution',
    ):
        """Generate a chart showing the distribution of residuals."""
        return self.residual_distribution.generate(
            worst_residuals, remaining_residuals, all_residuals, title
        )
    ...
    def generate_model_comparison(
        ...
        return self.model_comparison.generate(
            perturbation_levels, models_data, title, metric_name
        )

    def generate_model_comparison_scatter(
        self,
        models_data,
        ...
        return self.model_comparison_scatter.generate(
            models_data, title, x_label, y_label
        )
    ...
    def generate_distance_metrics_comparison(
        ...
        return self.distance_metrics_comparison.generate(
            alpha_levels, metrics_data, title, y_label
        )

    def generate_feature_distance_heatmap(
        self,
        feature_distances,
        title='Feature Distance Heatmap',
        top_n=15,
        cmap='viridis',
    ):
        """Generate a heatmap showing feature distances across multiple models or metrics."""
        return self.feature_distance_heatmap.generate(
            feature_distances, title, top_n, cmap
        )

    def generate_model_resilience_scores(
        self,
        models_data,
        ...
        return self.model_resilience_scores.generate(
            models_data, title, sort_by, ascending
        )
    ...
    def generate_performance_gap_by_alpha(
        ...
        return self.performance_gap_by_alpha.generate(
            alpha_levels, models_data, title, y_label
        )
