...
class PerformanceGapByAlphaChart(BaseChartGenerator):
    ...
    def _validate_data(self, models_data: Dict[str, Any]) -> bool:
        ...
        if not super()._validate_data(models_data):
            logger.warning(
                'Basic validation failed for performance gap by alpha chart'
            )
            return False

        # Add detailed logging
        logger.info(
            f'Validating performance gap by alpha data with: type={type(models_data)}'
        )
        ...
        if isinstance(models_data, dict):
            ...
            if 'calibration_results' in models_data:
                ...
                if isinstance(cal_results, dict):
                    logger.info(
                        f'calibration_results keys: {list(cal_results.keys())}'
                    )
                    ...
                    logger.info(
                        f'Has required data: alpha_values={has_alpha}, coverage_values={has_coverage}, expected_coverages={has_expected}'
                    )
            ...
            has_alpha = (
                'alpha_levels' in models_data or 'alphas' in models_data
            )
            has_perf_gaps = 'performance_gaps' in models_data

            if has_alpha and has_perf_gaps:
                logger.info(
                    'Found alternative data paths with alpha values and performance gaps'
                )
                return True

            # Check for primary_model with plot_data
            if 'primary_model' in models_data and isinstance(
                models_data['primary_model'], dict
            ):
                primary_model = models_data['primary_model']

                if 'plot_data' in primary_model and isinstance(
                    primary_model['plot_data'], dict
                ):
                    plot_data = primary_model['plot_data']

                    if 'alpha_comparison' in plot_data and isinstance(
                        plot_data['alpha_comparison'], dict
                    ):
                        alpha_data = plot_data['alpha_comparison']
                        logger.info(
                            f'alpha_comparison keys: {list(alpha_data.keys())}'
                        )
                        ...
                        logger.info(
                            f'alpha_comparison has required data: alphas={has_alpha}, coverages={has_coverage}, expected_coverages={has_expected}'
                        )
        ...
        logger.warning(
            'No suitable data found for performance gap by alpha chart'
        )
        return False
    ...
    def generate(
        ...
        try:
            ...
            if not self._validate_data(models_data):
                logger.warning(
                    'Invalid data provided for performance gap by alpha chart'
                )
                return None
            ...
            if 'calibration_results' in models_data and isinstance(
                ...
                if 'alpha_values' in models_data['calibration_results']:
                    alpha_values = models_data['calibration_results'][
                        'alpha_values'
                    ]

                if 'performance_gaps' in models_data['calibration_results']:
                    performance_gaps = models_data['calibration_results'][
                        'performance_gaps'
                    ]
                elif (
                    'coverage_values' in models_data['calibration_results']
                    and 'expected_coverages'
                    in models_data['calibration_results']
                ):
                    # Calculate performance gaps as the difference between actual and expected coverage
                    actual = models_data['calibration_results'][
                        'coverage_values'
                    ]
                    expected = models_data['calibration_results'][
                        'expected_coverages'
                    ]
            ...
            if (
                alpha_values is None or performance_gaps is None
            ) and 'primary_model' in models_data:
                primary_model = models_data['primary_model']

                if (
                    'plot_data' in primary_model
                    and 'alpha_comparison' in primary_model['plot_data']
                ):
                    ...
                    if performance_gaps is None:
                        if 'performance_gaps' in alpha_data:
                            ...
                            'coverages' in alpha_data
                            and 'expected_coverages' in alpha_data
                        ):
            ...
            if alpha_values is None or performance_gaps is None:
                logger.warning(
                    'Alpha values or performance gaps not found in data'
                )
                return None
            ...
            if len(alpha_values) != len(performance_gaps):
                logger.warning(
                    f'Alpha values ({len(alpha_values)}) and performance gaps ({len(performance_gaps)}) have different lengths'
                )
            ...
            plt.bar(
                alpha_values, performance_gaps, color='steelblue', alpha=0.7
            )
            ...
            if add_annotations:
                for i, gap in enumerate(performance_gaps):
                    color = (
                        'green'
                        if abs(gap) < 0.05
                        else ('orange' if abs(gap) < 0.1 else 'red')
                    )
                    plt.annotate(
                        f'{gap:.3f}',
