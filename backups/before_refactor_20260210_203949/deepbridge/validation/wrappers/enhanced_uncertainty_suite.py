...
class EnhancedUncertaintySuite(UncertaintySuite):
    ...
    def __init__(
        ...
        self.reliable_threshold_ratio = (
            1.1  # Multiplier for classification of reliable/unreliable points
        )
        self.bin_count = 10  # Number of bins for marginal bandwidth analysis

    def evaluate_uncertainty(
        self, method: str, params: Dict, feature=None
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f'[EVALUATE_DEBUG] Standard results keys: {list(results.keys())}'
        )
        ...
        if method == 'crqr':
            ...
            logger.info(
                f'[EVALUATE_DEBUG] Starting enhanced analysis with data: X shape={X.shape}, widths shape={widths.shape}'
            )

            # Calculate additional metrics
            additional_metrics = self._calculate_additional_metrics(
                X, y, model, lower_bound, upper_bound, widths
            )
            results.update(additional_metrics)
            logger.info(
                f'[EVALUATE_DEBUG] Added additional metrics: {list(additional_metrics.keys())}'
            )

            # Add reliability analysis
            reliability_analysis = self._analyze_reliability(
                X, y, lower_bound, upper_bound, widths
            )
            results['reliability_analysis'] = reliability_analysis
            logger.info(
                f'[EVALUATE_DEBUG] Added reliability_analysis with keys: {list(reliability_analysis.keys())}'
            )
            ...
            logger.info(
                f'[EVALUATE_DEBUG] Added marginal_bandwidth for features: {list(marginal_bandwidth.keys())}'
            )
            ...
            for key in ['reliability_analysis', 'marginal_bandwidth']:
                if key in results:
                    logger.info(
                        f'[EVALUATE_DEBUG] Verified {key} is in the results'
                    )
                else:
                    logger.error(
                        f'[EVALUATE_DEBUG] ERROR: {key} is missing from the results'
                    )

        # Log the final set of top-level keys
        logger.info(
            f'[EVALUATE_DEBUG] Final evaluation results keys: {list(results.keys())}'
        )

        return results

    def _calculate_additional_metrics(
        self, X, y, model, lower_bound, upper_bound, widths
    ):
        ...
        for threshold in threshold_values:
            ...
            if np.sum(mask) > 0:
                ...
                coverage = np.mean(
                    (y[mask] >= lower_bound[mask])
                    & (y[mask] <= upper_bound[mask])
                )
    ...
    def _analyze_reliability(self, X, y, lower_bound, upper_bound, widths):
        ...
        logger.info(
            f'[RELIABILITY_DEBUG] X shape: {X.shape}, y shape: {y.shape}'
        )
        logger.info(
            f'[RELIABILITY_DEBUG] lower_bound shape: {lower_bound.shape}, upper_bound shape: {upper_bound.shape}'
        )
        ...
        logger.info(
            f'[RELIABILITY_DEBUG] Calculated threshold: {threshold} (median width: {np.median(widths)} × ratio: {self.reliable_threshold_ratio})'
        )
        ...
        logger.info(
            f'[RELIABILITY_DEBUG] Reliable points: {reliable_count}, Unreliable points: {unreliable_count}'
        )
        ...
        logger.info(
            f'[RELIABILITY_DEBUG] Selected features for analysis: {feature_names}'
        )
        ...
        for feature in feature_names:
            ...
            logger.info(
                f"[RELIABILITY_DEBUG] Feature '{feature}': reliable={len(reliable_values)} values, unreliable={len(unreliable_values)} values"
            )
            ...
            if len(reliable_list) > 0:
                logger.info(
                    f"[RELIABILITY_DEBUG] '{feature}' reliable sample: {reliable_list[:3]}"
                )
            if len(unreliable_list) > 0:
                logger.info(
                    f"[RELIABILITY_DEBUG] '{feature}' unreliable sample: {unreliable_list[:3]}"
                )
            ...
            if len(reliable_values) > 10 and len(unreliable_values) > 10:
                ...
                logger.info(
                    f"[RELIABILITY_DEBUG] Feature '{feature}' PSI: {psi}"
                )

        # Verify feature_distributions is properly structured
        logger.info(
            f'[RELIABILITY_DEBUG] feature_distributions types: {list(feature_distributions.keys())}'
        )
        for dist_type, features in feature_distributions.items():
            logger.info(
                f"[RELIABILITY_DEBUG] '{dist_type}' has {len(features)} features"
            )
    ...
    def _analyze_marginal_bandwidth(self, X, widths):
        ...
        logger.info(
            f'[MARGINAL_DEBUG] X shape: {X.shape}, widths shape: {widths.shape}'
        )
        ...
        feature_names = X.columns[:num_features].tolist()
        logger.info(
            f'[MARGINAL_DEBUG] Selected features for analysis: {feature_names}'
        )
        ...
        for feature in feature_names:
            ...
            logger.info(
                f"[MARGINAL_DEBUG] Feature '{feature}' range: [{min_val}, {max_val}]"
            )
            ...
            logger.info(
                f'[MARGINAL_DEBUG] Created {len(bin_centers)} bins with centers: {bin_centers}'
            )
            ...
            for i in range(len(bin_centers)):
                ...
                mask = (feature_values >= bin_edges[i]) & (
                    feature_values < bin_edges[i + 1]
                )
                ...
                if bin_count == 0:
                    logger.info(
                        f'[MARGINAL_DEBUG] Bin {i} has no values, adding zeros'
                    )
                    avg_widths.append(0)
                ...
                logger.info(
                    f'[MARGINAL_DEBUG] Bin {i} stats: avg_width={avg_width:.4f}, reliable={below_count}, unreliable={above_count}'
                )
        ...
        logger.info(
            f'[MARGINAL_DEBUG] Result contains data for {len(result)} features: {list(result.keys())}'
        )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        results = super().run()
        logger.info(
            f'[RUN_DEBUG] Base results contain top-level keys: {list(results.keys())}'
        )
        ...
        if 'test_predictions' in results:
            ...
            logger.info(
                f"[RUN_DEBUG] Preserving test_predictions with shape: {test_predictions.shape if hasattr(test_predictions, 'shape') else 'N/A'}"
            )
        if 'test_labels' in results:
            test_labels = results['test_labels']
            logger.info(
                f"[RUN_DEBUG] Preserving test_labels with shape: {test_labels.shape if hasattr(test_labels, 'shape') else 'N/A'}"
            )

        # Check CRQR results
        if 'crqr' in results:
            logger.info(
                f"[RUN_DEBUG] CRQR results keys: {list(results['crqr'].keys())}"
            )
            if 'all_results' in results['crqr']:
                logger.info(
                    f"[RUN_DEBUG] Found {len(results['crqr']['all_results'])} CRQR test results"
                )

        # Additional enhancements for the overall results
        if (
            'crqr' in results
            and 'all_results' in results['crqr']
            and results['crqr']['all_results']
        ):
            # Collect interval widths across all tests
            all_widths = []
            for i, result in enumerate(results['crqr']['all_results']):
                logger.info(
                    f'[RUN_DEBUG] Checking result {i} keys: {list(result.keys())}'
                )
                if 'widths' in result:
                    widths_count = len(result['widths'])
                    logger.info(
                        f'[RUN_DEBUG] Result {i} has {widths_count} widths'
                    )
                    all_widths.extend(result['widths'])

            if all_widths:
                logger.info(
                    f'[RUN_DEBUG] Collected {len(all_widths)} total interval widths'
                )

                # Store them for the overall model
                results['interval_widths'] = {'primary_model': all_widths}
                ...
                logger.info(
                    f"[RUN_DEBUG] Width statistics: mean={results['mean_width']:.4f}, median={results['median_width']:.4f}"
                )

                # Add reliable/unreliable threshold
                threshold = (
                    np.median(all_widths) * self.reliable_threshold_ratio
                )
                results['threshold_value'] = float(threshold)
                logger.info(
                    f"[RUN_DEBUG] Set threshold_value: {results['threshold_value']}"
                )

                # Add coverage value
                if (
                    'avg_coverage' not in results
                    and results['crqr']['all_results']
                ):
                    coverages = [
                        r.get('coverage', 0)
                        for r in results['crqr']['all_results']
                    ]
                    if coverages:
                        ...
                        logger.info(
                            f"[RUN_DEBUG] Set coverage: {results['coverage']}"
                        )
                ...
                if (
                    'lower_bounds' in test_result
                    and 'upper_bounds' in test_result
                    and 'widths' in test_result
                ):
                    # Extract needed data
                    ...
                    results['reliability_analysis'] = reliability_analysis
                    logger.info(
                        f'[RUN_DEBUG] Added reliability_analysis with keys: {list(reliability_analysis.keys())}'
                    )

                    # Add marginal bandwidth analysis
                    marginal_bandwidth = self._analyze_marginal_bandwidth(
                        X, widths
                    )
                    results['marginal_bandwidth'] = marginal_bandwidth
                    logger.info(
                        f'[RUN_DEBUG] Added marginal_bandwidth for features: {list(marginal_bandwidth.keys())}'
                    )
                else:
                    logger.warning(
                        '[RUN_DEBUG] Missing required data in CRQR results for enhanced analysis'
                    )
            else:
                logger.warning(
                    '[RUN_DEBUG] No interval widths found in any CRQR result'
                )
        else:
        ...
        for key in distribution_keys:
            if key in results:
                ...
                if isinstance(results[key], dict):
                    logger.info(
                        f'[RUN_DEBUG] {key} has keys: {list(results[key].keys())}'
                    )
            else:
        ...
        logger.info(
            f'[RUN_DEBUG] Final results contain keys: {list(results.keys())}'
        )
        ...
        results['timestamp'] = datetime.datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S'
        )
            ...
            try:
                ...
                if self.uncertainty_models:
                    ...
                    (
                        lower_bound,
                        upper_bound,
                    ) = uncertainty_model.predict_interval(X)
        ...
        results['config'] = {
            'alpha': results['alpha_levels'][0]
            if 'alpha_levels' in results and results['alpha_levels']
            else 0.1,
        ...
        if test_predictions is not None:
            ...
            logger.info(
                f'[RUN_DEBUG] Re-added test_predictions to final results'
            )
        ...
        if 'primary_model' in results:
            logger.info(
                f"[RUN_DEBUG] primary_model exists, keys: {list(results['primary_model'].keys())[:10]}"
            )

            # Check if plot_data exists
            if 'plot_data' in results['primary_model']:
                logger.info(
                    f"[RUN_DEBUG] plot_data exists in primary_model with keys: {list(results['primary_model']['plot_data'].keys())}"
                )
            else:
            ...
            if 'reliability_analysis' in results:
                results['primary_model']['reliability_analysis'] = results.pop(
                    'reliability_analysis'
                )
                logger.info(
                    '[RUN_DEBUG] Moved reliability_analysis to primary_model'
                )
            if 'marginal_bandwidth' in results:
                results['primary_model']['marginal_bandwidth'] = results.pop(
                    'marginal_bandwidth'
                )
                logger.info(
                    '[RUN_DEBUG] Moved marginal_bandwidth to primary_model'
                )
            if 'interval_widths' in results:
                results['primary_model']['interval_widths'] = results.pop(
                    'interval_widths'
                )
                logger.info(
                    '[RUN_DEBUG] Moved interval_widths to primary_model'
                )

            # Ensure plot_data is preserved
            if (
                'plot_data' not in results['primary_model']
                and 'plot_data' in results
            ):
                results['primary_model']['plot_data'] = results['plot_data']
                logger.info('[RUN_DEBUG] Copied plot_data to primary_model')

            logger.info(
                f"[RUN_DEBUG] Final primary_model keys: {list(results['primary_model'].keys())[:15]}"
            )
...
def run_enhanced_uncertainty_tests(
    dataset, config_name='full', verbose=True, feature_subset=None
):
    """
    ...
    uncertainty = EnhancedUncertaintySuite(
        dataset, verbose=verbose, feature_subset=feature_subset
    )

    # Configure and run tests with feature subset if specified
    results = uncertainty.config(
        config_name, feature_subset=feature_subset
    ).run()

    if verbose:
        print(f'\nEnhanced Uncertainty Test Summary:')
        print(
            f"Overall uncertainty quality score: {results.get('uncertainty_quality_score', 0):.3f}"
        )
