...
class StaticRobustnessRenderer:
            ...
            if self._has_data(report_data, ['raw']):
                ...
                if chart_data['perturbation_levels']:
                    result = self.chart_registry.generate(
                        'perturbation_impact_static', chart_data
                    )
    ...
    def _prepare_perturbation_data(
            ...
            for level in levels:
                ...
                if (
                    ...
                    if 'all_features' in result:
                        ...
                        mean_scores.append(
                            result['all_features'].get('mean_score', 0)
                        )
                        std_scores.append(
                            result['all_features'].get('std_score', 0)
                        )
    ...
    def _prepare_feature_robustness_data(
        self, report_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _prepare_model_comparison_data(
        ...
        metrics = {'robustness': [report_data.get('robustness_score', 0)]}
    ...
    def _render_html(self, context: Dict[str, Any]) -> str:
        ...
        if not template_path:
            raise FileNotFoundError(
                f'No static template found for robustness report'
            )
