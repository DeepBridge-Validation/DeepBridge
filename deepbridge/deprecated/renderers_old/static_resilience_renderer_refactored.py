...
class StaticResilienceRenderer:
    ...
    def _generate_charts(
        ...
        try:
            ...
            if self._has_data(report_data, ['scenarios']):
                scenario_data = self._prepare_scenario_degradation_data(
                    report_data
                )
            ...
            if self._has_data(report_data, ['feature_distances']):
                ...
                if feature_data['features']:
                    ...
                    result = self.chart_registry.generate(
                        'feature_robustness', feature_data
                    )
                    ...
                    if result.is_success:
                        ...
                        logger.info(
                            'Generated feature distribution shift chart'
                        )

            # Chart 4: Model Comparison
            if self._has_data(
                report_data, ['alternative_models']
            ) and report_data.get('alternative_models'):
    ...
    def _prepare_test_type_data(
        ...
        return {'test_types': test_types, 'scores': scores}

    def _prepare_scenario_degradation_data(
        self, report_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for scenario_name, scenario_info in scenario_data.items():
            if isinstance(scenario_info, dict):
                ...
                performance.append(
                    scenario_info.get(
                        'performance', scenario_info.get('score', 0.0)
                    )
                )
    ...
    def _prepare_feature_shift_data(
        self, report_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for feature, distance in sorted_features[:10]:
            ...
            stability = (
                max(0, 1 - min(distance, 1))
                if isinstance(distance, (int, float))
                else 0.5
            )
            robustness_scores.append(stability)

        return {'features': features, 'robustness_scores': robustness_scores}
    ...
    def _prepare_model_comparison_data(
        ...
        for model_name, model_data in report_data.get(
            'alternative_models', {}
        ).items():
            models.append(model_name)
            metrics['resilience'].append(
                model_data.get('resilience_score', 0.0)
            )
    ...
    def _render_html(self, context: Dict[str, Any]) -> str:
        ...
        if not template_path:
            raise FileNotFoundError(
                f'No static template found for resilience report'
            )
