...
class ReliabilityPerformanceChart(BaseChartGenerator):
    ...
    def _calculate_performance_by_confidence(
        self,
        y_true: np.ndarray,
        y_prob: np.ndarray,
        ...
        for i, (bin_start, bin_end) in enumerate(
            ...
            if sample_counts[i] > 0:
                ...
                if metric == 'accuracy':
                    ...
                    precision = np.sum(
                        (y_true_bin == 1) & (y_pred_bin == 1)
                    ) / max(np.sum(y_pred_bin == 1), 1)
                    recall = np.sum(
                        (y_true_bin == 1) & (y_pred_bin == 1)
                    ) / max(np.sum(y_true_bin == 1), 1)
    ...
    def generate(
        ...
        if not models_data or not isinstance(models_data, dict):
            logger.warning(
                'Invalid models data for reliability performance chart'
            )
            return ''
        ...
        try:
            ...
            # Process each model
            for idx, (model_name, model_data) in enumerate(
                models_data.items()
            ):
                if 'y_true' not in model_data or 'y_prob' not in model_data:
                    logger.warning(
                        f'Missing required data for model {model_name}'
                    )
                    continue
                ...
                (
                    ...
                    performance_values,
                    sample_counts,
                ) = self._calculate_performance_by_confidence(
            ...
            ax1.set_title(
                f'{title} - {metric.capitalize()} by Confidence Level',
                fontsize=14,
                fontweight='bold',
            )
            ax1.set_xlim([48, 102])
            ...
            if all_bin_centers is not None:
                ...
                ax1.plot(
                    ...
                    alpha=0.3,
                    linewidth=1,
                    label='Expected (if perfectly calibrated)',
                )
            ...
            if show_sample_sizes:
                ...
                ax2.set_title(
                    'Sample Distribution across Confidence Bins', fontsize=12
                )
                ax2.set_xlim([48, 102])
    ...
    def generate_multi_metric(
        ...
        n_bins: int = 10,
        title: str = 'Multi-Metric Reliability Performance',
    ) -> str:
        """
        ...
        try:
            ...
            for metric_idx, metric in enumerate(metrics):
                ...
                for model_idx, (model_name, model_data) in enumerate(
                    ...
                    if (
                        'y_true' not in model_data
                        or 'y_prob' not in model_data
                    ):
                        continue
                ...
                ax.set_title(
                    f'{metric.capitalize()}', fontsize=11, fontweight='bold'
                )
                ax.set_xlim([48, 102])
                ax.set_ylim([0, 105])
            ...
            logger.error(
                f'Error generating multi-metric reliability performance: {str(e)}'
            )
            return ''
    ...
    def generate_from_intervals(
        ...
        models_data: Dict[str, Dict[str, Any]],
        n_bins: int = 10,
        title: str = 'Interval-based Reliability Performance',
    ) -> str:
        """
        ...
        try:
            ...
            for idx, (model_name, model_data) in enumerate(
                ...
                if max_width > min_width:
                    confidence = 1 - (interval_widths - min_width) / (
                        max_width - min_width
                    )
                else:
                ...
                ax2.plot(
                    bin_centers[valid_mask_mae] * 100,
            ...
            ax1.set_xlabel(
                'Confidence Level (based on interval width)', fontsize=12
            )
            ax1.set_ylabel('Coverage (%)', fontsize=12)
            ax1.set_title(
                'Coverage vs Confidence Level', fontsize=12, fontweight='bold'
            )
            ax1.set_xlim([-5, 105])
            ax1.set_ylim([0, 105])
            ax1.grid(True, alpha=0.3, linestyle='--')
            ...
            ax2.set_title(
                'Prediction Error vs Confidence Level',
                fontsize=12,
                fontweight='bold',
            )
            ax2.set_xlim([-5, 105])
            ...
            logger.error(
                f'Error generating interval-based reliability performance: {str(e)}'
            )
            return ''
