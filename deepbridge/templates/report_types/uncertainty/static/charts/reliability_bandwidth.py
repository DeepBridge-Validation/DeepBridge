...
class ReliabilityBandwidthChart(BaseChartGenerator):
    ...
    def _calculate_confidence_bands(
        ...
        upper_band = np.percentile(
            bootstrap_curves, (1 - alpha / 2) * 100, axis=0
        )
    ...
    def generate(
        ...
        if not models_data or not isinstance(models_data, dict):
            logger.warning(
                'Invalid models data for reliability bandwidth chart'
            )
            return ''
        ...
        try:
            ...
            for idx, (model_name, model_data) in enumerate(
                ...
                if len(np.unique(y_true)) > 2:
                    logger.warning(
                        f'Non-binary labels detected for model {model_name}'
                    )
                    continue
                ...
                (
                    ...
                    fraction_positives,
                    bin_edges,
                ) = self._calculate_calibration_curve(y_true, y_prob, n_bins)
                ...
                if (
                    show_confidence_bands and len(y_true) > 30
                ):  # Need enough samples
            ...
            ax1.set_ylabel(
                'Fraction of Positives (Observed Frequency)', fontsize=12
            )
            ...
            if show_histogram:
                ...
                ax2.set_title(
                    'Distribution of Predicted Probabilities', fontsize=12
                )
                ax2.set_xlim([0, 1])
    ...
    def generate_from_intervals(
        ...
        title: str = 'Reliability Bandwidth from Prediction Intervals',
    ) -> str:
        """
        ...
        try:
            ...
            for idx, (model_name, model_data) in enumerate(
                ...
                for alpha in alpha_levels:
                    if (
                        ...
                        and 'lower_bounds' in model_data
                        and 'upper_bounds' in model_data
                    ):
                        y_true = np.array(model_data['y_true'])
                        ...
                        logger.warning(
                            f'Missing interval data for model {model_name}'
                        )
                        break
            ...
            logger.error(
                f'Error generating reliability bandwidth from intervals: {str(e)}'
            )
            return ''
