...
try:
    ...
    fixed_radar_spec = importlib.util.find_spec(
        'deepbridge.templates.report_types.uncertainty.static.charts.fixed_radar'
    )
...
try:
    ...
    fixed_charts_spec = importlib.util.find_spec(
        'deepbridge.templates.report_types.uncertainty.static.charts.enhanced_charts_fixed'
    )
...
class UncertaintyChartGenerator:
    ...
    def __init__(self, seaborn_chart_generator=None):
        ...
        self.coverage_vs_expected = CoverageVsExpectedChart(
            seaborn_chart_generator
        )
        self.width_vs_coverage = WidthVsCoverageChart(seaborn_chart_generator)
        self.uncertainty_metrics = UncertaintyMetricsChart(
            seaborn_chart_generator
        )
        self.feature_importance = FeatureImportanceChart(
            seaborn_chart_generator
        )
        self.model_comparison = ModelComparisonChart(seaborn_chart_generator)
        self.performance_gap_by_alpha = PerformanceGapByAlphaChart(
            seaborn_chart_generator
        )

    # Wrapper methods to maintain backward compatibility

    def generate_coverage_vs_expected(
        self,
        models_data,
        ...
        if 'calibration_results' in models_data and isinstance(
            models_data['calibration_results'], dict
        ):
            # Log data structure for debugging
            if 'alpha_values' in models_data['calibration_results']:
                logger.info(
                    f"Alpha values for generate_coverage_vs_expected: {models_data['calibration_results']['alpha_values']}"
                )
            if 'coverage_values' in models_data['calibration_results']:
                logger.info(
                    f"Coverage values for generate_coverage_vs_expected: {models_data['calibration_results']['coverage_values']}"
                )
            if 'expected_coverages' in models_data['calibration_results']:
                logger.info(
                    f"Expected coverages for generate_coverage_vs_expected: {models_data['calibration_results']['expected_coverages']}"
                )

            # Reformat data for the chart generator
            formatted_data = {
                'Primary Model': {
                    'alphas': models_data['calibration_results'].get(
                        'alpha_values', []
                    ),
                    'coverages': models_data['calibration_results'].get(
                        'coverage_values', []
                    ),
                    'expected_coverages': models_data[
                        'calibration_results'
                    ].get('expected_coverages', []),
                }
            }
                ...
                for model_name, model_data in models_data[
                    ...
                    if 'calibration_results' in model_data:
                        formatted_data[model_name] = {
                            'alphas': model_data['calibration_results'].get(
                                'alpha_values', []
                            ),
                            'coverages': model_data['calibration_results'].get(
                                'coverage_values', []
                            ),
                            'expected_coverages': model_data[
                                'calibration_results'
                            ].get('expected_coverages', []),
                        }

            # Check if we have valid data
            if formatted_data and any(
                all(
                    key in model_data
                    for key in ['alphas', 'coverages', 'expected_coverages']
                )
                for model_data in formatted_data.values()
            ):
                logger.info(
                    f'Formatted data for coverage chart: {formatted_data}'
                )
                return self.coverage_vs_expected.generate(
                    formatted_data, title, add_annotations
                )
            else:
                logger.warning(
                    'No valid data found after formatting for coverage_vs_expected'
                )
                return None
        else:
    ...
    def generate_width_vs_coverage(
        self, models_data, title='Interval Width vs Coverage'
    ):
        ...
        if 'calibration_results' in models_data and isinstance(
            ...
            width_values = models_data['calibration_results'].get(
                'width_values', []
            )
            coverage_values = models_data['calibration_results'].get(
                'coverage_values', []
            )
            ...
            if (
                width_values
                and coverage_values
                and len(width_values) > 0
                and len(coverage_values) > 0
            ):
                logger.info(
                    f'Found calibration_results: {len(width_values)} width values, {len(coverage_values)} coverage values'
                )
                ...
                logger.warning(
                    f'calibration_results exists but data is empty or invalid: width_values={len(width_values) if width_values else 0}, coverage_values={len(coverage_values) if coverage_values else 0}'
                )
        ...
        if not formatted_data and 'coverage_vs_width' in models_data:
            ...
            logger.info(
                f'Found coverage_vs_width data: {cvw_data.keys() if isinstance(cvw_data, dict) else type(cvw_data)}'
            )

            if isinstance(cvw_data, dict):
                # Check for both 'mean_widths' and 'widths' keys
                widths = cvw_data.get(
                    'mean_widths', cvw_data.get('widths', [])
                )
                ...
                if (
                    ...
                    logger.info(
                        f'Using coverage_vs_width: {len(widths)} widths, {len(coverages)} coverages'
                    )
                    formatted_data['Primary Model'] = {
                        'widths': widths,
                        'coverages': coverages,
        ...
        if not formatted_data and 'plot_data' in models_data:
            ...
            if (
                isinstance(plot_data, dict)
                and 'coverage_vs_width' in plot_data
            ):
                ...
                if (
                    ...
                    logger.info(
                        f'Using plot_data.coverage_vs_width: {len(widths)} widths, {len(coverages)} coverages'
                    )
                    formatted_data['Primary Model'] = {
                        'widths': widths,
                        'coverages': coverages,
        ...
        if formatted_data and 'alternative_models' in models_data:
            for model_name, model_data in models_data[
                ...
                if 'calibration_results' in model_data:
                    width_values = model_data['calibration_results'].get(
                        'width_values', []
                    )
                    coverage_values = model_data['calibration_results'].get(
                        'coverage_values', []
                    )
        ...
        if formatted_data:
            logger.info(
                f'Generating width_vs_coverage chart with data for models: {list(formatted_data.keys())}'
            )
            return self.width_vs_coverage.generate(formatted_data, title)
        else:
            logger.warning(
                'No valid data found for width_vs_coverage chart - CRQR results may be missing or empty'
            )
            logger.warning(
                'Available keys in models_data: '
                + str(
    ...
    def generate_uncertainty_metrics(
        self, models_data, title='Uncertainty Metrics Comparison'
    ):
    ...
    def generate_model_comparison(self, models_data, title='Model Comparison'):
        ...
        logger.info(
            f'generate_model_comparison called with: models_data={type(models_data)}'
        )
        ...
        if 'alternative_models' in models_data and isinstance(
            models_data['alternative_models'], dict
        ):
            for model_name, model_data in models_data[
                'alternative_models'
            ].items():
        ...
        if has_primary:
            ...
            formatted_data[primary_name] = {
                ...
                'coverage': get_metric_value(
                    models_data, 'coverage', ['avg_coverage']
                ),
            ...
            logger.info(
                f"Added primary model '{primary_name}' with metrics: {formatted_data[primary_name]}"
            )
            ...
            for model_name, model_data in models_data[
                ...
                if has_metrics(model_data):
                    formatted_data[model_name] = {
                        ...
                        'coverage': get_metric_value(
                            model_data, 'coverage', ['avg_coverage']
                        ),
                        'mean_width': get_metric_value(
                            model_data,
                            'mean_width',
                    ...
                    logger.info(
                        f"Added alternative model '{model_name}' with metrics: {formatted_data[model_name]}"
                    )

        # Check if we have valid data
        if formatted_data:
            logger.info(
                f'Formatted data for model comparison: {formatted_data}'
            )
            # Use the standard metric names that the chart generator expects
            metrics = ['uncertainty_score', 'coverage', 'mean_width']
            try:
                return self.model_comparison.generate(
                    formatted_data, metrics, title
                )
            except Exception as e:
                logger.error(
    ...
    def generate_performance_gap_by_alpha(
        self,
        models_data,
        ...
        logger = logging.getLogger('deepbridge.reports')
        logger.info(
            f'generate_performance_gap_by_alpha called with: models_data={type(models_data)}, title={title}, add_annotations={add_annotations}'
        )
        ...
        if isinstance(models_data, dict):
            ...
            if 'calibration_results' in models_data:
                logger.info(
                    f"calibration_results keys: {list(models_data['calibration_results'].keys())}"
                )
                ...
                if isinstance(models_data['calibration_results'], dict):
                    ...
                    for key in [
                        ...
                        if key in models_data['calibration_results']:
                            ...
                            if hasattr(value, 'tolist') and callable(
                                getattr(value, 'tolist')
                            ):
                                models_data['calibration_results'][
                                    key
                                ] = value.tolist()
                            # Ensure non-list iterables are converted to lists
                            elif not isinstance(value, list) and hasattr(
                                value, '__iter__'
                            ):
                                try:
                                    models_data['calibration_results'][
                                    ...
                                    logger.warning(
                                        f'Failed to convert {key} to list: {str(e)}'
                                    )
            ...
            if 'alpha_levels' in models_data:
                ...
                if hasattr(models_data['alpha_levels'], 'tolist') and callable(
                    getattr(models_data['alpha_levels'], 'tolist')
                ):
                    models_data['alpha_levels'] = models_data[
                        'alpha_levels'
                    ].tolist()
                elif not isinstance(
                    models_data['alpha_levels'], list
                ) and hasattr(models_data['alpha_levels'], '__iter__'):
                    try:
                        models_data['alpha_levels'] = list(
                            models_data['alpha_levels']
                        )
                    except Exception as e:
                        logger.warning(
                            f'Failed to convert alpha_levels to list: {str(e)}'
                        )

        try:
            return self.performance_gap_by_alpha.generate(
                models_data, title, add_annotations
            )
        except Exception as e:
            logger.error(
