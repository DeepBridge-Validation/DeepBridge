...
class EnhancedUncertaintyCharts:
    ...
    def generate_reliability_distribution(
        self, data: Dict[str, Any], feature: Optional[str] = None
    ):
        """
        ...
        try:
            ...
            logger.info(
                f'[DISTRIBUTION_DEBUG] generate_reliability_distribution called with data keys: {list(data.keys())}'
            )

            if 'reliability_analysis' not in data:
                logger.error(
                    '[DISTRIBUTION_DEBUG] No reliability_analysis data found in input data'
                )
                # Log some of the top-level keys to help debug
                for key in list(data.keys())[:5]:  # First 5 keys
                    if isinstance(data[key], dict):
                        logger.info(
                            f"[DISTRIBUTION_DEBUG] data['{key}'] is a dict with keys: {list(data[key].keys())}"
                        )
                    else:
                        logger.info(
                            f"[DISTRIBUTION_DEBUG] data['{key}'] type: {type(data[key])}"
                        )
                return None
            ...
            logger.info(
                f'[DISTRIBUTION_DEBUG] reliability_analysis keys: {list(reliability.keys())}'
            )

            if 'feature_distributions' not in reliability:
                logger.error(
                    '[DISTRIBUTION_DEBUG] No feature_distributions found in reliability_analysis'
                )
                return None
            ...
            logger.info(
                f'[DISTRIBUTION_DEBUG] feature_distributions keys: {list(feature_distributions.keys())}'
            )
            ...
            for dist_type, features in feature_distributions.items():
                if features and isinstance(features, dict):
                    logger.info(
                        f"[DISTRIBUTION_DEBUG] Distribution type '{dist_type}' has {len(features)} features"
                    )
                    has_data = True
                    # Log a few feature names as example
                    feature_list = list(features.keys())
                    logger.info(
                        f'[DISTRIBUTION_DEBUG] Example features: {feature_list[:3]}'
                    )
                    ...
                    for fname in feature_list[:2]:  # Check first 2 features
                        ...
                        if isinstance(feature_values, (list, tuple)):
                            logger.info(
                                f"[DISTRIBUTION_DEBUG] Feature '{fname}' has {len(feature_values)} values"
                            )
                            if len(feature_values) > 0:
                                logger.info(
                                    f"[DISTRIBUTION_DEBUG] Feature '{fname}' sample values: {feature_values[:5]}"
                                )
                        else:
                            logger.info(
                                f"[DISTRIBUTION_DEBUG] Feature '{fname}' has non-list data: {type(feature_values)}"
                            )

            if not has_data:
                logger.error(
                    '[DISTRIBUTION_DEBUG] feature_distributions exists but contains no valid data'
                )
                return None
            ...
            if feature is None:
                available_features = list(
                    feature_distributions.get('reliable', {}).keys()
                )
            ...
            if feature not in feature_distributions.get(
                'reliable', {}
            ) or feature not in feature_distributions.get('unreliable', {}):
            ...
            ax.set_title(
                f'Distribution of {feature} for Reliable vs Unreliable Predictions'
            )
    ...
    def generate_marginal_bandwidth_chart(
        self, data: Dict[str, Any], feature: Optional[str] = None
    ):
        """
        ...
        try:
            ...
            logger.info(
                f'[BANDWIDTH_DEBUG] generate_marginal_bandwidth_chart called with data keys: {list(data.keys())}'
            )
            logger.info(f'[BANDWIDTH_DEBUG] Feature parameter: {feature}')

            if 'marginal_bandwidth' not in data:
                logger.error(
                    '[BANDWIDTH_DEBUG] No marginal_bandwidth data found in input data'
                )
                # Log some of the top-level keys to help debug
                for key in list(data.keys())[:5]:  # First 5 keys
                    if isinstance(data[key], dict):
                        logger.info(
                            f"[BANDWIDTH_DEBUG] data['{key}'] is a dict with keys: {list(data[key].keys())}"
                        )
                    else:
                        logger.info(
                            f"[BANDWIDTH_DEBUG] data['{key}'] type: {type(data[key])}"
                        )
                return None
            ...
            logger.info(
                f'[BANDWIDTH_DEBUG] marginal_bandwidth type: {type(marginal_bandwidth)}'
            )

            if not isinstance(marginal_bandwidth, dict):
                logger.error(
                    f'[BANDWIDTH_DEBUG] marginal_bandwidth is not a dictionary: {type(marginal_bandwidth)}'
                )
                return None

            logger.info(
                f'[BANDWIDTH_DEBUG] marginal_bandwidth keys: {list(marginal_bandwidth.keys())}'
            )
            ...
            if feature is None:
                ...
                if not available_features:
                    logger.error(
                        '[BANDWIDTH_DEBUG] No features found in marginal bandwidth data'
                    )
                    return None
                feature = available_features[0]
                logger.info(
                    f'[BANDWIDTH_DEBUG] Selected first available feature: {feature}'
                )

            # Check if feature exists
            if feature not in marginal_bandwidth:
                logger.error(
                    f'[BANDWIDTH_DEBUG] Feature {feature} not found in marginal bandwidth data'
                )
                return None
            ...
            logger.info(
                f'[BANDWIDTH_DEBUG] Feature data keys: {list(feature_data.keys())}'
            )
            ...
            missing_keys = [
                key for key in required_keys if key not in feature_data
            ]

            if missing_keys:
                logger.error(
                    f'[BANDWIDTH_DEBUG] Missing required data for feature {feature}: {missing_keys}'
                )
                return None
            ...
            for key in required_keys:
                if key in feature_data:
                    ...
                    if isinstance(value, (list, tuple, np.ndarray)):
                        logger.info(
                            f'[BANDWIDTH_DEBUG] {key} has {len(value)} values'
                        )
                        if len(value) > 0:
                            logger.info(
                                f'[BANDWIDTH_DEBUG] {key} sample values: {value[:3]}'
                            )
                    else:
                        logger.info(
                            f'[BANDWIDTH_DEBUG] {key} = {value} (type: {type(value)})'
                        )
            ...
            fig, (ax1, ax2) = plt.subplots(
                2, 1, figsize=(10, 10), height_ratios=[3, 1], sharex=True
            )
            ...
            width = (
                min(np.diff(bin_centers).min() * 0.8, 0.8)
            ...
            ax2.bar(
                ...
                counts_below,
                width,
                label='Reliable (Below Threshold)',
                color='green',
                alpha=0.7,
            )
            ax2.bar(
                ...
                width,
                bottom=counts_below,
                label='Unreliable (Above Threshold)',
                color='red',
                alpha=0.7,
            )
    ...
    def generate_interval_widths_boxplot(self, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f'[INTERVAL_DEBUG] generate_interval_widths_boxplot called with data keys: {list(data.keys())}'
            )

            if 'interval_widths' not in data:
                logger.error(
                    '[INTERVAL_DEBUG] No interval_widths data found in input data'
                )
                # Log some of the top-level keys to help debug
                for key in list(data.keys())[:5]:  # First 5 keys
                    if isinstance(data[key], dict):
                        logger.info(
                            f"[INTERVAL_DEBUG] data['{key}'] is a dict with keys: {list(data[key].keys())}"
                        )
                    else:
                        logger.info(
                            f"[INTERVAL_DEBUG] data['{key}'] type: {type(data[key])}"
                        )
                return None
            ...
            logger.info(
                f'[INTERVAL_DEBUG] interval_widths type: {type(interval_widths)}'
            )
            ...
            if isinstance(interval_widths, dict):
                logger.info(
                    f'[INTERVAL_DEBUG] interval_widths is a dictionary with keys: {list(interval_widths.keys())}'
                )
                # Sample check of values
                for model, widths in list(interval_widths.items())[
                    :2
                ]:  # First 2 models
                    if isinstance(widths, (list, tuple, np.ndarray)):
                        logger.info(
                            f"[INTERVAL_DEBUG] Model '{model}' has {len(widths)} width values"
                        )
                        if len(widths) > 0:
                            logger.info(
                                f"[INTERVAL_DEBUG] Model '{model}' sample values: {widths[:5]}"
                            )
                    else:
                        logger.info(
                            f"[INTERVAL_DEBUG] Model '{model}' has non-list data: {type(widths)}"
                        )
            elif isinstance(interval_widths, list):
                logger.info(
                    f'[INTERVAL_DEBUG] interval_widths is a list with {len(interval_widths)} elements'
                )
                # Check what's inside the list
                if len(interval_widths) > 0:
                    first_item = interval_widths[0]
                    logger.info(
                        f'[INTERVAL_DEBUG] First element type: {type(first_item)}'
                    )

                    if isinstance(first_item, dict):
                        logger.info(
                            f'[INTERVAL_DEBUG] First element is a dict with keys: {list(first_item.keys())}'
                        )
                        # Check if it's alpha-based data
                        if 'alpha' in first_item and 'widths' in first_item:
                            logger.info(
                                f'[INTERVAL_DEBUG] Alpha-based data detected'
                            )
                            logger.info(
                                f"[INTERVAL_DEBUG] First element: alpha={first_item['alpha']}, widths has {len(first_item['widths'])} values"
                            )
                    elif isinstance(first_item, (list, tuple, np.ndarray)):
                        logger.info(
                            f'[INTERVAL_DEBUG] First element is a list with {len(first_item)} values'
                        )
                        if len(first_item) > 0:
                            logger.info(
                                f'[INTERVAL_DEBUG] Sample values: {first_item[:5]}'
                            )
            else:
                logger.error(
                    f'[INTERVAL_DEBUG] interval_widths has unexpected type: {type(interval_widths)}'
                )
            ...
            if not boxplot_data:
                logger.error(
                    'Could not format interval widths data for boxplot'
                )
                return None
            ...
            if threshold is None and 'reliability_analysis' in data:
                threshold = data['reliability_analysis'].get(
    ...
    def generate_model_metrics_comparison(self, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.warning(
                'Fixed radar chart implementation not available, using fallback'
            )
        except Exception as e:
        ...
        try:
            ...
            for model_name, model_data in alt_models.items():
                metrics[model_name] = {
                    'Uncertainty Score': float(
                        model_data.get('uncertainty_score', 0)
                    ),
                    'Coverage': float(model_data.get('coverage', 0)),
                    'Mean Width': float(model_data.get('mean_width', 0)),
                    'MSE': float(
                        model_data.get('mse', 0) if 'mse' in model_data else 0
                    ),
                    'MAE': float(
                        model_data.get('mae', 0) if 'mae' in model_data else 0
                    ),
                }
            ...
            logger.info(
                f'Using metrics dictionary with {len(metrics)} models for radar chart'
            )

            # Create radar chart
            if self.base_generator and hasattr(
                self.base_generator, 'metrics_radar_chart'
            ):
                ...
                for model_name in df.index:
                    models_metrics[model_name] = {
                        'metrics': {
                            metric: float(df.loc[model_name, metric])
                ...
                try:
                    ...
                    logger.error(
                        f'Error generating radar chart: {str(radar_err)}'
                    )
                    logger.error(traceback.format_exc())
            ...
            logger.error(
                f'Error generating model metrics comparison chart: {str(e)}'
            )
            logger.error(traceback.format_exc())
            return None
