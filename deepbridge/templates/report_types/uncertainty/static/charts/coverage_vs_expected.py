...
class CoverageVsExpectedChart(BaseChartGenerator):
    ...
    def _validate_data(self, models_data):
        ...
        logger.info(
            f'Validating coverage vs expected data: {type(models_data)}'
        )
        ...
        for model_name, model_data in models_data.items():
            logger.info(
                f"Checking model data for '{model_name}': {type(model_data)}"
            )

            if not isinstance(model_data, dict):
                logger.warning(
                    f"Model data for '{model_name}' is not a dictionary"
                )
                continue

            # Log the keys in model_data
            logger.info(
                f"model_data keys for '{model_name}': {list(model_data.keys())}"
            )
            ...
            for key in required_keys:
                if key not in model_data:
                    logger.warning(
                        f"Missing required key '{key}' in model data for '{model_name}'"
                    )
            ...
            if all(key in model_data for key in required_keys):
                ...
                for key in required_keys:
                    if not isinstance(model_data[key], (list, tuple)):
                        logger.warning(
                            f"'{key}' for model '{model_name}' is not a list or tuple: {type(model_data[key])}"
                        )
                        is_valid = False
                    elif len(model_data[key]) == 0:
                        logger.warning(
                            f"'{key}' for model '{model_name}' is empty"
                        )
                        is_valid = False
                    else:
                        logger.info(
                            f"'{key}' for model '{model_name}' has {len(model_data[key])} values"
                        )
    ...
    def generate(
        ...
        if not self._validate_data(models_data):
            logger.warning(
                'Invalid models data for coverage vs expected coverage chart'
            )
            return ''
        ...
        if self.chart_generator and hasattr(
            ...
            try:
                ...
                logger.error(
                    f'Error using chart generator for coverage vs expected coverage: {str(e)}'
                )
        ...
        try:
            ...
            for i, (model_name, model_data) in enumerate(models_data.items()):
                ...
                min_len = min(
                    len(alphas), len(coverages), len(expected_coverages)
                )
                ...
                if (
                    add_annotations and i == 0
                ):  # Only annotate the first model to avoid clutter
            ...
            logger.error(
                f'Error generating coverage vs expected coverage chart: {str(e)}'
            )
            return ''
