...
class ModelComparisonScatterChart(BaseChartGenerator):
    ...
    def generate(
        ...
        if not self._validate_data(models_data):
            logger.warning(
                'Invalid or empty models data for comparison scatter plot'
            )
            return ''
        ...
        for model_name, model_info in models_data.items():
            ...
            if (
                'accuracy' not in model_info
                or 'resilience_score' not in model_info
            ):
                continue
        ...
        if self.chart_generator and hasattr(
            ...
            try:
                ...
                return self.chart_generator.scatter_plot(
                    data=scatter_data, title=title
                )
            except Exception as e:
                logger.error(
                    f'Error using chart generator for model comparison scatter: {str(e)}'
                )
        ...
        try:
            ...
            accuracy_values = [
                model_info['accuracy']
                for model_info in clean_models_data.values()
            ]
            resilience_values = [
                model_info['resilience_score']
                for model_info in clean_models_data.values()
            ]
            ...
            scatter_colors = [
                colors[i % len(colors)] for i in range(len(model_names))
            ]
