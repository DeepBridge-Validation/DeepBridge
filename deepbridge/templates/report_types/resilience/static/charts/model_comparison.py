...
class ModelComparisonChart(BaseChartGenerator):
    ...
    def generate(
        ...
        if not self._validate_data(
            perturbation_levels
        ) or not self._validate_data(models_data):
            logger.warning(
                'Invalid perturbation levels or models data for comparison chart'
            )
            return ''
        ...
        for model_name, model_info in models_data.items():
            ...
            if clean_scores:
                ...
                if 'base_score' in model_info:
                    try:
                        clean_model['base_score'] = float(
                            model_info['base_score']
                        )
                    except (ValueError, TypeError):
                        pass
            ...
            try:
                ...
                if hasattr(self.chart_generator, 'model_comparison_chart'):
                    ...
                    for model_name, model_info in clean_models_data.items():
                        if len(model_info.get('scores', [])) != len(
                            clean_perturbations
                        ):
                            ...
                            if len(model_info.get('scores', [])) < len(
                                ...
                                scores = model_info.get('scores', [])
                                padding = [None] * (
                                    len(clean_perturbations) - len(scores)
                                )
                                model_info['scores'] = scores + padding
                            # If scores are longer, truncate
                            else:
                                model_info['scores'] = model_info.get(
                                    'scores', []
                                )[: len(clean_perturbations)]
                ...
                logger.error(
                    f'Error using chart generator for model comparison: {str(e)}'
                )
        ...
        try:
            ...
            for i, (model_name, model_info) in enumerate(
                ...
                if base_score is not None:
                    ax.axhline(
                        y=base_score, color=color, linestyle='--', alpha=0.5
                    )
