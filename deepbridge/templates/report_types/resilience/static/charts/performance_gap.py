...
class PerformanceGapChart(BaseChartGenerator):
    ...
    def generate(
        ...
        if not self._validate_data(performance_metrics):
            logger.warning(
                'Invalid or empty performance metrics data for chart'
            )
            return ''
        ...
        for metric in metric_keys:
            ...
            if (
                worst_key in performance_metrics
                and remaining_key in performance_metrics
            ):
                try:
                    worst_val = float(performance_metrics[worst_key])
                    remaining_val = float(performance_metrics[remaining_key])
                    logger.info(
                        f'Found metric pair: {metric} - worst={worst_val}, remaining={remaining_val}'
                    )
                    ...
                    logger.warning(
                        f'Could not convert {metric} values to float: {e}'
                    )
                    continue
        ...
        if self.chart_generator and (metric_names and worst_values):
                ...
                try:
                    ...
                    if hasattr(self.chart_generator, 'model_metrics_heatmap'):
                        return self.chart_generator.model_metrics_heatmap(
                            results_df=performance_data, title=title
                        )
                    # Fall back to bar_chart for first metric if available
                    elif (
                        hasattr(self.chart_generator, 'bar_chart')
                        and len(metric_names) > 0
                    ):
                        bar_data = {
                    ...
                    logger.error(
                        f'Error using chart generator for performance gap: {str(e)}'
                    )
            # If we only have individual metrics
                ...
                try:
                    ...
                    return self.chart_generator.bar_chart(
                        data=bar_data, title='Performance Metrics'
                    )
                except Exception as e:
                    logger.error(
                        f'Error using chart generator for performance metrics: {str(e)}'
                    )
        ...
        try:
            if not metric_names:
                logger.warning(
                    'No matching metrics found for performance gap chart'
                )
                return ''
            ...
            if len(remaining_values) > 0:
                ...
                df_melted = self.pd.melt(
                    df,
                    id_vars=['Metric'],
                    value_vars=['Worst', 'Remaining'],
                    var_name='Sample Group',
                    value_name='Value',
                )

                # Create grouped bar chart
                bars = self.sns.barplot(
                    x='Metric',
                    y='Value',
                    hue='Sample Group',
                    data=df_melted,
                ...
                # Create a simple bar chart with available metrics
                df = self.pd.DataFrame(
                    {'Metric': metric_names, 'Value': worst_values}
                )
