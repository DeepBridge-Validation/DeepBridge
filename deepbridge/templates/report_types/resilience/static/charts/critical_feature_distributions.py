...
class CriticalFeatureDistributionsChart(BaseChartGenerator):
    ...
    def generate(
        ...
        title: str = 'Critical Feature Distributions',
    ) -> str:
        """
        ...
        if not self._validate_data(worst_samples) and not self._validate_data(
            remaining_samples
        ):
            logger.warning(
                'No valid sample data for critical feature distributions chart'
            )
            return ''
        ...
        if not self._validate_data(top_features):
            ...
            if not top_features:
                logger.warning(
                    'No feature names available for distributions chart'
                )
                return ''
        ...
        if self.chart_generator:
            ...
            for feature in features_to_plot:
                ...
                if (
                    self._validate_data(worst_samples)
                    and feature in worst_samples
                    and self._validate_data(worst_samples[feature])
                ):
                    try:
                ...
                if (
                    self._validate_data(remaining_samples)
                    and feature in remaining_samples
                    and self._validate_data(remaining_samples[feature])
                ):
                    try:
                        # Convert to list of floats if possible
            ...
            if boxplot_data:
                try:
                    ...
                    logger.error(
                        f'Error using chart generator for critical features: {str(e)}'
                    )
        ...
        try:
            ...
            n_features = sum(
                ...
                if (
                    f in worst_samples
                    and self._validate_data(worst_samples[f])
                )
                or (
                    f in remaining_samples
                    and self._validate_data(remaining_samples[f])
                )
            )

            if n_features == 0:
                logger.warning(
                    'No valid feature data found for distributions chart'
                )
                return ''
            ...
            for feature in features_to_plot:
                ...
                if worst_data or remaining_data:
                    ...
                    # Set labels
                    ax.set_title(
                        feature.replace('_', ' ').title(), fontsize=12
                    )
