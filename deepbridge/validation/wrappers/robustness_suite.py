...
from deepbridge.validation.robustness import OverfitAnalyzer, WeakspotDetector
...
class RobustnessSuite:
    ...
    def __init__(
        ...
        self.evaluator = RobustnessEvaluator(
            dataset, metric, verbose, random_state, n_iterations
        )
        ...
        if self.verbose:
            ...
            print(
                f'Using {self.n_iterations} iterations per perturbation level'
            )
    ...
    def config(
        ...
        if config_name not in self._CONFIG_TEMPLATES:
            raise ValueError(
                f'Unknown configuration: {config_name}. Available options: {list(self._CONFIG_TEMPLATES.keys())}'
            )

        # Clone the configuration template
        self.current_config = self._clone_config(
            self._CONFIG_TEMPLATES[config_name]
        )
    ...
    def run(
        ...
        if X is None or y is None:
            ...
            if (
                hasattr(self.dataset, 'test_data')
                and self.dataset.test_data is not None
            ):
                X = self.dataset.get_feature_data('test')
                y = self.dataset.get_target_data('test')
            else:
                raise ValueError(
                    'No test data available in dataset. Please provide X and y.'
                )
        ...
        try:
            model_feature_importance = (
                self.evaluator.get_model_feature_importance()
            )

            if model_feature_importance:
                if self.verbose:
                    print(
                        f'Model feature importance detected with {len(model_feature_importance)} features'
                    )
                    top_features = sorted(
                ...
                if self.verbose:
                    print(
                        'WARNING: Model does not provide native feature importance'
                    )
        ...
        for test_idx, test_config in enumerate(self.current_config, 1):
            ...
            test_feature_subset = params.get(
            ...
            if test_feature_subset:
                ...
                eval_results[
                    'all_features'
                ] = self.evaluator.evaluate_perturbation(
            ...
            if test_type == 'raw':
                if level_key not in results['raw']['by_level']:
                    results['raw']['by_level'][level_key] = {
                        'runs': {},
                        'overall_result': {},
                    }
                ...
                if (
                    'all_features'
                    not in results['raw']['by_level'][level_key]['runs']
                ):
                    results['raw']['by_level'][level_key]['runs'][
                        'all_features'
                    ] = []
                if (
                    'feature_subset'
                ...
                for run_key, eval_result in eval_results.items():
                    ...
                    mean_score = np.mean(
                        [run['perturbed_score'] for run in runs]
                    )
                    std_score = np.std(
                        [run['perturbed_score'] for run in runs]
                    )
                    ...
                    results['raw']['by_level'][level_key]['overall_result'][
                        ...
                        'worst_score': np.mean(worst_scores)
                        if worst_scores
                        else 0,
                    }
            ...
            # Process results for quantile method
                ...
                if level_key not in results['quantile']['by_level']:
                    results['quantile']['by_level'][level_key] = {
                        'runs': {},
                        'overall_result': {},
                    }
                ...
                if (
                    'all_features'
                    not in results['quantile']['by_level'][level_key]['runs']
                ):
                    results['quantile']['by_level'][level_key]['runs'][
                        'all_features'
                    ] = []
                if (
                    'feature_subset'
        ...
        if raw_levels:
            ...
            try:
                ...
                if self.verbose:
                    print(
                        f'Calculating feature importance using raw perturbation at level {median_level}'
                    )
                ...
                if feature_importance:
                    if self.verbose:
                        print(
                            f'Feature importance calculation successful - found {len(feature_importance)} features'
                        )
                        ...
                        print(
                            f'Top 5 features by robustness importance: {top_features}'
                        )

                    results['feature_importance'] = feature_importance
                else:
                    print(
                        'WARNING: Feature importance calculation returned empty results'
                    )
                    # Initialize with empty dictionary to avoid None
        ...
        results['avg_raw_impact'] = (
            np.mean(all_raw_impacts) if all_raw_impacts else 0
        )
        results['avg_quantile_impact'] = (
            np.mean(all_quantile_impacts) if all_quantile_impacts else 0
        )
        results['avg_overall_impact'] = (
            np.mean(all_impacts) if all_impacts else 0
        )
        ...
        if self.verbose:
            ...
            print(
                f"Average quantile impact: {results['avg_quantile_impact']:.3f}"
            )
            print(
                f"Overall average impact: {results['avg_overall_impact']:.3f}"
            )
    ...
    def compare(
        self,
        alternative_models: Dict[str, Any],
        ...
        # Store results
        all_results = {'primary_model': primary_results}

        # Add the model_type to the primary_model results
        if hasattr(self.dataset, 'model'):
            all_results['primary_model']['model_type'] = type(
                self.dataset.model
            ).__name__
    ...
    def save_report(
        ...
        if not self.results:
            raise ValueError(
                'No results to generate report from. Run tests first.'
            )
        ...
        try:
            report_path = report_manager.generate_robustness_report(
                self.results, output_path, model_name
            )
    ...
    def get_visualizations(self) -> Dict[str, Any]:
        ...
        print(
            'Please use save_report() to generate a full interactive HTML report.'
        )
    ...
    def run_weakspot_detection(
        ...
        if X is None or y is None:
            if (
                ...
                raise ValueError(
                    'No test data available. Please provide X and y.'
                )
        ...
        if self.verbose:
            ...
            print('=' * 70)
            print(
                f'Analyzing {X.shape[0]} samples across {X.shape[1]} features'
            )
    ...
    def run_overfitting_analysis(
        ...
        if X_train is None or y_train is None:
            if (
                hasattr(self.dataset, 'train_data')
                and self.dataset.train_data is not None
            ):
                X_train = self.dataset.get_feature_data('train')
                y_train = self.dataset.get_target_data('train')
            else:
                raise ValueError(
                    'No training data available. Please provide X_train and y_train.'
                )
        ...
        if X_test is None or y_test is None:
            if (
                ...
                raise ValueError(
                    'No test data available. Please provide X_test and y_test.'
                )
        ...
        if metric_func is None:
            ...
            if hasattr(self.dataset, 'experiment_type'):
                ...
                if 'classification' in exp_type.lower():
                    ...
                    metric_func = lambda y_true, y_pred: roc_auc_score(
                        y_true, y_pred
                    )
                    if self.verbose:
                    ...
                    metric_func = lambda y_true, y_pred: r2_score(
                        y_true, y_pred
                    )
                    if self.verbose:
        ...
        if slice_features is None:
            ...
            slice_features = X_train.select_dtypes(
                include=[np.number]
            ).columns.tolist()
            if self.verbose:
                print(
                    f'No features specified, analyzing all {len(slice_features)} numeric features'
                )
        ...
        if isinstance(slice_features, list) and len(slice_features) > 1:
            ...
            if self.verbose:
                print(
                    f'Analyzing overfitting across {len(slice_features)} features...'
                )
    ...
    def update_model_name(
        self, original_results: Dict, model_type: str
    ) -> Dict:
        """
        ...
        for key, value in results.items():
            if isinstance(value, dict):
                ...
                if (
                    'model_name' in value
                    and value['model_name'] == 'primary_model'
                ):
                ...
                for sub_key, sub_value in value.items():
                    if isinstance(sub_value, dict):
                        ...
                        if (
                            'model_name' in sub_value
                            and sub_value['model_name'] == 'primary_model'
                        ):
