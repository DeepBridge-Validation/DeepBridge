...
class DataPerturber:
    ...
    def perturb_data(
        ...
        for feature in perturb_features:
            if isinstance(X, pd.DataFrame):
                ...
                if isinstance(feature, int) and (
                    feature < 0 or feature >= X.shape[1]
                ):
                    continue
                col = feature
    ...
    def _apply_raw_perturbation(
        ...
        feature_values = (
            X.iloc[:, col] if isinstance(X, pd.DataFrame) else X[:, col]
        )
    ...
    def _apply_quantile_perturbation(
        self,
        X: Union[pd.DataFrame, np.ndarray],
        X_perturbed: Union[pd.DataFrame, np.ndarray],
        col: Union[int, str],
    ...
    def perturb_features_individually(
        self,
        X: Union[pd.DataFrame, np.ndarray],
        perturb_method: str,
        level: float,
