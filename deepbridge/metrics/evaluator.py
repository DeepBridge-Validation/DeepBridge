...
class MetricsEvaluator:
        ...
        try:
            ...
            if metric not in self.results_df.columns:
                ...
                self.config.log_info(
                    f"Metric '{metric}' not found. Available metrics: {available_metrics}"
                )
            ...
            self.config.log_info(
                f'Best model configuration based on {metric}:'
            )
            ...
            self.config.log_info(
                f'Error finding best model for {metric}: {str(e)}'
            )
            import traceback
    ...
    def get_valid_results(
        self, metric: Optional[str] = 'test_accuracy'
    ) -> pd.DataFrame:
        """
        ...
        try:
            if metric:
                if metric not in self.results_df.columns:
                    self.config.log_info(
                        f'Metric {metric} not found in results. Available columns: {self.results_df.columns.tolist()}'
                    )
                    return pd.DataFrame()
    ...
    def get_model_comparison_metrics(self) -> pd.DataFrame:
        ...
        try:
            ...
            if valid_results.empty:
                self.config.log_info(
                    'No valid results for model comparison metrics'
                )
                return pd.DataFrame()
            ...
            for metric_col, (avg_name, max_name) in metrics_map.items():
                if (
                    metric_col in valid_results.columns
                    and not valid_results[metric_col].isna().all()
                ):
                    available_metrics[metric_col] = (avg_name, max_name)

            self.config.log_info(
                f'Available metrics for comparison: {list(available_metrics.keys())}'
            )

            # If no metrics available, return empty DataFrame
            if not available_metrics:
                self.config.log_info(
                    'No metrics available for model comparison'
                )
                return pd.DataFrame()

            for model in model_types:
                model_data = valid_results[
                    valid_results['model_type'] == model
                ]
                ...
                for metric_col, (
                    ...
                    if metric_col in [
                        ...
                        model_metric[max_or_min_name] = model_data[
                            metric_col
                        ].min()
                    else:
            ...
            self.config.log_info(
                f'Created comparison metrics with columns: {result_df.columns.tolist()}'
            )
            ...
            self.config.log_info(
                f'Error in get_model_comparison_metrics: {str(e)}'
            )
            import traceback
    ...
    def get_factor_impact(self, factor: str) -> pd.DataFrame:
        ...
        try:
            ...
            if valid_results.empty:
                self.config.log_info(
                    f'No valid results for {factor} impact analysis'
                )
                return pd.DataFrame()
            ...
            if factor not in valid_results.columns:
                self.config.log_info(
                    f"Factor '{factor}' not found in results columns: {valid_results.columns.tolist()}"
                )
                return pd.DataFrame()
            ...
            available_metrics = [
                col for col in metrics_columns if col in valid_results.columns
            ]

            if not available_metrics:
                self.config.log_info(
                    'No metrics columns available for factor impact analysis'
                )
                return pd.DataFrame()
            ...
            try:
                impact = (
                    valid_results.groupby(['model_type', factor])[
                        available_metrics
                    ]
                    .agg('mean')
                    .reset_index()
                ...
                self.config.log_info(
                    f'Created impact analysis with metrics: {available_metrics}'
                )
                return impact
            except Exception as e:
            ...
            self.config.log_info(
                f'Error in get_factor_impact for {factor}: {str(e)}'
            )
            import traceback
    ...
    def get_available_metrics(self) -> List[str]:
        ...
        try:
            if self.results_df is None or self.results_df.empty:
                self.config.log_info(
                    'No results available for metrics analysis'
                )
                return []
            ...
            for metric in base_metrics:
                ...
                if (
                    test_metric in self.results_df.columns
                    and not self.results_df[test_metric].isna().all()
                ):
