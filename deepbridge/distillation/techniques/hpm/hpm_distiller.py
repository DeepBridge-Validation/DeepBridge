...
class HPMDistiller(BaseDistiller):
    ...
    def _initialize_components(self):
        ...
        self.shared_memory = SharedOptimizationMemory(
            cache_size=100, similarity_threshold=0.8
        )
    ...
    def fit(
        ...
        configs = self._select_configurations(
            model_types, temperatures, alphas, dataset_features
        )
        ...
        if self.config.use_parallel:
            parallel_results = self._run_parallel_training(
                configs, X_train, y_train, X_val, y_val, teacher_probs
            )
        else:
        ...
        successful_models = [
            r for r in parallel_results if r.success and r.model is not None
        ]
        if self.config.use_multi_teacher and len(successful_models) > 0:
            ensemble_model = self._create_multi_teacher_ensemble(
                parallel_results, X_train, y_train, X_val, y_val
            )
        else:
            if self.config.use_multi_teacher and len(successful_models) == 0:
                logger.warning(
                    'Multi-teacher ensemble disabled: no successful models trained'
                )
            ensemble_model = None

        # Select best model
        self._select_best_model(
            parallel_results, chain_results, ensemble_model
        )
        ...
        logger.info(
            f'HPM distillation completed in {self.total_time:.2f} seconds'
        )
        ...
        for i, config in enumerate(configs):
            try:
                ...
                result = WorkloadResult(
                    ...
                    training_time=result_metrics.get('training_time', 0.0)
                    if result_metrics
                    else 0.0,
                )
                ...
                if result.success:
                    ...
                    logger.warning(
                        f'Failed to train config_{i}: {result.error_message}'
                    )
    ...
    def _create_multi_teacher_ensemble(
        ...
        if X_val is not None and y_val is not None:
            fused_predictions, weights = self.multi_teacher.adaptive_fusion(
                X_val, y_val, optimize_weights=True
            )
    ...
    def _select_best_model(
        ...
        logger.info(
            f'Best model from {best_source} with score {best_score:.4f}'
        )
    ...
    def predict(self, X: Union[np.ndarray, pd.DataFrame]) -> np.ndarray:
        ...
        if not self.is_fitted:
            raise ValueError(
                'Distiller must be fitted before making predictions'
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'temp_scheduler': self.temp_scheduler.get_stats()
            if self.temp_scheduler
            else None,
        }
