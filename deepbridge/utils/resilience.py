...
def run_resilience_tests(
    ...
    resilience = ResilienceSuite(
        dataset, verbose=verbose, metric=metric, feature_subset=feature_subset
    )

    # Configure and run tests with feature subset if specified
    results = resilience.config(
    ...
    if verbose:
        ...
        print(
            f"Overall resilience score: {results.get('resilience_score', 0):.3f}"
        )

        # Print alpha-specific results
        for alpha, alpha_data in sorted(
            results.get('distribution_shift', {}).get('by_alpha', {}).items()
        ):
            print(
                f"Alpha = {alpha}: Average performance gap: {alpha_data.get('avg_performance_gap', 0):.3f}"
            )
...
def compare_models_resilience(results_dict):
    ...
    for model_name, results in results_dict.items():
        ...
        comparison['resilience_scores'].append(
            results.get('resilience_score', 0)
        )

        # Collect performance gaps by alpha
        for alpha, alpha_data in (
            results.get('distribution_shift', {}).get('by_alpha', {}).items()
        ):
        ...
        for dm, dm_data in (
            results.get('distribution_shift', {})
            ...
            for feature, value in dm_data.get('top_features', {}).items():
                ...
                comparison['feature_importance'][dm][feature].append(
                    {'model': model_name, 'importance': value}
                )
