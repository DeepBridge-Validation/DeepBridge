...
class DataValidator:
    ...
    def validate_data_input(
        ...
        if data is not None and (
            train_data is not None or test_data is not None
        ):
            raise ValueError(
                'Cannot provide both data and train/test data. Choose one option.'
            )
        ...
        if target_column is None:
            ...
            if (
                ...
                or isinstance(train_data, pd.DataFrame)
                or isinstance(test_data, pd.DataFrame)
            ):
                raise ValueError(
                    'target_column must be provided when using DataFrame inputs'
                )
        ...
        if not isinstance(data, pd.DataFrame):
            if hasattr(data, 'feature_names'):
                if features is None:
                    ...
                    if missing_features:
                        raise ValueError(
                            f'Features {missing_features} not found in data'
                        )
                    return features
