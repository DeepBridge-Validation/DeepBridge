...
class StandardProcessor(BaseProcessor):
    ...
    def process(
        self, data: t.Union[pd.DataFrame, np.ndarray], **kwargs
        ...
        if categorical_columns:
            result = self._process_categorical(
                result, categorical_columns, fit
            )
    ...
    def _process_numerical(
        self, data: pd.DataFrame, columns: t.List[str], fit: bool
    ) -> pd.DataFrame:
        """
        Process numerical columns.
    ...
    def _infer_column_types(
        ...
        for col in columns:
            ...
            # Detect numeric columns
                ...
                if (
                    unique_count <= 10
                ):  # Arbitrary threshold for categorical integers
                    categorical_columns.append(col)
                else:
