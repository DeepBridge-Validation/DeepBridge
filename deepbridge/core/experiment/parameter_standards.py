...
class ParameterNames:
    ...
    CONFIG_NAME = (
        'config_name'  # Consistently use config_name instead of suite
    )
...
UNCERTAINTY_CONFIGS = {
    ConfigName.QUICK.value: [
        {
            'method': 'crqr',
            'params': {'alpha': 0.1, 'test_size': 0.3, 'calib_ratio': 1 / 3},
        },
        {
            'method': 'crqr',
            'params': {'alpha': 0.2, 'test_size': 0.3, 'calib_ratio': 1 / 3},
        },
    ],
    ConfigName.MEDIUM.value: [
        {
            'method': 'crqr',
            'params': {'alpha': 0.05, 'test_size': 0.3, 'calib_ratio': 1 / 3},
        },
        {
            'method': 'crqr',
            'params': {'alpha': 0.1, 'test_size': 0.3, 'calib_ratio': 1 / 3},
    ...
    ConfigName.FULL.value: [
        {
            'method': 'crqr',
            'params': {'alpha': 0.01, 'test_size': 0.3, 'calib_ratio': 1 / 3},
        },
        {
            'method': 'crqr',
            'params': {'alpha': 0.05, 'test_size': 0.3, 'calib_ratio': 1 / 3},
        ...
        {
            'method': 'crqr',
            'params': {'alpha': 0.3, 'test_size': 0.3, 'calib_ratio': 1 / 3},
        },
    ],
}
...
def get_test_config(test_type: str, config_name: str) -> t.Dict[str, t.Any]:
    ...
    if not is_valid_test_type(test_type):
        raise ValueError(
            f'Invalid test type: {test_type}. Valid options: {get_test_types()}'
        )

    if not is_valid_config_name(config_name):
        raise ValueError(
            f'Invalid configuration name: {config_name}. Valid options: {get_config_names()}'
        )

    if test_type not in TEST_CONFIGS:
        raise ValueError(
            f'No configuration template defined for test type: {test_type}'
        )

    test_config = TEST_CONFIGS[test_type]
    if config_name not in test_config:
        raise ValueError(
            f'No {config_name} configuration defined for test type: {test_type}'
        )
