...
class ModelEvaluation:
    ...
    def evaluate_distillation(self, model, dataset, X, y, prob=None):
        ...
        student_prob_pos = (
            y_prob[:, 1] if y_prob.shape[1] > 1 else student_probs
        )
        ...
        if prob is not None:
            ...
            if isinstance(prob, pd.DataFrame):
                if 'prob_class_1' in prob.columns:
                    logger.debug(
                        f"Using 'prob_class_1' column from teacher probabilities"
                    )
                    ...
                    if 'prob_class_0' in prob.columns:
                        logger.debug(
                            f"Found 'prob_class_0' column in teacher probabilities"
                        )
                        teacher_probs = prob[
                        ...
                        logger.debug(
                            f"Calculating 'prob_class_0' as (1 - prob_class_1)"
                        )
                        # Calcula prob_class_0 como 1 - prob_class_1
                        teacher_probs = np.column_stack(
                            [1 - teacher_prob_pos, teacher_prob_pos]
                        )
                else:
                    # Assume the last column is the probability of the positive class
                    logger.debug(
                        f'Using last column as positive class probability'
                    )
            ...
            logger.debug(
                f"Teacher probabilities shape: {teacher_probs.shape if hasattr(teacher_probs, 'shape') else 'unknown'}"
            )
            logger.debug(
                f'First 3 teacher probabilities (positive class): {teacher_prob_pos[:3]}'
            )

            # Calculate distribution comparison metrics
            ks_stat, ks_pvalue, r2 = self._calculate_distribution_metrics(
                teacher_prob_pos, student_prob_pos
            )
        else:
            logger.debug(
                f'No teacher probabilities available for {dataset} dataset'
            )
        ...
        self._add_distribution_metrics(
            metrics, teacher_prob_pos, student_prob_pos, ks_stat, ks_pvalue, r2
        )
        ...
        metrics['distillation_method'] = getattr(
            model, '__class__', 'unknown'
        ).__name__

        # Include predictions
        predictions_df = pd.DataFrame(
            {'y_true': y, 'y_pred': y_pred, 'y_prob': student_prob_pos}
        )
    ...
    def _add_distribution_metrics(
        self, metrics, teacher_probs, student_probs, ks_stat, ks_pvalue, r2
    ):
        ...
        if 'kl_divergence' not in metrics and teacher_probs is not None:
            try:
                ...
                kl_div_pos = np.mean(
                    teacher_prob_pos
                    * np.log(teacher_prob_pos / student_prob_pos)
                )
                kl_div_neg = np.mean(
                    teacher_prob_neg
                    * np.log(teacher_prob_neg / student_prob_neg)
                )
        ...
        if distilled_model is not None:
            try:
                ...
                metrics['model_name'] = (
                    'Distilled_'
                    + getattr(
                        distilled_model, '__class__', type(distilled_model)
                    ).__name__
                )
