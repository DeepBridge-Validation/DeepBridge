...
class ReportManager:
    ...
    def __init__(self, templates_dir: Optional[str] = None):
        ...
        from .renderers.resilience_renderer_simple import (
            ResilienceRendererSimple,
        )
        from .renderers.robustness_renderer_simple import (
            RobustnessRendererSimple,
        )
        from .renderers.uncertainty_renderer_simple import (
            UncertaintyRendererSimple,
        )

        # Import static renderers
        try:
            from .renderers.static import (
            ...
            logger.warning(
                'Static renderers not available, will use interactive renderers for all reports'
            )
            self.has_static_renderers = False

        # Set up renderers for different report types
        self.renderers = {
            'robustness': RobustnessRendererSimple(
                self.template_manager, self.asset_manager
            ),  # Using simple renderer (supports advanced tests)
            'uncertainty': UncertaintyRendererSimple(
                self.template_manager, self.asset_manager
            ),  # Using NEW simple renderer
            'resilience': ResilienceRendererSimple(
                self.template_manager, self.asset_manager
            ),  # Using NEW simple renderer
            'hyperparameter': HyperparameterRenderer(
                self.template_manager, self.asset_manager
            ),
            'hyperparameters': HyperparameterRenderer(
                self.template_manager, self.asset_manager
            ),
            'fairness': FairnessRendererSimple(
                self.template_manager, self.asset_manager
            ),
        }
        ...
        if self.has_static_renderers:
            self.static_renderers = {
                ...
                'resilience': StaticResilienceRenderer(
                    self.template_manager, self.asset_manager
                )
    ...
    def generate_report(
        ...
        if report_type_lower not in ['interactive', 'static']:
            logger.warning(
                f"Invalid report_type '{report_type}', defaulting to 'interactive'"
            )
        ...
        if report_type_lower == 'static' and self.has_static_renderers:
            if test_type_lower in self.static_renderers:
                ...
                logger.warning(
                    f'Static renderer for {test_type} is not implemented, falling back to interactive renderer'
                )
                if test_type_lower not in self.renderers:
                    raise NotImplementedError(
                        f"Report generation for test type '{test_type}' is not implemented"
                    )
        ...
        try:
            ...
            report_path = renderer.render(
                results, file_path, model_name, report_type_lower, save_chart
            )
            logger.info(
                f'Report generated and saved to: {report_path} (type: {report_type_lower})'
            )
            return report_path
        except Exception as e:
            logger.error(f'Error generating {test_type} report: {str(e)}')
            raise ValueError(
                f'Failed to generate {test_type} report: {str(e)}'
            )
