        ...
        try:
            ...
            if not template_path:
                raise FileNotFoundError(
                    f'No static template found for robustness report in: {template_paths}'
                )
            ...
            context = self.base_renderer._create_static_context(
                report_data, 'robustness', css_content
            )
            ...
            try:
                ...
                context[
                    'charts'
                ] = {}  # Empty charts to prevent template errors
            ...
            # Se não encontrar, tenta buscar no modelo primário
                ...
                if 'features' in report_data['primary_model'] and isinstance(
                    report_data['primary_model']['features'], list
                ):
                    all_features = report_data['primary_model']['features']
                elif 'feature_importance' in report_data[
                    'primary_model'
                ] and isinstance(
                    report_data['primary_model']['feature_importance'], dict
                ):
                    all_features = list(
                        report_data['primary_model'][
                            'feature_importance'
                        ].keys()
                    )
            ...
            if not metrics and 'primary_model' in report_data:
                if 'metrics' in report_data['primary_model']:
                    ...
                    logger.info(
                        f'Using metrics from primary_model with keys: {list(metrics.keys())}'
                    )

            # Log de métricas
            logger.info(
                f"Available metrics: {list(metrics.keys()) if isinstance(metrics, dict) else 'None'}"
            )
            logger.info(
                f"Available metrics_details: {list(metrics_details.keys()) if isinstance(metrics_details, dict) else 'None'}"
            )
            ...
            if 'alternative_models' in report_data:
                for model_name, model_data in report_data[
                    ...
                    if 'metrics' in model_data:
                        logger.info(
                            f"Model {model_name} has metrics: {list(model_data['metrics'].keys()) if isinstance(model_data['metrics'], dict) else 'None'}"
                        )
            ...
            context.update(
                {
                    ...
                    'resilience_score': float(robustness_score)
                    if robustness_score is not None
                    else 0.0,  # Backward compatibility
                    'raw_impact': float(report_data.get('raw_impact', 0))
                    if report_data.get('raw_impact') is not None
                    else 0.0,
                    'quantile_impact': float(
                        report_data.get('quantile_impact', 0)
                    )
                    if report_data.get('quantile_impact') is not None
                    else 0.0,
                    'base_score': float(report_data.get('base_score', 0))
                    if report_data.get('base_score') is not None
                    else 0.0,
            ...
            try:
                ...
                logger.error(
                    f'Error rendering template: {str(template_error)}'
                )
                import traceback

                logger.error(f'Traceback: {traceback.format_exc()}')
                # Generate a simplified fallback report
                rendered_html = self._generate_fallback_report(
            ...
            logger.error(
                f'Error generating static robustness report: {str(e)}'
            )
            try:
                # Last resort fallback
                fallback_html = self._generate_fallback_report(
                    results, model_name
                )
                return self.base_renderer._write_report(
                    fallback_html, file_path
                )
                ...
                raise ValueError(
                    f'Failed to generate static robustness report: {str(e)}'
                )
        ...
        if isinstance(data, dict):
            ...
            if 'primary_model' in data:
                ...
                if isinstance(primary_model, dict):
                    if 'base_score' in primary_model and isinstance(
                        primary_model['base_score'], (int, float)
                    ):
                        base_score = primary_model['base_score']
                    if 'robustness_score' in primary_model and isinstance(
                        primary_model['robustness_score'], (int, float)
                    ):
        ...
        try:
            ...
            if 'raw' in report_data and 'by_level' in report_data['raw']:
                    ...
                    if (
                        ...
                        if 'feature_subset' in result:
                            feature_subset_scores.append(
                                result['feature_subset'].get(
                                    'mean_score', None
                                )
                            )
                            # Also extract worst scores for feature subset
                            feature_subset_worst_scores.append(
                                result['feature_subset'].get(
                                    'min_score',
                                    result['feature_subset'].get(
                                        'worst_score', None
                            ...
                            feature_subset_impact = result[
                                'feature_subset'
                            ].get('impact', 0)
                            if (
                                feature_subset_impact
                                > max_feature_subset_impact
                            ):
                                max_feature_subset_impact = (
                                    feature_subset_impact
                                )
                            ...
                            logger.info(
                                f'Level {level_str} feature subset impact: {feature_subset_impact}'
                            )
                        else:
                ...
                if has_feature_subset:
                    logger.info(
                        f'Max feature subset impact: {max_feature_subset_impact} at level {max_feature_subset_impact_level}'
                    )
            ...
            if perturbation_levels and perturbed_scores:
                ...
                subset_data = (
                    feature_subset_scores if has_feature_subset else None
                )

                # Generate average performance chart
                charts[
                    'overview_chart'
                ] = self.chart_generator.robustness_overview_chart(
                ...
                if worst_scores and any(
                    score is not None for score in worst_scores
                ):
                    subset_worst_data = (
                    ...
                    charts[
                        'worst_performance_chart'
                    ] = self.chart_generator.worst_performance_chart(
                        perturbation_levels=perturbation_levels,
            ...
            if (
                ...
                if primary_model_scores:
                    models_data[
                        report_data.get('model_name', 'Primary Model')
                    ] = {
                ...
                for model_name, model_data in report_data[
                    ...
                    if 'raw' in model_data and 'by_level' in model_data['raw']:
                        ...
                        for level in perturbation_levels:
                            level_str = str(level)
                            if (
                                level_str in alt_raw_data
                                and 'overall_result' in alt_raw_data[level_str]
                            ):
                                result = alt_raw_data[level_str][
                                    'overall_result'
                                ]
                ...
                if len(models_data) > 1:
                    charts[
                        'comparison_chart'
                    ] = self.chart_generator.model_comparison_chart(
                        perturbation_levels=perturbation_levels,
                        models_data=models_data,
                        metric_name=report_data.get('metric', 'Score'),
                    )
                    logger.info(
                        f'Generated model comparison chart with {len(models_data)} models'
                    )
            ...
            if feature_importance:
                ...
                logger.info(
                    f'Generated feature importance chart with {len(feature_importance)} features'
                )
            ...
            if 'iterations_by_level' in report_data:
                for level_scores in report_data[
                    'iterations_by_level'
                ].values():
            ...
            if (
                'alternative_models' in report_data
                and 'alt_iterations_by_level' in report_data
            ):
                for model_name, model_data in report_data[
                    'alternative_models'
                ].items():
            ...
            if (
                'initial_results' in report_data
                and 'models' in report_data['initial_results']
            ):
                for model_id, model_data in report_data['initial_results'][
                    'models'
                ].items():
                    ...
                    if (
                        ...
                        and model_data['evaluation_results']['scores']
                    ):
            ...
            if boxplot_models:
                ...
                logger.info(
                    f'Generated boxplot chart with {len(boxplot_models)} models'
                )
