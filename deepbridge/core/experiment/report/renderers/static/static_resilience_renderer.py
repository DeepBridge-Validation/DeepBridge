...
class StaticResilienceRenderer:
    ...
    def __init__(self, template_manager, asset_manager):
        ...
        try:
            ...
            if self.resilience_chart_generator:
                resilience_methods = [
                    method
                    for method in dir(self.resilience_chart_generator)
                    if callable(
                        getattr(self.resilience_chart_generator, method)
                    )
                    and not method.startswith('_')
                ]
                logger.info(
                    f'Available resilience chart methods: {resilience_methods}'
                )
            ...
            if hasattr(
                ...
                try:
                    self.resilience_chart_generator._validate_chart_generator()
                    logger.info(
                        'ResilienceChartGenerator validation successful'
                    )
        ...
        try:
            ...
            try:
                ...
                raise FileNotFoundError(
                    f'No static template found for resilience report: {str(e)}'
                )
            ...
            logger.info(
                'Transforming resilience data with standard transformer'
            )
            report_data = self.data_transformer.transform(results, model_name)
            logger.info(
                f'Standard transformation complete, top-level keys: {list(report_data.keys() if isinstance(report_data, dict) else [])}'
            )

            # Then apply additional transformations for static reports using the dedicated transformer
            from ...transformers.static.static_resilience import (
            ...
            transformed_data = static_transformer.transform(
                results, model_name
            )
            report_data = transformed_data
            logger.info('Applied static transformations to report data')
            logger.info(
                f'Static transformation result keys: {list(report_data.keys() if isinstance(report_data, dict) else [])}'
            )
            ...
            if (
                ...
                and isinstance(report_data['charts'], dict)
                and report_data['charts']
            ):
                # Use charts from transformer
                logger.info(
                    f"Using {len(report_data['charts'])} charts from transformer"
                )
                charts = report_data['charts']
            else:
                # Generate charts with real data from test results
                logger.info(
                    'Generating charts using real data from test results'
                )
            ...
            context = self.base_renderer._create_static_context(
                report_data, 'resilience', css_content
            )
            ...
            context['avg_performance_gap'] = report_data.get(
                'avg_performance_gap'
            )
            context['metric'] = report_data.get('metric', 'Score')
            context['timestamp'] = report_data.get(
                'timestamp',
                datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            )
            context['model_type'] = report_data.get(
            ...
            if 'sensitive_features' in report_data:
                context['sensitive_features'] = report_data[
                    'sensitive_features'
                ]
            ...
            if 'most_affected_scenario' in report_data:
                context['most_affected_scenario'] = report_data[
            ...
            logger.info(
                f"Template variables: resilience_score={context.get('resilience_score')}, avg_performance_gap={context.get('avg_performance_gap')}, metric={context.get('metric')}"
            )
            ...
            logger.error(
                f'Error generating static resilience report: {str(e)}'
            )
            logger.error(f'Traceback: {traceback.format_exc()}')
            raise

    def _save_charts_as_png(
    ...
    def _generate_charts(self, report_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        logger.info(
            f'Input data for chart generation has keys: {list(report_data.keys())}'
        )
        ...
        for key in [
            ...
            if key in report_data:
                ...
                if isinstance(report_data[key], dict):
                    logger.info(
                        f"  '{key}' is a dictionary with {len(report_data[key])} entries"
                    )
                elif isinstance(report_data[key], list):
                    logger.info(
                        f"  '{key}' is a list with {len(report_data[key])} items"
                    )
            else:
        ...
        try:
            ...
            logger.info(
                f'Resilience score data: score={resilience_score}, performance_gap={performance_gap}'
            )
            ...
            if resilience_score is not None or performance_gap is not None:
                ...
                if resilience_score is not None:
                    try:
                        ...
                        logger.info(
                            f'Added resilience score to chart: {float(resilience_score)}'
                        )
                    except (ValueError, TypeError) as e:
                        logger.error(
                            f'Could not convert resilience score to float: {e}'
                        )
                ...
                if performance_gap is not None:
                    try:
                        ...
                        logger.info(
                            f'Added performance gap to chart: {float(performance_gap)}'
                        )
                    except (ValueError, TypeError) as e:
                        logger.error(
                            f'Could not convert performance gap to float: {e}'
                        )
                ...
                if not x_values or not y_values:
                    logger.error(
                        'No valid data for resilience score chart after conversion'
                    )
                else:
                    ...
                    try:
                        ...
                        if result:
                            ...
                            logger.info(
                                f'Generated resilience score chart, size: {chart_size} bytes'
                            )
                        else:
                            logger.error('Chart generated but result is empty')
                    except Exception as e:
                        logger.error(
                            f'Error during chart_generator.bar_chart call: {str(e)}'
                        )
                        logger.error(f'Traceback: {traceback.format_exc()}')
            else:
                logger.warning(
                    'No resilience score or performance gap data available for chart'
                )
        except Exception as e:
        ...
        try:
            ...
            logger.info(
                f'Distribution shift data present: {distribution_shift is not None}'
            )
                ...
                if not feature_distances:
                    ...
                    if isinstance(distribution_shift, dict):
                        ...
                        if 'feature_distances' in distribution_shift:
                            ...
                            logger.info(
                                f'Found feature_distances in distribution_shift'
                            )
                        # Try to find distances by key pattern
                        for key in distribution_shift.keys():
                            if (
                                'feature' in key.lower()
                                and 'distance' in key.lower()
                                and isinstance(distribution_shift[key], dict)
                            ):
                                feature_distances = distribution_shift[key]
                                logger.info(
                                    f'Using {key} as feature_distances'
                                )
                                break

            if feature_distances and isinstance(feature_distances, dict):
                logger.info(
                    f'Feature distances data has {len(feature_distances)} entries'
                )
                # Log a sample of feature distances
                sample = {
                    k: v
                    for i, (k, v) in enumerate(feature_distances.items())
                    if i < 3
                }
                ...
                if self.resilience_chart_generator:
                    try:
                        logger.info(
                            'Calling resilience_chart_generator.generate_feature_distribution_shift'
                        )
                        ...
                        if result:
                            ...
                            logger.info(
                                f'Successfully generated feature distribution shift chart, size: {len(result)} bytes'
                            )
                        else:
                            logger.error(
                ...
                logger.warning(
                    'No valid feature distances data for feature distribution shift chart'
                )
        except Exception as e:
            logger.error(
        ...
        try:
            ...
            if not performance_metrics:
                for key in ['metrics', 'evaluation_metrics', 'results']:
                    if key in report_data and isinstance(
                        report_data[key], dict
                    ):
                        logger.info(
                            f"Checking '{key}' for performance metrics"
                        )
                        metrics_data = report_data[key]
                        # Look for worst_ and remaining_ prefixes in keys
                        has_worst = any(
                            k.startswith('worst_') for k in metrics_data.keys()
                        )
                        has_remaining = any(
                            k.startswith('remaining_')
                            for k in metrics_data.keys()
                        )
                        if has_worst or has_remaining:
                            performance_metrics = metrics_data
                            logger.info(
                                f"Found performance metrics in '{key}'"
                            )
                            break
            ...
            if performance_metrics and isinstance(performance_metrics, dict):
                ...
                if paired_metrics:
                    if self.resilience_chart_generator:
                        try:
                            logger.info(
                                'Calling resilience_chart_generator.generate_performance_gap'
                            )
                            ...
                            if result:
                                ...
                                logger.info(
                                    f'Successfully generated performance gap chart, size: {len(result)} bytes'
                                )
                            else:
                                logger.error(
                        ...
                        logger.error(
                            'resilience_chart_generator is not available'
                        )
                else:
                    logger.warning(
                        'No paired performance metrics found (worst_X and remaining_X)'
                    )
            else:
        ...
        try:
            ...
            if feature_importance and (worst_samples or remaining_samples):
                ...
                if self.resilience_chart_generator:
                    ...
                    ] = self.resilience_chart_generator.generate_critical_feature_distributions(
                    ...
                    logger.info(
                        'Generated critical feature distributions chart'
                    )
        except Exception as e:
            logger.error(
        ...
        try:
            ...
            if feature_correlations and isinstance(feature_correlations, dict):
                if self.resilience_chart_generator:
                    ...
                    ] = self.resilience_chart_generator.generate_feature_residual_correlation(
                        feature_correlations=feature_correlations,
                        title='Feature-Residual Correlation',
                        top_n=8,
                    )
                    logger.info(
                        'Generated feature-residual correlation heatmap'
                    )
        except Exception as e:
            logger.error(
        ...
        try:
            ...
            if residuals or worst_residuals or remaining_residuals:
                if self.resilience_chart_generator:
                    ...
                    ] = self.resilience_chart_generator.generate_residual_distribution(
        ...
        try:
            ...
            if feature_importance and isinstance(feature_importance, dict):
                ...
                ] = self.chart_generator.feature_importance_chart(
                ...
                if model_feature_importance and isinstance(
                    ...
                    ] = self.chart_generator.feature_comparison_chart(
            ...
            if (
                ...
                if primary_alpha_levels and primary_scores:
                    ...
                    for model_name, model_data in alternative_models.items():
                        if 'by_alpha' in model_data and isinstance(
                            model_data['by_alpha'], dict
                        ):
                            model_alpha_levels = []
                            model_scores = []

                            for alpha_str, alpha_data in sorted(
                                model_data['by_alpha'].items()
                            ):
                                try:
                                    alpha = float(alpha_str)
                                    score = alpha_data.get('score')
                    ...
                    if len(models_data) > 1:
                        if self.resilience_chart_generator:
                            ...
                            ] = self.resilience_chart_generator.generate_model_comparison(
        ...
        try:
            ...
            if len(models_accuracy_resilience) >= 2:
                if self.resilience_chart_generator:
                    try:
                        logger.info(
                            'Calling generate_model_comparison_scatter'
                        )
        ...
        try:
            ...
            if primary_resilience_score is not None:
                try:
                    models_resilience[primary_model_name] = float(
            ...
            if alternative_models and isinstance(alternative_models, dict):
                for model_name, model_data in alternative_models.items():
                    ...
                    if alt_resilience_score is not None:
                        try:
                            models_resilience[model_name] = float(
                                alt_resilience_score
                            )
                        except (ValueError, TypeError):
                            continue
        ...
        try:
            ...
            if alpha_levels:
                ...
                def extract_performance_data(results):
                    ...
                    for result in sorted(
                        results, key=lambda x: x.get('alpha', 0)
                    ):
                        if (
                    ...
                    if 'by_distance_metric' in dist_shift and isinstance(
                        ...
                        for metric_data in dist_shift[
                            ...
                            if (
                                ...
                                if results and isinstance(results, list):
                                    primary_perf_data = (
                                        extract_performance_data(results)
                                    )
                ...
                if alternative_models and isinstance(alternative_models, dict):
                    for model_name, model_data in alternative_models.items():
                        ...
                        if 'distribution_shift' in model_data and isinstance(
                            model_data['distribution_shift'], dict
                        ):
                            alt_dist_shift = model_data['distribution_shift']

                            if (
                                'by_distance_metric' in alt_dist_shift
                                and isinstance(
                                    alt_dist_shift['by_distance_metric'], dict
                                )
                            ):
                                # Use the first metric with results
                                for metric_data in alt_dist_shift[
                                    'by_distance_metric'
                                ].values():
                                    if (
                                        isinstance(metric_data, dict)
                ...
                if models_perf_data:
                    if self.resilience_chart_generator:
                        try:
                            logger.info(
                                'Calling generate_performance_gap_by_alpha'
                            )
                            ...
                            if gap_chart:
                                ...
                                logger.info(
                                    f'Generated performance gap by alpha chart: {len(gap_chart)} bytes'
                                )
                            else:
                                logger.error(
                                    'generate_performance_gap_by_alpha returned empty result'
                                )
                        except Exception as inner_e:
                            logger.error(
                                f'Error in generate_performance_gap_by_alpha: {str(inner_e)}'
                            )
                            logger.error(
                                f'Traceback: {traceback.format_exc()}'
                            )
                    ...
                    logger.warning(
                        'No models with performance data available for performance gap chart'
                    )
            else:
                logger.warning(
                    'No alpha levels available for performance gap chart'
                )
        except Exception as e:
            logger.error(
        ...
        try:
            ...
            if primary_resilience_score is not None:
                models_scores_data[primary_model_name] = {
                    ...
                    'performance_gap': report_data.get(
                    ...
                    if isinstance(model_data, dict):
                        ...
                        if model_score is not None:
                            models_scores_data[model_name] = {
                                'score': model_score,
                                'performance_gap': model_data.get(
                                    'avg_performance_gap', 0
                                ),
                            }
            ...
            if models_scores_data and self.resilience_chart_generator:
                try:
                    logger.info(
                        f'Calling generate_model_resilience_scores with {len(models_scores_data)} models'
                    )
                    ...
                    if scores_chart:
                        ...
                        logger.info(
                            f'Generated model resilience scores chart: {len(scores_chart)} bytes'
                        )
                    else:
                        logger.error(
                            'generate_model_resilience_scores returned empty result'
                ...
                if not models_scores_data:
                    logger.warning(
                        'No models data available for resilience scores chart'
                    )
                if not self.resilience_chart_generator:
                    logger.error('resilience_chart_generator is not available')
        except Exception as e:
            logger.error(
                f'Error generating model resilience scores chart: {str(e)}'
            )
            logger.error(f'Traceback: {traceback.format_exc()}')
        ...
        try:
            ...
            if (
                by_alpha
                and isinstance(by_alpha, dict)
                and 'performance_gap_by_alpha' not in charts
            ):
                ...
                for alpha_str in sorted(by_alpha.keys()):
                    ...
                    if isinstance(alpha_data, dict):
                        try:
                            ...
                            gap = alpha_data.get(
                                'performance_gap',
                                alpha_data.get('remaining_score', 0)
                                - alpha_data.get('worst_score', 0),
                            )
                ...
                if (
                    ...
                    and self.resilience_chart_generator
                ):
            ...
            logger.error(
                f'Error ensuring performance gap by alpha chart: {str(e)}'
            )
