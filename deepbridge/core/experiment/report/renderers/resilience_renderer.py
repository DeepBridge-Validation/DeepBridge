    ...
    def _process_js_content(self, content: str, file_path: str) -> str:
        ...
        try:
            ...
            for line in lines:
                ...
                if line_stripped.startswith('const ') and any(
                    pattern in file_path
                    for pattern in ['Controller', 'Manager']
                ):
                    component_name = (
                        line_stripped.split('const ')[1].split(' =')[0].strip()
                    )
        ...
        try:
            ...
            if not template_path:
                raise FileNotFoundError(
                    f'No template found for resilience report in: {template_paths}'
                )
            ...
            if (
                ...
                and 'by_distance_metric' in report_data['distribution_shift']
            ):
                dist_values = []
                for dm, dm_data in report_data['distribution_shift'][
                    'by_distance_metric'
                ].items():
                    for feature, val in dm_data.get(
            ...
            if (
                ...
                if all_results:
                    ...
                    max_result = max(
                        all_results,
                        key=lambda x: x.get('performance_gap', 0)
                        if isinstance(
                            x.get('performance_gap', 0), (int, float)
                        )
                        else 0,
                    )
                    ...
                    if 'alpha' in max_result:
                        scenario_components.append(
                            f"{int(max_result['alpha'] * 100)}% shift"
                        )
                    if 'distance_metric' in max_result:
                        scenario_components.append(
                            f"{max_result['distance_metric']} metric"
                        )
                    if scenario_components:
                        most_affected_scenario = ' with '.join(
                            scenario_components
                        )
                    else:
            ...
            if (
                ...
                for result in report_data['distribution_shift']['all_results']:
                    if (
                        ...
                        if (
                            result.get('performance_gap') is not None
                            and result.get('alpha') is not None
                        ):
                            sensitivity = abs(result['performance_gap']) / (
                                result['alpha'] * 100
                            )
                            sensitivity_values.append(sensitivity)
                if sensitivity_values:
                    outlier_sensitivity = sum(sensitivity_values) / len(
                        sensitivity_values
                    )
            ...
            if 'dataset_info' in report_data:
                if 'baseline_name' in report_data['dataset_info']:
                    baseline_dataset = report_data['dataset_info'][
                        'baseline_name'
                    ]
            ...
            if (
                ...
                for result in report_data['distribution_shift']['all_results']:
                    ...
                    if isinstance(worst_metric, float) and math.isnan(
                        worst_metric
                    ):
                        worst_metric = None

                    remaining_metric = result.get('remaining_metric', 0)
                    if isinstance(remaining_metric, float) and math.isnan(
                        remaining_metric
                    ):
                        remaining_metric = None

                    performance_gap = result.get('performance_gap', 0)
                    if isinstance(performance_gap, float) and math.isnan(
                        performance_gap
                    ):
                    ...
                    scenario = {
                        'name': result.get(
                            'name', f'Scenario {len(shift_scenarios) + 1}'
                        ),
                        'alpha': result.get('alpha', 0),
                        'metric': result.get('metric', 'unknown'),
                        'distance_metric': result.get(
            ...
            if (
                ...
                for dm, dm_data in report_data['distribution_shift'][
                    ...
                    for feature, value in dm_data.get(
                        'top_features', {}
                    ).items():
            ...
            context = self.base_renderer._create_context(
                report_data, 'resilience', css_content, js_content, report_type
            )
            ...
            context.update(
                {
                    ...
                    'most_affected_scenario': most_affected_scenario
                    or 'No scenario data',
                    # Lists and collections with empty defaults
                    'distance_metrics': report_data.get(
                        'distance_metrics', []
                    ),
                    'distribution_shift_results': report_data.get(
                    ...
                    # Metadata
                    'resilience_module_version': report_data.get(
                        'module_version', '1.0'
                    ),
