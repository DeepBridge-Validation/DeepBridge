...
class FileDiscoveryManager:
        ...
        if report_type:
            ...
            if report_type_lower in ['interactive', 'static']:
                ...
                if os.path.exists(priority_path):
                    logger.info(
                        f'Using CSS path for {report_type_lower} report: {priority_path}'
                    )
    ...
    def find_js_path(self, test_type: str, report_type: str = None) -> str:
        ...
        if report_type:
            ...
            if report_type_lower in ['interactive', 'static']:
                ...
                if os.path.exists(priority_path):
                    logger.info(
                        f'Using JS path for {report_type_lower} report: {priority_path}'
                    )
                    return priority_path
    ...
    def get_common_html_files(self) -> Dict[str, str]:
        ...
        if not os.path.exists(self.asset_manager.common_dir):
            logger.warning(
                f'Common directory not found: {self.asset_manager.common_dir}'
            )
            return {}
        ...
        for file_name in os.listdir(self.asset_manager.common_dir):
            if file_name.endswith('.html'):
                file_path = os.path.join(
                    self.asset_manager.common_dir, file_name
                )
                try:
                    ...
                    logger.warning(
                        f'Error reading HTML file {file_path}: {str(e)}'
                    )
    ...
    def get_test_partials(self, test_type: str) -> Dict[str, str]:
        ...
        if not os.path.exists(partials_dir):
            logger.warning(
                f'Partials directory not found for {test_type}: {partials_dir}'
            )
            return {}
        ...
        for file_name in os.listdir(partials_dir):
            if file_name.endswith('.html'):
                ...
                try:
                    ...
                    logger.warning(
                        f'Error reading partial file {file_path}: {str(e)}'
                    )
    ...
    def _discover_css_files(self, css_dir: str) -> Dict[str, str]:
        ...
        if 'main' not in discovered_files:
            for file_name in os.listdir(css_dir):
                if file_name.endswith('.css') and (
                    'main' in file_name.lower() or 'style' in file_name.lower()
                ):
                    discovered_files['main'] = file_name
                    logger.info(
                        f'Found alternative main CSS file: {file_name}'
                    )
                    break
        ...
        if os.path.exists(components_dir) and os.path.isdir(components_dir):
            ...
            for file_name in os.listdir(components_dir):
                if file_name.endswith('.css'):
                    ...
                    logger.info(
                        f'Found component CSS: {component_name} -> {rel_path}'
                    )
        ...
        for file_name in os.listdir(css_dir):
            if (
                ...
                and file_name not in main_css_candidates
            ):
                file_base = os.path.splitext(file_name)[0]
                if file_base not in discovered_files:
                    discovered_files[file_base] = file_name
                    logger.info(
                        f'Found additional CSS file in root: {file_base} -> {file_name}'
                    )
    ...
    def _discover_js_files(self, js_dir: str) -> Dict[str, str]:
        ...
        subdirs = {'charts': False, 'controllers': False, 'components': False}
            ...
            if os.path.exists(subdir_path) and os.path.isdir(subdir_path):
                ...
                for file_name in os.listdir(subdir_path):
                    if file_name.endswith('.js'):
                        ...
                        logger.info(
                            f'Found JS file: {logical_name} -> {rel_path}'
                        )
        ...
        for file_name in os.listdir(js_dir):
            if (
                ...
                and file_name != 'utils.js'
            ):
                file_base = os.path.splitext(file_name)[0]
                if file_base not in discovered_files:
                    discovered_files[file_base] = file_name
                    logger.info(
                        f'Found additional JS file in root: {file_base} -> {file_name}'
                    )

        return discovered_files

    def write_json_data_file(
        self,
        data: Dict[str, Any],
        output_dir: str,
        filename: str = 'dados_transformados.json',
    ) -> str:
        """
    ...
    def copy_assets_to_output(
        self, test_type: str, output_dir: str
    ) -> Dict[str, str]:
        """
        ...
        try:
            if os.path.exists(self.asset_manager.favicon_path):
                favicon_output_path = os.path.join(
                    images_output_dir, 'favicon.ico'
                )
                self._copy_file(
                    self.asset_manager.favicon_path, favicon_output_path
                )
            ...
            if os.path.exists(icons_dir):
                ...
                for file_name in os.listdir(icons_dir):
                    if file_name.endswith(
                        ...
                        icon_output_path = os.path.join(
                            icons_output_dir, file_name
                        )
                        self._copy_file(icon_path, icon_output_path)
                        logger.info(
                            f'Icon {file_name} copied to: {icon_output_path}'
                        )
    ...
    def _copy_file(self, src_path: str, dest_path: str) -> None:
        ...
        try:
            ...
            logger.error(
                f'Error copying file from {src_path} to {dest_path}: {str(e)}'
            )
