    ...
    def list_templates(self, pattern: Optional[str] = None) -> list:
        ...
        Args:
            pattern: Optional glob pattern to filter templates.
                    Pattern matching respects path separators. For example:
                    - "*.html" matches only top-level HTML files
                    - "**/*.html" matches HTML files at any depth
                    - "robustness/*.html" matches HTML files in robustness directory
        ...
        if pattern:
            ...
            # If pattern doesn't contain '/', it should only match files in root
            if '/' not in pattern and not pattern.startswith('**'):
                templates = [t for t in templates if '/' not in t and fnmatch.fnmatch(t, pattern)]
            else:
