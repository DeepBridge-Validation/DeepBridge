...
class FairnessDataTransformer:
    ...
    def transform(
        ...
        transformed = {
            ...
            # Test configuration
            'test_config': self._transform_test_config(
                config, age_grouping_applied
            ),
            ...
            'metadata': {
                ...
                'total_pretrain_metrics': sum(
                    len(m) for m in pretrain_metrics.values()
                ),
                'total_posttrain_metrics': sum(
                    len(m) for m in posttrain_metrics.values()
                ),
        ...
        logger.info(
            f'Transformation complete. {len(protected_attrs)} protected attributes analyzed'
        )
        return transformed
                ...
                for metric_name, metric_result in pretrain[attr].items():
                    if isinstance(metric_result, dict):
                        metric_data = {
                            ...
                            'interpretation': metric_result.get(
                                'interpretation', ''
                            ),
                ...
                for metric_name, metric_result in posttrain[attr].items():
                    if isinstance(metric_result, dict):
                        ...
                        if 'disparity' in metric_result:
                            metric_data['disparity'] = metric_result.get(
                                'disparity'
                            )
                        ...
                        if 'testable_groups' in metric_result:
                            metric_data['testable_groups'] = metric_result[
                                'testable_groups'
                            ]
                        if 'excluded_groups' in metric_result:
                            metric_data['excluded_groups'] = metric_result[
                                'excluded_groups'
                            ]
                        if 'min_representation_pct' in metric_result:
                            metric_data[
                                'min_representation_pct'
                            ] = metric_result['min_representation_pct']
                        ...
                        if metric_name in POSTTRAIN_MAIN_METRICS:
                            ...
                            attr_data['posttrain_complementary'].append(
                                metric_data
                            )
                        else:
    ...
    def _transform_issues(
        self, warnings: List[str], critical: List[str]
    ) -> Dict[str, List]:
    ...
    def _transform_dataset_info(self, dataset_info: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'protected_attributes_distribution': dataset_info.get(
                'protected_attributes_distribution', {}
            ),
        }

    def _transform_test_config(
        self, config: Dict, age_grouping_applied: Dict
    ) -> Dict[str, Any]:
        """
        ...
        transformed_config = {
            ...
            'confusion_matrix_enabled': config.get(
                'include_confusion_matrix', False
            ),
            'threshold_analysis_enabled': config.get(
