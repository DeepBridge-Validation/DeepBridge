...
class ChartFactory:
    ...
    def __init__(self):
        ...
        # Distribution charts
        self.protected_attrs_distribution = (
            ProtectedAttributesDistributionChart()
        )
    ...
    def create_all_charts(self, results: Dict[str, Any]) -> Dict[str, str]:
        ...
        if confusion_matrix:
            try:
                ...
                logger.warning(
                    f'Failed to create confusion_matrices chart: {e}'
                )
        ...
        if threshold_analysis_data:
            try:
                ...
                logger.warning(
                    f'Failed to create threshold_analysis chart: {e}'
                )

        # Distribution charts
        if dataset_info and dataset_info.get(
            'protected_attributes_distribution'
        ):
            try:
                charts[
                    'protected_attributes_distribution'
                ] = self.protected_attrs_distribution.create(
                    {
                        'protected_attrs_distribution': dataset_info.get(
                            'protected_attributes_distribution', {}
                        ),
                        'protected_attrs': protected_attrs,
                    }
                ...
                logger.warning(
                    f'Failed to create protected_attributes_distribution chart: {e}'
                )
        ...
        if dataset_info and dataset_info.get('target_distribution'):
            try:
                charts[
                    'target_distribution'
                ] = self.target_distribution.create(
                    {
                        'target_distribution': dataset_info.get(
                            'target_distribution', {}
                        )
                    }
                )
                ...
                logger.warning(
                    f'Failed to create target_distribution chart: {e}'
                )
        ...
        if posttrain_metrics:
            try:
                charts[
                    'posttrain_disparate_impact_gauge'
                ] = self.disparate_impact_gauge.create(
                    {
                        'posttrain_metrics': posttrain_metrics,
                        'protected_attrs': protected_attrs,
                    }
                ...
                logger.warning(
                    f'Failed to create disparate_impact_gauge chart: {e}'
                )

            try:
                charts[
                    'posttrain_disparity_comparison'
                ] = self.disparity_comparison.create(
                    {
                        'posttrain_metrics': posttrain_metrics,
                        'protected_attrs': protected_attrs,
                    }
                ...
                logger.warning(
                    f'Failed to create disparity_comparison chart: {e}'
                )
        ...
        if pretrain_metrics:
            try:
                charts[
                    'pretrain_metrics_overview'
                ] = self.pretrain_overview.create(
                    {
                ...
                logger.warning(
                    f'Failed to create pretrain_overview chart: {e}'
                )

            try:
                charts[
                    'pretrain_concept_balance'
                ] = self.concept_balance.create(
                    {
                        'pretrain_metrics': pretrain_metrics,
                        'protected_attrs': protected_attrs,
                    }
        ...
        if confusion_matrix and posttrain_metrics:
            try:
                charts[
                    'complementary_precision_accuracy'
                ] = self.precision_accuracy.create(
                    {
                        'posttrain_metrics': posttrain_metrics,
                        'confusion_matrix': confusion_matrix,
                        'protected_attrs': protected_attrs,
                ...
                logger.warning(
                    f'Failed to create precision_accuracy chart: {e}'
                )

            try:
                charts[
                    'complementary_treatment_equality'
                ] = self.treatment_equality.create(
                    {
                        'confusion_matrix': confusion_matrix,
                        'protected_attrs': protected_attrs,
                    }
                ...
                logger.warning(
                    f'Failed to create treatment_equality chart: {e}'
                )
        ...
        if posttrain_metrics:
            try:
                charts[
                    'complementary_radar'
                ] = self.complementary_radar.create(
                    {
                        'posttrain_metrics': posttrain_metrics,
                        'protected_attrs': protected_attrs,
                    }
                ...
                logger.warning(
                    f'Failed to create complementary_radar chart: {e}'
                )
