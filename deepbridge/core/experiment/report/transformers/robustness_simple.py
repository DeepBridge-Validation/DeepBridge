...
class RobustnessDataTransformerSimple:
    ...
    def transform(
        ...
        logger.info(
            f"Transformation complete. {transformed['metadata']['total_levels']} levels, "
            ...
            levels_data.append(
                {
                    ...
                    'worst_score': float(
                        overall_result.get('worst_score', 0.0)
                    ),
                }
            )
    ...
    def _transform_features(
        ...
        if not feature_importance and 'primary_model' in initial_eval:
            feature_importance = initial_eval.get('primary_model', {}).get(
                'feature_importance', {}
            )
        ...
        if (
            ...
            and 'initial_model_evaluation' in initial_eval
        ):
            ime = initial_eval['initial_model_evaluation']
            if 'models' in ime and 'primary_model' in ime['models']:
                feature_importance = ime['models']['primary_model'].get(
                    'feature_importance', {}
                )
        ...
        if (
            not feature_importance
            and 'model_feature_importance' in primary_model
        ):
            feature_importance = primary_model.get(
                'model_feature_importance', {}
            )
            logger.info(
                f'Using model_feature_importance from primary_model: {len(feature_importance)} features'
            )
        ...
        if not feature_importance:
            logger.warning(
                'No feature importance data found. Features section will be empty.'
            )
    ...
    def _prepare_charts(
        ...
        charts['score_distribution'] = self._create_score_distribution_chart(
            primary_model
        )
        ...
        for level_str in sorted(raw_data.keys(), key=float):
            ...
            if runs:
                ...
                if scores:
                    fig.add_trace(
                        go.Box(
                            y=scores, name=f'Level {level_str}', boxmean='sd'
                        )
                    )
    ...
    def _count_features(
        self, initial_eval: Dict, primary_model: Dict = None
    ) -> int:
