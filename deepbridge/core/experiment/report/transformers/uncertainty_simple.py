...
class UncertaintyDataTransformerSimple:
    ...
    def transform(
        ...
        logger.debug(
            f'[FEATURE_IMPACT_DEBUG] transform() input - results keys: {list(results.keys())}'
        )
        ...
        if 'test_results' in results:
            ...
            logger.debug(
                '[FEATURE_IMPACT_DEBUG] Using test_results wrapper format'
            )
        else:
            # Format from save_html: results['primary_model'] directly
            primary_model = results.get('primary_model', {})
            logger.debug(
                '[FEATURE_IMPACT_DEBUG] Using direct primary_model format'
            )

        initial_eval = results.get('initial_model_evaluation', {})
        logger.debug(
            f"[FEATURE_IMPACT_DEBUG] primary_model keys: {list(primary_model.keys()) if primary_model else 'None'}"
        )
        logger.debug(
            f"[FEATURE_IMPACT_DEBUG] initial_eval keys: {list(initial_eval.keys()) if initial_eval else 'None'}"
        )
        ...
        if 'metrics' not in primary_model:
            ...
            if (
                'test_results' in results
                and 'metrics' in results['test_results']
            ):
                primary_model['metrics'] = results['test_results']['metrics']
                logger.info(
                    '[METRICS_DEBUG] Added metrics from test_results root to primary_model'
                )
            # Try to get from initial_model_evaluation
            elif (
                initial_eval
                and 'models' in initial_eval
                and 'primary_model' in initial_eval['models']
            ):
                init_metrics = initial_eval['models']['primary_model'].get(
                    'metrics', {}
                )
                if init_metrics:
                    primary_model['metrics'] = init_metrics
                    logger.info(
                        '[METRICS_DEBUG] Added metrics from initial_model_evaluation to primary_model'
                    )
            else:
                logger.warning(
                    '[METRICS_DEBUG] No metrics found in any location!'
                )
        ...
        transformed = {
            ...
            'alphas': alphas_data,
            'alpha_results': self._format_alpha_results(
                alphas_data
            ),  # For coverage table
            # Feature importance
        ...
        logger.info(
            f"Transformation complete. {transformed['metadata']['total_alphas']} alpha levels, "
            f"{transformed['features']['total']} features, "
            f"{len(transformed['feature_impacts'])} feature impacts"
        )
        logger.debug(
            f"[COVERAGE_DEBUG] alpha_results array has {len(transformed['alpha_results'])} entries"
        )
        logger.debug(
            f"[FEATURE_IMPACT_DEBUG] feature_impacts array has {len(transformed['feature_impacts'])} entries"
        )
        return transformed
    ...
    def _create_summary(self, primary_model: Dict) -> Dict[str, Any]:
        ...
        if by_alpha:
            ...
            avg_coverage_error = (
                np.mean(coverage_errors) if coverage_errors else 0
            )
        ...
        if 'metrics' in primary_model:
            ...
            base_score = metrics.get(
                'base_score',
                metrics.get(
                    'accuracy',
                    metrics.get(
                        'r2_score',
                        metrics.get('roc_auc', metrics.get('f1', 0.0)),
                    ),
                ),
            )
        ...
        logger.info(
            f'[SUMMARY_DEBUG] avg_coverage_error (calibration_error): {avg_coverage_error:.4f}'
        )
    ...
    def _transform_alphas(self, primary_model: Dict) -> List[Dict[str, Any]]:
        ...
        transformed_alphas = []
        for alpha_key, alpha_data in sorted(
            by_alpha.items(), key=lambda x: float(x[0])
        ):
            overall = alpha_data.get('overall_result', {})
    ...
    def _transform_feature_impacts(
        ...
        logger.warning(
            '[FEATURE_IMPACT_DEBUG] by_feature data is currently buggy - all features have identical values'
        )
        logger.info(
            '[FEATURE_IMPACT_DEBUG] Using feature importance as workaround'
        )
        ...
        if not feature_importance:
            logger.warning(
                '[FEATURE_IMPACT_DEBUG] No feature_importance found!'
            )
            return []

        logger.info(
            f'[FEATURE_IMPACT_DEBUG] Using feature_importance with {len(feature_importance)} features'
        )
        ...
        if by_alpha:
            ...
            overall_result = by_alpha[first_alpha_key].get(
                'overall_result', {}
            )
        ...
        for feature_name, importance in feature_importance.items():
            ...
            logger.debug(
                f'[FEATURE_IMPACT_DEBUG] {feature_name}: importance={importance:.4f}, '
        ...
        logger.info(
            f'[FEATURE_IMPACT_DEBUG] Created {len(feature_impacts)} feature impacts using importance'
        )
        if feature_impacts:
            logger.debug(
                f'[FEATURE_IMPACT_DEBUG] Top 3 impacts: {feature_impacts[:3]}'
            )

        return feature_impacts

    def _format_alpha_results(
            ...
            formatted.append(
                {
                    ...
                    'avg_width': alpha_data[
                        'mean_width'
                    ],  # Template expects 'avg_width'
                    'calibration_error': alpha_data[
                        'coverage_error'
                    ],  # Template expects 'calibration_error'
                }
            )
        logger.debug(
            f'[COVERAGE_DEBUG] Formatted {len(formatted)} alpha results for table'
        )
        return formatted
    ...
    def _transform_features(
        ...
        logger.debug(
            f'[FEATURE_IMPACT_DEBUG] primary_model keys: {list(primary_model.keys())}'
        )
        ...
        if initial_eval:
            ...
            logger.debug(
                f'[FEATURE_IMPACT_DEBUG] feature_importance from initial_eval: {len(feature_importance)} features'
            )
        ...
        if not feature_importance:
            feature_importance = primary_model.get('feature_importance', {})
            logger.debug(
                f'[FEATURE_IMPACT_DEBUG] feature_importance from primary_model: {len(feature_importance)} features'
            )

        if not feature_importance:
            logger.warning(
                '[FEATURE_IMPACT_DEBUG] No feature importance data found in ANY source!'
            )
            return {
                'total': 0,
                'importance': {},
        ...
        feature_list_sorted = sorted(
            feature_list, key=lambda x: x['importance_abs'], reverse=True
        )
    ...
    def _prepare_charts(
        self, primary_model: Dict, initial_eval: Dict
    ) -> Dict[str, Any]:
        ...
        logger.debug(
            f"[FEATURE_IMPACT_DEBUG] Transformed features: {features['total']} total features"
        )

        # Create feature chart
        feature_chart = self._chart_feature_importance(features)
        logger.info(
            f"[FEATURE_IMPACT_DEBUG] Feature chart data length: {len(feature_chart.get('data', []))}"
        )
        logger.debug(
            f"[FEATURE_IMPACT_DEBUG] Feature chart has layout: {bool(feature_chart.get('layout'))}"
        )
        ...
        charts = {
            ...
            'coverage': self._chart_coverage_by_alpha(
                alphas
            ),  # Renamed from 'coverage_by_alpha' to match template
            'tradeoff': self._chart_width_analysis(
                alphas
            ),  # Renamed from 'width_analysis' to match template
        ...
        logger.debug(
            f'[COVERAGE_DEBUG] Created coverage chart with {len(alphas)} alpha levels'
        )
        logger.info(
            f'[FEATURE_IMPACT_DEBUG] Charts dictionary keys: {list(charts.keys())}'
        )
        logger.info(
            f"[FEATURE_IMPACT_DEBUG] charts['features'] exists: {'features' in charts}"
        )
        logger.info(
            f"[FEATURE_IMPACT_DEBUG] charts['features'] has data: {len(charts.get('features', {}).get('data', []))} traces"
        )
        logger.info(
            f"[WIDTH_DIST_DEBUG] charts['width_distribution'] exists: {'width_distribution' in charts}"
        )
        logger.info(
            f"[WIDTH_DIST_DEBUG] width_distribution has data: {len(width_dist_chart.get('data', []))} traces"
        )
    ...
    def _chart_coverage_by_alpha(self, alphas: List[Dict]) -> Dict[str, Any]:
        ...
        logger.debug(
            f'[COVERAGE_DEBUG] _chart_coverage_by_alpha called with {len(alphas)} alphas'
        )

        if not alphas:
            logger.warning(
                '[COVERAGE_DEBUG] _chart_coverage_by_alpha: No alphas data!'
            )
            return {'data': [], 'layout': {}}
        ...
        logger.debug(
            f'[COVERAGE_DEBUG] _chart_coverage_by_alpha: alpha_values={alpha_values}'
        )
        logger.debug(
            f'[COVERAGE_DEBUG] _chart_coverage_by_alpha: coverages={coverages}'
        )
        logger.debug(
            f'[COVERAGE_DEBUG] _chart_coverage_by_alpha: expected_coverages={expected_coverages}'
        )
    ...
    def _chart_boxplot_width(self, alphas: List[Dict]) -> Dict[str, Any]:
        ...
        for alpha_data in alphas:
            traces.append(
                {
                    'y': [
                        alpha_data['min_width'],
                        alpha_data['mean_width'],
                        alpha_data['max_width'],
                    ],
                    'type': 'box',
    ...
    def _chart_feature_importance(self, features: Dict) -> Dict[str, Any]:
        ...
        logger.debug(
            f'[FEATURE_IMPACT_DEBUG] features dict keys: {list(features.keys())}'
        )
        logger.debug(
            f"[FEATURE_IMPACT_DEBUG] features['total']: {features.get('total', 0)}"
        )

        top_features = features.get('top_10', [])
        logger.info(
            f'[FEATURE_IMPACT_DEBUG] top_features count: {len(top_features)}'
        )

        if not top_features:
            logger.warning(
                '[FEATURE_IMPACT_DEBUG] No top_features found! Returning empty chart.'
            )
            return {'data': [], 'layout': {}}
        ...
        logger.debug(
            f'[FEATURE_IMPACT_DEBUG] Feature importances: {importances}'
        )
        ...
        logger.info(
            '[FEATURE_IMPACT_DEBUG] Feature chart created successfully'
        )
        return {'data': [trace], 'layout': layout}
