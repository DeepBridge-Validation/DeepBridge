    ...
    def transform(
        ...
        if (
            ...
            if 'hyperparameter' in report_data['results']:
                hyperparameter_results = report_data['results'][
                    'hyperparameter'
                ]
                if (
                    'results' in hyperparameter_results
                    and 'alternative_models'
                    in hyperparameter_results['results']
                ):
                    logger.info('Found alternative_models in nested structure')
                    report_data['alternative_models'] = hyperparameter_results[
                        'results'
                    ]['alternative_models']
        ...
        if 'importance_results' not in report_data:
            ...
            if (
                'importance' in report_data
                and 'all_results' in report_data['importance']
            ):
                report_data['importance_results'] = report_data['importance'][
                    'all_results'
                ]
            else:
        ...
        if 'tuning_order' not in report_data:
            ...
            if (
                ...
                if 'tuning_order' in result:
                    ...
                    report_data['tuning_order'] = list(
                        result['sorted_importance'].keys()
                    )
