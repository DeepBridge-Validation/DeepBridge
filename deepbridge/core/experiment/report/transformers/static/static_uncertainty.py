...
class StaticUncertaintyTransformer:
    ...
    def transform(
        ...
        """
        logger.info(
            'Transforming uncertainty data for static report - Using improved transformer'
        )
        ...
        try:
            ...
            if 'primary_model' in data:
                logger.info(
                    f"Primary model keys: {list(data['primary_model'].keys())}"
                )
            ...
            try:
                ...
                logger.warning(
                    f'Data contains non-serializable objects: {str(e)}'
                )
        except Exception as e:
        ...
        if 'primary_model' in data and isinstance(data['primary_model'], dict):
            ...
            logger.info(
                f'[TRANSFORMER] primary_model keys: {list(primary_model.keys())[:15]}'
            )

            # Extract key metrics from primary_model if available
            if 'uncertainty_quality_score' in primary_model:
                output['uncertainty_score'] = primary_model[
            ...
            if 'avg_width' in primary_model and 'mean_width' not in output:
                ...
                'avg_normalized_width' in primary_model
                and 'mean_width' not in output
            ):
            ...
            if 'plot_data' in primary_model:
                ...
                if 'alpha_comparison' in plot_data:
                    ...
                    logger.info(
                        f"alpha_comparison keys: {list(alpha_data.keys()) if isinstance(alpha_data, dict) else 'not a dict'}"
                    )
                    ...
                    expected_coverages = alpha_data.get(
                        'expected_coverages', []
                    )
                    ...
                    if hasattr(expected_coverages, 'tolist') and callable(
                    ...
                    logger.info(
                        f"Transformed calibration_results: {output['calibration_results']}"
                    )
                ...
                if 'feature_importance' in plot_data and isinstance(
                    plot_data['feature_importance'], list
                ):
                    ...
                    for item in feature_data:
                        if 'feature' in item and 'importance' in item:
                            feature_importance[item['feature']] = item[
                                'importance'
                            ]
                ...
                if 'width_distribution' in plot_data:
                    ...
                    if isinstance(width_dist, list) and len(width_dist) > 0:
                        ...
                        for alpha_widths in width_dist:
                            if 'widths' in alpha_widths and hasattr(
                                alpha_widths['widths'], 'tolist'
                            ):
                                all_widths.append(
                                    alpha_widths['widths'].tolist()
                                )
                ...
                if 'coverage_vs_width' in plot_data:
                    output['coverage_vs_width'] = plot_data[
                        'coverage_vs_width'
                    ]
                    logger.debug(
                        f"[TRANSFORM_DEBUG] Found coverage_vs_width in plot_data: {plot_data['coverage_vs_width'].keys() if isinstance(plot_data['coverage_vs_width'], dict) else type(plot_data['coverage_vs_width'])}"
                    )
                else:
                    logger.warning(
                        '[TRANSFORM_DEBUG] coverage_vs_width NOT found in plot_data'
                    )

            # Extract feature reliability if available
            if 'feature_reliability' in primary_model:
                output['feature_reliability'] = primary_model[
                    'feature_reliability'
                ]
            ...
            if 'reliability_analysis' in primary_model:
                logger.info(
                    '[TRANSFORM_DEBUG] Found reliability_analysis in primary_model'
                )
                logger.info(
                    f"[TRANSFORM_DEBUG] reliability_analysis keys: {list(primary_model['reliability_analysis'].keys())}"
                )

                # Check if feature_distributions is present
                if (
                    'feature_distributions'
                    in primary_model['reliability_analysis']
                ):
                    ra_fd = primary_model['reliability_analysis'][
                        'feature_distributions'
                    ]
                    logger.info(
                        f'[TRANSFORM_DEBUG] feature_distributions types: {list(ra_fd.keys())}'
                    )

                    # Check each distribution type
                    for dist_type, features in ra_fd.items():
                        logger.info(
                            f"[TRANSFORM_DEBUG] '{dist_type}' has {len(features)} features"
                        )
                        feature_names = list(features.keys())
                        if feature_names:
                            logger.info(
                                f"[TRANSFORM_DEBUG] '{dist_type}' example features: {feature_names[:3]}"
                            )
                            ...
                            for feature in feature_names[:2]:
                                ...
                                if isinstance(values, list):
                                    logger.info(
                                        f"[TRANSFORM_DEBUG] '{dist_type}' feature '{feature}' has {len(values)} values"
                                    )
                                else:
                                    logger.info(
                                        f"[TRANSFORM_DEBUG] '{dist_type}' feature '{feature}' has non-list data: {type(values)}"
                                    )

                # Copy the data to output
                output['reliability_analysis'] = primary_model[
                    'reliability_analysis'
                ].copy()
                logger.info(
                    '[TRANSFORM_DEBUG] Copied reliability_analysis to output'
                )

            # Extract marginal bandwidth data if available
            if 'marginal_bandwidth' in primary_model:
                logger.info(
                    '[TRANSFORM_DEBUG] Found marginal_bandwidth in primary_model'
                )
                logger.info(
                    f"[TRANSFORM_DEBUG] marginal_bandwidth has {len(primary_model['marginal_bandwidth'])} features"
                )

                feature_names = list(
                    primary_model['marginal_bandwidth'].keys()
                )
                if feature_names:
                    logger.info(
                        f'[TRANSFORM_DEBUG] marginal_bandwidth features: {feature_names}'
                    )

                    # Check each feature's data
                    for feature in feature_names[
                        :2
                    ]:  # Look at first 2 features
                        feature_data = primary_model['marginal_bandwidth'][
                            feature
                        ]
                        logger.info(
                            f"[TRANSFORM_DEBUG] Feature '{feature}' data keys: {list(feature_data.keys())}"
                        )
                        ...
                        for key, value in feature_data.items():
                            if isinstance(value, list):
                                logger.info(
                                    f"[TRANSFORM_DEBUG] '{feature}' {key} has {len(value)} values"
                                )
                            else:
                                logger.info(
                                    f"[TRANSFORM_DEBUG] '{feature}' {key} = {value}"
                                )

                # Copy the data to output
                output['marginal_bandwidth'] = primary_model[
                    'marginal_bandwidth'
                ].copy()
                logger.info(
                    '[TRANSFORM_DEBUG] Copied marginal_bandwidth to output'
                )

            # Extract interval widths for boxplots/violinplots
            if 'interval_widths' in primary_model:
                logger.info(
                    '[TRANSFORM_DEBUG] Found interval_widths in primary_model'
                )

                # Log details based on type
                if isinstance(primary_model['interval_widths'], dict):
                    logger.info(
                        f"[TRANSFORM_DEBUG] interval_widths is a dictionary with keys: {list(primary_model['interval_widths'].keys())}"
                    )

                    # Check values
                    for model, widths in primary_model[
                        'interval_widths'
                    ].items():
                        if isinstance(widths, list):
                            logger.info(
                                f"[TRANSFORM_DEBUG] Model '{model}' has {len(widths)} width values"
                            )
                        else:
                            logger.info(
                                f"[TRANSFORM_DEBUG] Model '{model}' has non-list data: {type(widths)}"
                            )

                elif isinstance(primary_model['interval_widths'], list):
                    logger.info(
                        f"[TRANSFORM_DEBUG] interval_widths is a list with {len(primary_model['interval_widths'])} elements"
                    )
                    ...
                    if primary_model['interval_widths']:
                        ...
                        logger.info(
                            f'[TRANSFORM_DEBUG] First element type: {type(first_item)}'
                        )

                        if isinstance(first_item, dict):
                            logger.info(
                                f'[TRANSFORM_DEBUG] First element keys: {list(first_item.keys())}'
                            )
                        elif isinstance(first_item, list):
                            logger.info(
                                f'[TRANSFORM_DEBUG] First element is a list with {len(first_item)} values'
                            )
                else:
                    logger.info(
                        f"[TRANSFORM_DEBUG] interval_widths has unexpected type: {type(primary_model['interval_widths'])}"
                    )

                # Copy the data to output
                output['interval_widths'] = primary_model['interval_widths']
                logger.info(
                    '[TRANSFORM_DEBUG] Copied interval_widths to output'
                )

            # Extract test predictions and labels for reliability charts
            if 'test_predictions' in primary_model:
                logger.info(
                    f"[TRANSFORM_DEBUG] Found test_predictions in primary_model with shape: {primary_model['test_predictions'].shape if hasattr(primary_model['test_predictions'], 'shape') else 'N/A'}"
                )
                output['test_predictions'] = primary_model['test_predictions']

            if 'test_labels' in primary_model:
                logger.info(
                    f"[TRANSFORM_DEBUG] Found test_labels in primary_model with shape: {primary_model['test_labels'].shape if hasattr(primary_model['test_labels'], 'shape') else 'N/A'}"
                )
        ...
        if (
            'reliability_analysis' not in output
            and 'reliability_analysis' in data
        ):
        ...
        if 'test_predictions' not in output and 'test_predictions' in data:
            ...
            logger.info(
                f"[TRANSFORM_DEBUG] Found test_predictions at top level with shape: {data['test_predictions'].shape if hasattr(data['test_predictions'], 'shape') else 'N/A'}"
            )

        if 'test_labels' not in output and 'test_labels' in data:
            output['test_labels'] = data['test_labels']
            logger.info(
                f"[TRANSFORM_DEBUG] Found test_labels at top level with shape: {data['test_labels'].shape if hasattr(data['test_labels'], 'shape') else 'N/A'}"
            )
        ...
        if 'alternative_models' in data and isinstance(
            ...
            if 'uncertainty_score' in output and (
                'coverage' in output or 'mean_width' in output
            ):
                primary_model_data = {
                    'name': model_name,
            ...
            for alt_name, model_data in data['alternative_models'].items():
                ...
                if 'uncertainty_quality_score' in model_data:
                    model_info['uncertainty_score'] = model_data[
                        'uncertainty_quality_score'
                    ]
                ...
                if 'avg_width' in model_data:
                    ...
                    model_info['mean_width'] = model_data[
                        'avg_normalized_width'
                    ]
                else:
                ...
                if 'plot_data' in model_data:
                    if 'alpha_comparison' in model_data['plot_data']:
                        alpha_data = model_data['plot_data'][
                            'alpha_comparison'
                        ]
                ...
                if 'feature_importance' in model_data:
                    model_info['feature_importance'] = model_data[
                        'feature_importance'
                    ]
                ...
                if all(
                    k in model_info
                    for k in ['uncertainty_score', 'coverage', 'mean_width']
                ):
                    models_data_for_charts.append(
                        {
                            'name': alt_name,
                            'uncertainty_score': model_info[
                                'uncertainty_score'
                            ],
        ...
        logger.info(
            f"Transformed uncertainty data: uncertainty_score={output.get('uncertainty_score', 'N/A')}, coverage={output.get('coverage', 'N/A')}, mean_width={output.get('mean_width', 'N/A')}"
        )
        return output
