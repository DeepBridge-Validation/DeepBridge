...
try:
    ...
    logger.warning(
        'JavaScript syntax fixer not available, syntax errors may occur'
    )
...
class AssetProcessor:
    ...
    def get_css_content(
        ...
        if files is None:
            files = self.asset_manager.file_manager._discover_css_files(
                css_dir
            )
        ...
        for name, rel_path in files_to_process.items():
            ...
            if os.path.exists(component_path):
                try:
                    ...
                    logger.warning(
                        f'Error reading CSS file {component_path}: {str(e)}'
                    )
            else:
    ...
    def get_js_content(
        self, js_dir: str, files: Optional[Dict[str, str]] = None
    ) -> str:
        """
        ...
        if files is None:
            ...
            if not files:
                raise FileNotFoundError(
                    f'No JavaScript files found in {js_dir}'
                )
        ...
        for name, rel_path in files.items():
            ...
            if os.path.exists(full_path):
                try:
                    ...
                    logger.warning(
                        f'Error processing JavaScript file {full_path}: {str(e)}'
                    )
            else:
        ...
        if not processed_js:
            raise FileNotFoundError(
                f'No valid JavaScript files found in {js_dir}'
            )
        ...
        if 'main' in processed_js:
            ...
            del processed_js[
                'main'
            ]  # Remove from processed_js so it's not included again

        # First process utility files
        utils_files = [
            name for name in processed_js.keys() if 'util' in name.lower()
        ]
        ...
        chart_files = [
            name for name in processed_js.keys() if 'chart' in name.lower()
        ]
    ...
    def get_generic_js_content(self) -> str:
        ...
        if not files:
            raise FileNotFoundError(
                f'No generic JavaScript files found in {js_dir}'
            )
        ...
        for name, rel_path in files.items():
            ...
            try:
                ...
                logger.warning(
                    f'Error processing generic JavaScript file {full_path}: {str(e)}'
                )
        ...
        component_files = [
            name for name in processed_js.keys() if 'component' in name.lower()
        ]
    ...
    def get_combined_js_content(self, test_type: str) -> str:
        ...
        if not js_dir or not os.path.exists(js_dir):
            ...
            global_error_handler_path = os.path.join(
                js_dir, 'global_error_handler.js'
            )
            if os.path.exists(global_error_handler_path):
                try:
                    with open(
                        global_error_handler_path, 'r', encoding='utf-8'
                    ) as f:
                        critical_scripts.append(
                            f'// Global Error Handler\n{f.read()}'
                        )
                        logger.info(
                            'Added global_error_handler.js to critical scripts'
                        )
                except Exception as e:
                    logger.warning(
                        f'Error reading global_error_handler.js: {str(e)}'
                    )
            ...
            if os.path.exists(syntax_fixer_path):
                try:
                    with open(syntax_fixer_path, 'r', encoding='utf-8') as f:
                        ...
                        logger.info(
                            'Added syntax_fixer.js to critical scripts'
                        )
                except Exception as e:
            ...
            if os.path.exists(fixed_syntax_path):
                try:
                    with open(fixed_syntax_path, 'r', encoding='utf-8') as f:
                        ...
                        logger.info(
                            'Added fixed_syntax.js to critical scripts'
                        )
                except Exception as e:
                    logger.warning(f'Error reading fixed_syntax.js: {str(e)}')

            # Check for safe_chart_manager.js
            safe_chart_manager_path = os.path.join(
                js_dir, 'safe_chart_manager.js'
            )
            if os.path.exists(safe_chart_manager_path):
                try:
                    with open(
                        safe_chart_manager_path, 'r', encoding='utf-8'
                    ) as f:
                        critical_scripts.append(
                            f'// Safe Chart Manager\n{f.read()}'
                        )
                        logger.info(
                            'Added safe_chart_manager.js to critical scripts'
                        )
                except Exception as e:
                    logger.warning(
                        f'Error reading safe_chart_manager.js: {str(e)}'
                    )
            ...
            if os.path.exists(model_chart_fix_path):
                try:
                    with open(
                        model_chart_fix_path, 'r', encoding='utf-8'
                    ) as f:
                        critical_scripts.append(
                            f'// Model Chart Fix\n{f.read()}'
                        )
                        logger.info(
                            'Added model_chart_fix.js to critical scripts'
                        )
                except Exception as e:
                    logger.warning(
                        f'Error reading model_chart_fix.js: {str(e)}'
                    )
            ...
            try:
                ...
                logger.warning(
                    f'Error loading {test_type} JavaScript: {str(e)}'
                )
                test_js = f'// No {test_type} JavaScript loaded\n\n'
        ...
        if test_type == 'robustness':
            try:
                ...
                if os.path.exists(fix_boxplot_path):
                    ...
                    logger.info(
                        f'Successfully included fix_boxplot.js directly (Modified: {mod_time})'
                    )
                else:
                    logger.warning(
                        f'fix_boxplot.js not found at {fix_boxplot_path}'
                    )
            except Exception as e:
        ...
        if critical_js:
            ...
            logger.info(
                'Combined JS with critical fixes first, then generic and test-specific'
            )
        else:
            combined_js = '// ===== Combined JavaScript (Generic + Test-specific) ===== //\n\n'
            combined_js += generic_js + '\n\n' + test_js + additional_js
            logger.info(
                'Combined JS with standard approach (no critical fixes found)'
            )
    ...
    def create_full_report_assets(self, test_type: str) -> Dict[str, Any]:
        ...
        try:
            report_assets[
                'common_html'
            ] = self.asset_manager.get_common_html_files()
        except Exception as e:
        ...
        try:
            report_assets[
                'test_partials'
            ] = self.asset_manager.get_test_partials(test_type)
        except Exception as e:
