...
class DataValidator:
    ...
    def validate_robustness_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field, default in required_fields.items():
            if field not in validated or validated[field] is None:
                logger.warning(
                    f"Missing required field '{field}', setting default value"
                )
        ...
        if 'quantile' not in validated or not isinstance(
            validated['quantile'], dict
        ):
            validated['quantile'] = {}
        ...
        if validated['feature_subset']:
            validated['feature_subset_display'] = ', '.join(
                validated['feature_subset']
            )
        else:
    ...
    def validate_uncertainty_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'metrics' not in validated or not isinstance(
