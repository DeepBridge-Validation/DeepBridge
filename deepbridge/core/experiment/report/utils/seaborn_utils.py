...
class SeabornChartGenerator:
    ...
    def generate_encoded_chart(self, func, *args, **kwargs) -> str:
        ...
        *args, **kwargs
        ...
        try:
            ...
            fig.savefig(
                buf, format='png', dpi=dpi, bbox_inches='tight', pad_inches=0.5
            )
            buf.seek(0)
    ...
    def robustness_overview_chart(
        self,
        perturbation_levels: List[float],
        scores: List[float],
        base_score: float,
        title: str = 'Model Performance by Perturbation Level',
        metric_name: str = 'Score',
        feature_subset_scores: List[float] = None,
    ) -> str:
        """
        ...
        def _generate_chart(
            ...
            has_feature_subset = feature_subset_scores is not None and len(
            ...
            if has_feature_subset:
                df_melted = self.pd.melt(
                    df,
                    id_vars=['Perturbation Level'],
            ...
            ax.set_xlim(
                min(perturbation_levels) - 0.05,
                max(perturbation_levels) + 0.05,
            )

        return self.generate_encoded_chart(
            _generate_chart,
            perturbation_levels,
            scores,
            base_score,
    ...
    def worst_performance_chart(
        self,
        perturbation_levels: List[float],
        worst_scores: List[float],
        base_score: float,
        title: str = 'Worst Performance by Perturbation Level',
        metric_name: str = 'Score',
        feature_subset_worst_scores: List[float] = None,
    ) -> str:
        """
        ...
        def _generate_chart(
            ...
            title,
            metric_name,
            feature_subset_worst_scores=None,
        ):
            # Create DataFrame for Seaborn
            data = {
                'Perturbation Level': perturbation_levels,
            ...
            # Add feature subset scores if available
            has_feature_subset = (
                feature_subset_worst_scores is not None
                and len(feature_subset_worst_scores)
                == len(perturbation_levels)
            )
            if has_feature_subset:
    ...
    def model_comparison_chart(
        ...
        def _generate_chart(
            ax, perturbation_levels, models_data, title, metric_name
        ):
    ...
    def feature_importance_chart(
        ...
        if not features:
            logger.warning(
                'Empty features dictionary provided for feature importance chart'
            )
            return ''
        ...
        for feature, value in features.items():
            try:
                ...
                logger.warning(
                    f'Non-numeric importance for feature {feature}: {value}'
                )
                continue
        ...
        return self.generate_encoded_chart(
            ...
            max_features,
            color,
            figsize=(10, max(6, min(15, len(features) * 0.4))),
        )

    def feature_comparison_chart(
        self,
        ...
        if not model_importance or not robustness_importance:
            logger.warning(
                'Empty feature importance dictionaries provided for comparison chart'
            )
            return ''
        ...
        for feature, value in model_importance.items():
            try:
                ...
                logger.warning(
                    f'Non-numeric model importance for feature {feature}: {value}'
                )
                continue
        ...
        for feature, value in robustness_importance.items():
            try:
                ...
                logger.warning(
                    f'Non-numeric robustness importance for feature {feature}: {value}'
                )
                continue

        # Check if we have any common features
        common_features = set(clean_model_importance.keys()) & set(
            clean_robustness_importance.keys()
        )
        if not common_features:
            logger.warning(
                'No common features found between model and robustness importance'
            )
            # Create merged set from both dictionaries
            all_features = set(clean_model_importance.keys()) | set(
                clean_robustness_importance.keys()
            )
            if not all_features:
                return ''
        ...
        def _generate_chart(
            ax, model_importance, robustness_importance, title, max_features
        ):
            # Get common features
            common_features = set(model_importance.keys()) & set(
                robustness_importance.keys()
            )
    ...
    def individual_feature_impact_chart(
        self,
        ...
        def _generate_chart(ax, sorted_features, title):
            ...
            colors = [
                '#d62728' if impact < 0 else '#2ca02c' for impact in impacts
            ]

            # Create horizontal bar chart
            bars = ax.barh(
                range(len(features)), impacts, color=colors, alpha=0.7
            )
            ...
            ax.set_xlim(
                [
                    min(impacts) * 1.1 if min(impacts) < 0 else -0.05,
        ...
        return self.generate_encoded_chart(
            _generate_chart, sorted_features, title, figsize=(10, fig_height)
        )
    ...
    def method_comparison_chart(
        ...
        quantile_worst_scores: List[float] = None,
    ) -> str:
        """
        ...
        if len(perturbation_levels) != len(raw_scores) or len(
            perturbation_levels
        ) != len(quantile_scores):
            logger.warning(
                'Mismatched data lengths for method comparison chart'
            )
            return ''
        ...
        def _generate_chart(
            ...
            ax.plot(
                ...
                linewidth=2.5,
                markersize=8,
                label='Quantile Perturbation (avg)',
                alpha=0.8,
            )

            # Plot worst case lines if available
            if raw_worst_scores and len(raw_worst_scores) == len(
                perturbation_levels
            ):
                ax.plot(
                    perturbation_levels,
                    ...
                    markersize=6,
                    alpha=0.6,
                    label='Raw Perturbation (worst)',
                )

            if quantile_worst_scores and len(quantile_worst_scores) == len(
                perturbation_levels
            ):
                ax.plot(
                    perturbation_levels,
                    ...
                    markersize=6,
                    alpha=0.6,
                    label='Quantile Perturbation (worst)',
                )
            ...
            if (
                ...
                ax.text(
                    0.02,
                    0.98,
                    f'{better_method} method generally outperforms {worse_method}',
                    transform=ax.transAxes,
                    fontsize=10,
    ...
    def selected_features_comparison_chart(
        self,
        perturbation_levels: List[float],
        ...
        selected_features_worst: List[float] = None,
    ) -> str:
        """
        ...
        """
        if (
            not perturbation_levels
            or not all_features_scores
            or not selected_features_scores
        ):
            logger.warning(
                'Empty data provided for selected features comparison chart'
            )
            return ''

        if len(perturbation_levels) != len(all_features_scores) or len(
            perturbation_levels
        ) != len(selected_features_scores):
            logger.warning(
                'Mismatched data lengths for selected features comparison chart'
            )
            return ''
        ...
        def _generate_chart(
            ...
            ax.plot(
                ...
                linewidth=2.5,
                markersize=8,
                label=f'Selected Features (avg)',
                alpha=0.8,
            )

            # Plot worst case lines if available
            if all_features_worst and len(all_features_worst) == len(
                perturbation_levels
            ):
                ax.plot(
                    perturbation_levels,
                    ...
                    markersize=6,
                    alpha=0.6,
                    label='All Features (worst)',
                )

            if selected_features_worst and len(selected_features_worst) == len(
                perturbation_levels
            ):
                ax.plot(
                    perturbation_levels,
            ...
            differences = [
                sel - all_feat
                for sel, all_feat in zip(
                    selected_features_scores, all_features_scores
                )
            ]
            avg_diff = (
            ...
            if abs(avg_diff) > 0.001:
                better_approach = (
                    'Selected features' if avg_diff > 0 else 'All features'
                )
    ...
    def detailed_boxplot_chart(
        ...
        if not perturbation_data:
            logger.warning(
                'Empty perturbation data provided for detailed boxplot'
            )
            return ''
        ...
        for level in sorted_levels:
            ...
            statistics[level_str] = {
                ...
                'coverage': self.np.sum(
                    scores_array >= base_score * coverage_threshold
                )
                / len(scores_array)
                * 100,
            }
        ...
        def _generate_chart(
            ...
            df = self.pd.DataFrame(
                plot_data, columns=['Perturbation_Level', 'Score']
            )

            # Create detailed boxplot with gradient colors
            colors = self.plt.cm.RdYlBu_r(
                self.np.linspace(0.2, 0.8, len(level_labels))
            )
            ...
            if abs(coverage_line - base_score) > 0.001:
                ax.axhline(
                    ...
                    linewidth=1.5,
                    alpha=0.6,
                    label=f'{coverage_threshold*100:.0f}% Coverage Threshold',
                )
            ...
            ax.set_xticklabels(
                [f'Level {label}' for label in level_labels], fontsize=10
            )
            ...
            ax.text(
                ...
                horizontalalignment='left',
                bbox=dict(
                    boxstyle='round,pad=0.5', facecolor='lightcyan', alpha=0.8
                ),
            )
    ...
    def boxplot_chart(
        ...
        def _generate_chart(ax, models_data, title, metric_name):
            ...
            for model in models_data:
                ...
                # Create data for this model
                model_df = self.pd.DataFrame(
                    {'Model': [name] * len(scores), metric_name: scores}
                )
                df_list.append(model_df)
                ...
                for i, model in enumerate(models_data):
                    ...
                    if scores:
                        ...
                        ax.annotate(
                            ...
                            xytext=(i + 0.25, mean),
                            arrowprops=dict(
                                arrowstyle='-', color='black', alpha=0.7
                            ),
                            color='black',
                            fontsize=9,
                            fontweight='bold',
        ...
        height = max(
            8,
            min(
                12, 6 + len([m for m in models_data if m.get('scores')]) * 0.8
            ),
        )
        return self.generate_encoded_chart(
            _generate_chart,
    ...
    def uncertainty_violin_chart(
        self,
        models_data: List[Dict],
        title: str = 'Uncertainty Model Comparison',
        ...
        def _generate_chart(ax, models_data, title, metric_name):
            ...
            if models_data and len(models_data) > 0:
                base_model = models_data[
                    0
                ]  # Assume first model is the primary model

            # Log how many models we have for debugging
            logger.info(
                f'Processing {len(models_data)} models for uncertainty_violin_chart'
            )
            ...
            for model in models_data:
                ...
                logger.info(
                    f'Model: {name}, Has scores: {bool(scores)}, Scores count: {len(scores)}'
                )

                if not scores:
                    logger.warning(
                        f'Skipping model {name} due to empty scores'
                    )
                    continue
            ...
            palette = self.sns.color_palette(
                'husl', n_colors=max(8, len(model_names))
            )

            # Log unique models in DataFrame for debugging
            unique_models_in_df = df['Model'].unique()
            logger.info(
                f'Unique models in dataframe: {list(unique_models_in_df)}'
            )
            logger.info(f"Models we're displaying: {model_names}")

            if len(unique_models_in_df) < len(model_names):
                logger.warning(
                    f'Some models are missing from the DataFrame. Expected {len(model_names)}, got {len(unique_models_in_df)}'
                )

            # Log the number of models being displayed
            logger.info(
                f'Displaying {len(model_names)} models with {len(palette)} colors'
            )
            ...
            if base_model and base_model.get('name') in model_names:
                ...
                palette[
                    primary_idx
                ] = '#1b78de'  # Use primary color for base model
            ...
            if len(unique_models) < len(model_names):
                logger.warning(
                    f'Some models are missing from the plot. Expected {len(model_names)}, got {len(unique_models)}'
                )
        ...
        width = max(
            12,
            min(
                16, 8 + len([m for m in models_data if m.get('scores')]) * 1.2
            ),
        )
        return self.generate_encoded_chart(
            _generate_chart,
    ...
    def bar_chart(
        self, data: Dict[str, List], title: str = 'Bar Chart'
    ) -> str:
        """
        ...
        return self.generate_encoded_chart(
            _generate_chart, data, title, figsize=(10, 6)
        )

    def feature_psi_chart(
        self,
        psi_data: Dict[str, List],
        title: str = 'Feature Distribution Shift (PSI)',
    ) -> str:
        """
        ...
        def _generate_chart(ax, psi_data, title):
            ...
            self.sns.barplot(
                x='Feature', y='PSI', data=df, ax=ax, palette='Blues_d'
            )
        ...
        return self.generate_encoded_chart(
            _generate_chart, psi_data, title, figsize=(12, 7)
        )

    def distribution_grid_chart(
        self,
            ...
            for j, level in enumerate(sorted_levels):
                ...
                if i == 0:  # Top row
                    ax_current.set_title(
                        f'Level {level}', fontsize=10, fontweight='bold'
                    )
        ...
        if baseline_scores:
            legend_elements = [
                self.plt.Line2D(
                    [0], [0], color='red', linestyle='--', label='Baseline'
                )
            ]
            fig.legend(
    ...
    def coverage_analysis_chart(
        ...
        def _generate_chart(ax, alpha_values, coverage_values, title):
            if (
                ...
                or len(alpha_values) != len(coverage_values)
            ):
    ...
    def model_metrics_heatmap(
        ...
        def _generate_chart(ax, results_df, title):
            ...
            for metric in metrics:
                ...
                if metric not in results_df or len(results_df[metric]) != len(
                    models
                ):
                    continue
            ...
            for i, row in enumerate(heatmap_array):
                ...
                if metrics[i] in ['mse', 'mae', 'mean_width']:
                    ...
                    if self.np.max(row) > self.np.min(row):
                        normalized_data[i] = 1 - (row - self.np.min(row)) / (
                            self.np.max(row) - self.np.min(row)
                        )
                    else:
                    ...
                    if self.np.max(row) > self.np.min(row):
                        normalized_data[i] = (row - self.np.min(row)) / (
                            self.np.max(row) - self.np.min(row)
                        )
                    else:
                        normalized_data[i] = 0.5  # All values are the same
        ...
        return self.generate_encoded_chart(
            _generate_chart, results_df, title, figsize=(12, 8)
        )
    ...
    def metrics_radar_chart(
        ...
        def _generate_chart(ax, models_metrics, title):
            try:
                ...
                ax.set_yticklabels(
                    ['0.25', '0.5', '0.75', '1'], color='grey', size=8
                )
                ...
                for i, (model_name, model_data) in enumerate(
                    ...
                    values = [
                        model_data['metrics'].get(metric, 0)
                        for metric in metrics
                    ]
        ...
        return self.generate_encoded_chart(
            _generate_chart, models_metrics, title, figsize=(10, 10)
        )
