...
class Experiment(IExperiment):
    ...
    def detect_sensitive_attributes(
        dataset: 'DBDataset', threshold: float = 0.7
    ) -> t.List[str]:
        """
        ...
        if hasattr(dataset, '_data') and isinstance(
            dataset._data, pd.DataFrame
        ):
            columns = dataset._data.columns.tolist()
        elif hasattr(dataset, 'data') and isinstance(
            dataset.data, pd.DataFrame
        ):
            columns = dataset.data.columns.tolist()
        elif hasattr(dataset, 'features') and isinstance(
    ...
    def _initialize_components(self, dataset, test_size, random_state):
        ...
        self.model_manager = ModelManager(
            dataset, self.experiment_type, self.verbose
        )
        self.model_evaluation = ModelEvaluation(
            self.experiment_type, self.metrics_calculator
        )
        ...
        self.prob_train, self.prob_test = (
            self.data_manager.prob_train,
            self.data_manager.prob_test,
        )
        ...
        if 'models' in self.initial_results:
            ...
            if 'primary_model' in self.initial_results['models']:
                ...
                if (
                    ...
                    self._calculate_model_feature_importance(
                        'primary_model',
                        self.initial_results['models']['primary_model'],
                        self.dataset.model,
                    )
            ...
            for model_name, model_data in self.initial_results[
                'models'
            ].items():
                if model_name != 'primary_model':
                    model_obj = self.alternative_models.get(model_name)
                    self._standardize_metrics(
                        model_name, model_data, model_obj
                    )
    ...
    def __init__(
        ...
        if experiment_type not in self.VALID_TYPES:
            raise ValueError(
                f'experiment_type must be one of {self.VALID_TYPES}'
            )
        ...
        if 'fairness' in self.tests and not protected_attributes:
            self.logger.info(
                'Auto-detecting sensitive attributes for fairness testing...'
            )
            ...
            if detected:
                ...
                self.logger.info(
                    f'Auto-detected sensitive attributes: {detected}'
                )
                self.logger.warning(
    ...
    def _calculate_model_feature_importance(
        ...
        if hasattr(model_obj, 'feature_importances_'):
            ...
            if feature_names is not None and len(feature_names) == len(
                ...
                self.logger.debug(
                    f'Added feature importance from feature_importances_ for {model_name}'
                )
        ...
        # For linear models that use coef_ instead
            ...
            if feature_names is not None and len(feature_names) == len(coef):
                ...
                model_data['feature_importance'] = importance_dict
                self.logger.debug(
                    f'Added feature importance from coef_ for {model_name}'
                )
    ...
    def _standardize_metrics(
        ...
        model_name: str,
        model_data: dict,
        model_obj: t.Optional[t.Any] = None,
    ) -> None:
        """
            ...
            try:
                ...
                if hasattr(model_obj, 'predict_proba'):
                    ...
                    if n_classes > 2:
                        ...
                        self.logger.debug(
                            f'Calculated multiclass ROC AUC for {model_name}: {roc_auc}'
                        )
                    elif n_classes == 2:
                        # Binary: use second column (positive class probability)
                        roc_auc = float(
                            roc_auc_score(self.y_test, y_prob[:, 1])
                        )
                        metrics['roc_auc'] = roc_auc
                        self.logger.debug(
                            f'Calculated ROC AUC for {model_name}: {roc_auc}'
                        )
            except Exception as e:
                self.logger.warning(
                    f'Could not calculate ROC AUC for {model_name}: {str(e)}'
                )
    ...
    def run_tests(self, config_name: str = 'quick', **kwargs) -> dict:
        ...
        if 'models' in self.initial_results:
            if 'primary_model' in self.initial_results['models']:
                if (
                    'feature_importance'
                    in self.initial_results['models']['primary_model']
                ):
                    feature_importance = self.initial_results['models'][
                        'primary_model'
                    ]['feature_importance']
                    self._test_results[
    ...
    def get_feature_importance(self, model_name='primary_model'):
        ...
        if not hasattr(self, 'initial_results') or not self.initial_results:
            raise ValueError(
                'No model results available. Initialize the experiment first.'
            )
        ...
        if 'feature_importance' not in model_data:
            ...
            model_obj = None
            if model_name == 'primary_model' and hasattr(
                self.dataset, 'model'
            ):
            ...
            if 'feature_importance' not in model_data:
                raise ValueError(
                    f"Feature importance not available for model '{model_name}'. The model may not support feature importance."
                )
    ...
    def save_html(
        ...
        if not test_results:
            raise ValueError(
                f'No {test_type} test results found. Run the test first with experiment.run_tests()'
            )
        ...
        if hasattr(self, '_experiment_result') and isinstance(
            self._experiment_result, ExperimentResult
        ):
            return self._experiment_result.save_html(
                test_type,
                file_path,
                model_name or self._get_model_name(),
        ...
        experiment_result = wrap_results(combined_results)
        return experiment_result.save_html(
            test_type,
            file_path,
            model_name or self._get_model_name(),
            report_type=report_type,
    ...
    def run_fairness_tests(self, config: str = 'full') -> 'FairnessResult':
        ...
        if fairness_result.critical_issues:
            self.logger.warning(
                f'Found {len(fairness_result.critical_issues)} critical fairness issues'
            )
