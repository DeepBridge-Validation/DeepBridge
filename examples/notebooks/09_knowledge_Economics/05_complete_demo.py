    ...
    for i in range(n_samples):
        if is_post_crisis.iloc[i]:
            ...
            logit[i] = (
                ...
                + 0.025 * df['dti_ratio'].iloc[i]
                + 0.06 * df['interest_rate'].iloc[i]  # Aumentou
...
compliance_rate = check_constraint_compliance(
    student, feature_cols, economic_constraints
)
...
max_change_idx = np.argmax(
    np.abs(coefs_post - coefs_pre) / (np.abs(coefs_pre) + 1e-10)
)
max_change_feature = feature_cols[max_change_idx]
max_change_pct = (
    (coefs_post[max_change_idx] - coefs_pre[max_change_idx])
    / (abs(coefs_pre[max_change_idx]) + 1e-10)
) * 100
...
print(
    f'   Teacher (GBM):          AUC = {teacher_auc:.4f}, F1 = {teacher_f1:.4f}'
)
print(
    f'   Student (Linear):       AUC = {student_auc:.4f}, F1 = {student_f1:.4f}'
)
print(f'   Retenção:               {student_auc/teacher_auc*100:.1f}%')
...
print(f'   Sign Stability:         {sign_stability.mean()*100:.1f}%')
print(
    f"   Status:                 {'✅ Estável para inferência' if is_stable else '⚠️  Revisar'}"
)
...
print(
    f'   Interpretação:          Crise aumentou sensibilidade a {max_change_feature}'
)
...
print(
    f'   - Stability (30%):      {max(0, 1 - coef_cv.mean() / 0.15)*100:.1f}%'
)
...
print('   ' + '-' * 58)
print(
    f'   Perda vs Teacher         {(1-student_auc/teacher_auc)*100:5.1f}%     2-5%'
)
print(f'   Compliance               {compliance_rate:5.1f}%     95%+')
print(f'   CV médio                 {coef_cv.mean():5.3f}     < 0.15')
print(
    f'   Sign Stability           {sign_stability.mean()*100:5.1f}%     95%+'
)
...
print(
    '   3. ✅ Student destilado com retenção de {:.1f}%'.format(
        student_auc / teacher_auc * 100
    )
)
