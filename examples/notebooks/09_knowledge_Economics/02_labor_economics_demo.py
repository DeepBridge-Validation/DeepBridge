...
def generate_labor_dataset(n_samples=8000, n_features=20):
    ...
    feature_names = [
        'education_level', 'experience', 'age', 'wage_expectation', 'skills_score',
        'region', 'industry', 'unemployment_rate', 'gdp_growth', 'union_member',
        'part_time_preference', 'job_search_intensity', 'network_size',
        'training_programs', 'certifications', 'languages', 'mobility',
        'family_size', 'health_status', 'disability'
    ]
    ...
    df['education_level'] = np.clip(
        ...
        (df['education_level'].max() - df['education_level'].min()) * 4, 0, 4
    ).astype(int)  # 0=None, 1=HS, 2=Bachelor, 3=Master, 4=PhD

    df['experience'] = np.abs(df['experience']) * 15  # 0-30 anos
    df['age'] = np.clip(np.abs(df['age']) * 30 + 18, 18, 70)  # 18-70
    df['wage_expectation'] = np.abs(df['wage_expectation']) * 50000 + 20000  # 20k-150k
    df['skills_score'] = (df['skills_score'] - df['skills_score'].min()) / \
                         (df['skills_score'].max() - df['skills_score'].min()) * 100
    ...
    logit = education_effect + experience_effect + skills_effect + \
...
education_labels = {0: 'None', 1: 'High School', 2: 'Bachelor', 3: 'Master', 4: 'PhD'}
for level in sorted(X['education_level'].unique()):
    count = (X['education_level'] == level).sum()
    pct = count / len(X) * 100
    print(f"      {education_labels.get(level, level):12} → {count:4} ({pct:.1f}%)")
...
for feature, constraint in economic_constraints.items():
    print(f"   {feature:20} → {constraint['type']:15} → {constraint['justification']}")
...
for level in education_levels:
    ...
    print(f"      {label:12} → P(employed)={effect:.3f} (+{pct_change:+.1f} pp)")

# Verificar monotonia
is_monotonic = all(
    marginal_effects[education_levels[i]] <= marginal_effects[education_levels[i+1]]
...
for feature, constraint in economic_constraints.items():
    if constraint['type'] == 'sign':
        ...
        compliant, coef = check_coefficient_sign(baseline, feature_idx, constraint['sign'])
        status = "✅" if compliant else "❌"
        print(f"      {status} {feature:20} → {coef:+.4f} (esperado: {constraint['sign']:+d})")
...
print(f"   Ganho vs Baseline:       +{(economic_auc - baseline_auc)*100:.1f} pp")
