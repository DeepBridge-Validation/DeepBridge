...
for feature, constraint in economic_constraints.items():
    print(
        f"   {feature:20} → {constraint['type']:12} → {constraint['justification']}"
    )
...
feature_importance = pd.DataFrame(
    {'feature': X.columns, 'importance': teacher.feature_importances_}
...
baseline_student = LogisticRegression(max_iter=1000, random_state=RANDOM_STATE)
...
for feature, constraint in economic_constraints.items():
    if constraint['type'] == 'sign':
        ...
        print(
            f"      {status} {feature:20} → Coef: {coef:+.4f} (esperado: {constraint['sign']:+d})"
        )
...
results = pd.DataFrame(
    {
        ...
        'Compliance': [
            'N/A',
            f'{baseline_compliance:.1f}%',
            'N/A',
            f'{economic_compliance:.1f}%',
        ],
    }
)
...
print(
    f'   Perda de AUC vs Teacher:        {(teacher_auc - economic_auc)/teacher_auc*100:.1f}%'
)
print(
    f'   Ganho de AUC vs Baseline:       +{(economic_auc - baseline_auc)*100:.1f} pp'
)
...
print(
    '   2. Execute 03_stability_analysis_demo.py para análise de estabilidade'
)
