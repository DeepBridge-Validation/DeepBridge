...
print(
    f"Result keys: {list(fairness_result.keys()) if isinstance(fairness_result, dict) else 'N/A'}"
)
print(
    f"Result empty? {not fairness_result if isinstance(fairness_result, dict) else 'N/A'}"
)
...
try:
    ...
    if hasattr(fairness_result, 'save_html'):
        ...
        print(
            f'   💾 Size: {interactive_html_path.stat().st_size / 1024:.1f} KB'
        )
    else:
        print(f"   ⚠️  FairnessResult doesn't have save_html method")
except Exception as e:
